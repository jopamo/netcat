...
int mbedtls_test_rnd_buffer_rand(void* rng_state, unsigned char* output, size_t len) {
    ...
    if (len - use_len > 0) {
        if (info->fallback_f_rng != NULL) {
            return info->fallback_f_rng(info->fallback_p_rng, output + use_len, len - use_len);
        }
        else {
...
int mbedtls_test_rnd_pseudo_rand(void* rng_state, unsigned char* output, size_t len) {
    mbedtls_test_rnd_pseudo_info* info = (mbedtls_test_rnd_pseudo_info*)rng_state;
