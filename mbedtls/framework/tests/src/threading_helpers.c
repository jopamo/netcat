...
static int threading_thread_create_fail(mbedtls_test_thread_t* thread, void* (*thread_func)(void*), void* thread_data) {
    (void)thread;
    (void)thread_func;
    (void)thread_data;
 ...
 */
static mbedtls_threading_mutex_t* mutex_container(mbedtls_platform_mutex_t* platform_mutex) {
...
static void mbedtls_test_mutex_usage_error(mbedtls_platform_mutex_t* mutex, const char* msg) {
    (void)mutex;
...
void mbedtls_test_mutex_usage_check(void) {
    if (mutex_functions.lock(&mbedtls_test_mutex_mutex) == 0) {
        ...
        if (mbedtls_test_get_mutex_usage_error() != NULL && mbedtls_test_get_result() != MBEDTLS_TEST_RESULT_FAILED) {
