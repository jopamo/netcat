...
int mbedtls_test_read_mpi_core(mbedtls_mpi_uint** pX, size_t* plimbs, const char* input) {
    /* Sanity check */
    if (*pX != NULL) {
    ...
    *pX = mbedtls_calloc(*plimbs, sizeof(**pX));
    ...
    TEST_ASSERT(mbedtls_test_unhexify(byte_start, byte_len, input, &byte_len) == 0);
...
int mbedtls_test_read_mpi_modulus(mbedtls_mpi_mod_modulus* N, const char* s, mbedtls_mpi_mod_rep_selector int_rep) {
