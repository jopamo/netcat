...
static int check_key_attributes_sanity(mbedtls_svc_key_id_t key, int key_destroyable) {
    int ok = 0;
    ...
    if (PSA_KEY_LIFETIME_IS_VOLATILE(lifetime)) {
        TEST_ASSERT((PSA_KEY_ID_VOLATILE_MIN <= MBEDTLS_SVC_KEY_ID_GET_KEY_ID(id)) &&
                    (MBEDTLS_SVC_KEY_ID_GET_KEY_ID(id) <= PSA_KEY_ID_VOLATILE_MAX));
    }
    else {
        TEST_ASSERT((PSA_KEY_ID_USER_MIN <= MBEDTLS_SVC_KEY_ID_GET_KEY_ID(id)) &&
...
static int exercise_mac_key(mbedtls_svc_key_id_t key, psa_key_usage_t usage, psa_algorithm_t alg, int key_destroyable) {
    ...
    if (usage & PSA_KEY_USAGE_SIGN_HASH) {
        ...
        PSA_ASSERT(psa_mac_sign_finish(&operation, mac, sizeof(mac), &mac_length));
    }

    if (usage & PSA_KEY_USAGE_VERIFY_HASH) {
        psa_status_t verify_status = (usage & PSA_KEY_USAGE_SIGN_HASH ? PSA_SUCCESS : PSA_ERROR_INVALID_SIGNATURE);
        ...
        PSA_ASSERT(status);
        PSA_ASSERT(psa_mac_update(&operation, input, sizeof(input)));
        TEST_EQUAL(psa_mac_verify_finish(&operation, mac, mac_length), verify_status);
    }
...
static int exercise_cipher_key(mbedtls_svc_key_id_t key,
    ...
    if (usage & PSA_KEY_USAGE_ENCRYPT) {
        ...
        PSA_ASSERT(status);
        if (iv_length != 0) {
            PSA_ASSERT(psa_cipher_generate_iv(&operation, iv, sizeof(iv), &iv_length));
        }
        PSA_ASSERT(psa_cipher_update(&operation, plaintext, sizeof(plaintext), ciphertext, sizeof(ciphertext),
                                     &ciphertext_length));
        PSA_ASSERT(psa_cipher_finish(&operation, ciphertext + ciphertext_length, sizeof(ciphertext) - ciphertext_length,
                                     &part_length));
    ...
    if (usage & PSA_KEY_USAGE_DECRYPT) {
        ...
        PSA_ASSERT(
            psa_cipher_update(&operation, ciphertext, ciphertext_length, decrypted, sizeof(decrypted), &part_length));
        status = psa_cipher_finish(&operation, decrypted + part_length, sizeof(decrypted) - part_length, &part_length);
        /* For a stream cipher, all inputs are valid. For a block cipher,
         * if the input is some arbitrary data rather than an actual
           ciphertext, a padding error is likely.  */
        if (maybe_invalid_padding) {
            TEST_ASSERT(status == PSA_SUCCESS || status == PSA_ERROR_INVALID_PADDING);
        }
        else {
            PSA_ASSERT(status);
        }
...
static int exercise_aead_key(mbedtls_svc_key_id_t key,
    ...
    if (usage & PSA_KEY_USAGE_ENCRYPT) {
        status = psa_aead_encrypt(key, alg, nonce, nonce_length, NULL, 0, plaintext, sizeof(plaintext), ciphertext,
    ...
    if (usage & PSA_KEY_USAGE_DECRYPT) {
        psa_status_t verify_status = (usage & PSA_KEY_USAGE_ENCRYPT ? PSA_SUCCESS : PSA_ERROR_INVALID_SIGNATURE);
        status = psa_aead_decrypt(key, alg, nonce, nonce_length, NULL, 0, ciphertext, ciphertext_length, plaintext,
...
static int can_sign_or_verify_message(psa_key_usage_t usage, psa_algorithm_t alg) {
    ...
    return usage & (PSA_KEY_USAGE_SIGN_MESSAGE | PSA_KEY_USAGE_VERIFY_MESSAGE);
}
...
static int exercise_signature_key(mbedtls_svc_key_id_t key,
    ...
    if (PSA_ALG_IS_SIGN_HASH(alg) && hash_alg == PSA_ALG_ANY_HASH &&
        usage & (PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_VERIFY_HASH | PSA_KEY_USAGE_SIGN_MESSAGE |
    ...
    if (usage & (PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_VERIFY_HASH) && PSA_ALG_IS_SIGN_HASH(alg)) {
        ...
        if (usage & PSA_KEY_USAGE_SIGN_HASH) {
            status = psa_sign_hash(key, alg, payload, payload_length, signature, sizeof(signature), &signature_length);
            if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
                /* The key has been destroyed. */
                return 1;
            }
        ...
        if (usage & PSA_KEY_USAGE_VERIFY_HASH) {
            psa_status_t verify_status = (usage & PSA_KEY_USAGE_SIGN_HASH ? PSA_SUCCESS : PSA_ERROR_INVALID_SIGNATURE);
            status = psa_verify_hash(key, alg, payload, payload_length, signature, signature_length);
            if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
                /* The key has been destroyed. */
                return 1;
            }
    ...
    if (can_sign_or_verify_message(usage, alg)) {
        ...
        if (usage & PSA_KEY_USAGE_SIGN_MESSAGE) {
            status =
                psa_sign_message(key, alg, message, message_length, signature, sizeof(signature), &signature_length);
            if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
                /* The key has been destroyed. */
                return 1;
            }
        ...
        if (usage & PSA_KEY_USAGE_VERIFY_MESSAGE) {
            psa_status_t verify_status =
                (usage & PSA_KEY_USAGE_SIGN_MESSAGE ? PSA_SUCCESS : PSA_ERROR_INVALID_SIGNATURE);
            status = psa_verify_message(key, alg, message, message_length, signature, signature_length);
            if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
                /* The key has been destroyed. */
                return 1;
            }
...
static int exercise_asymmetric_encryption_key(mbedtls_svc_key_id_t key,
                                              ...
                                              psa_algorithm_t alg,
                                              int key_destroyable) {
    unsigned char plaintext[PSA_ASYMMETRIC_DECRYPT_OUTPUT_MAX_SIZE] = "Hello, world...";
    unsigned char ciphertext[PSA_ASYMMETRIC_ENCRYPT_OUTPUT_MAX_SIZE] = "(wabblewebblewibblewobblewubble)";
    size_t ciphertext_length = sizeof(ciphertext);
    size_t plaintext_length = 16;
    psa_status_t status = PSA_SUCCESS;
    if (usage & PSA_KEY_USAGE_ENCRYPT) {
        status = psa_asymmetric_encrypt(key, alg, plaintext, plaintext_length, NULL, 0, ciphertext, sizeof(ciphertext),
                                        &ciphertext_length);
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            /* The key has been destroyed. */
            return 1;
    ...
    if (usage & PSA_KEY_USAGE_DECRYPT) {
        status = psa_asymmetric_decrypt(key, alg, ciphertext, ciphertext_length, NULL, 0, plaintext, sizeof(plaintext),
                                        &plaintext_length);
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            /* The key has been destroyed. */
            return 1;
        ...
        TEST_ASSERT(status == PSA_SUCCESS ||
                    ((usage & PSA_KEY_USAGE_ENCRYPT) == 0 &&
                     (status == PSA_ERROR_INVALID_ARGUMENT || status == PSA_ERROR_INVALID_PADDING)));
    }
...
int mbedtls_test_psa_setup_key_derivation_wrap(psa_key_derivation_operation_t* operation,
    ...
    if (PSA_ALG_IS_HKDF(alg)) {
        PSA_ASSERT(psa_key_derivation_input_bytes(operation, PSA_KEY_DERIVATION_INPUT_SALT, input1, input1_length));
        status = psa_key_derivation_input_key(operation, PSA_KEY_DERIVATION_INPUT_SECRET, key);
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            /* The key has been destroyed. */
            return 1;
        }
        PSA_ASSERT(status);
        PSA_ASSERT(psa_key_derivation_input_bytes(operation, PSA_KEY_DERIVATION_INPUT_INFO, input2, input2_length));
    }
        ...
        PSA_ASSERT(psa_key_derivation_input_bytes(operation, PSA_KEY_DERIVATION_INPUT_SEED, input1, input1_length));
        status = psa_key_derivation_input_key(operation, PSA_KEY_DERIVATION_INPUT_SECRET, key);
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            /* The key has been destroyed. */
            return 1;
        ...
        PSA_ASSERT(status);
        PSA_ASSERT(psa_key_derivation_input_bytes(operation, PSA_KEY_DERIVATION_INPUT_LABEL, input2, input2_length));
    }
    else if (PSA_ALG_IS_PBKDF2(alg)) {
        PSA_ASSERT(psa_key_derivation_input_integer(operation, PSA_KEY_DERIVATION_INPUT_COST, 1U));
        PSA_ASSERT(psa_key_derivation_input_bytes(operation, PSA_KEY_DERIVATION_INPUT_SALT, input2, input2_length));
        status = psa_key_derivation_input_key(operation, PSA_KEY_DERIVATION_INPUT_PASSWORD, key);
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            /* The key has been destroyed. */
            return 1;
        }
        ...
        PSA_ASSERT(psa_key_derivation_input_bytes(operation, PSA_KEY_DERIVATION_INPUT_SECRET, input1, input1_length));
    }
    else {
...
static int exercise_key_derivation_key(mbedtls_svc_key_id_t key,
    ...
    if (usage & PSA_KEY_USAGE_DERIVE) {
        if (!mbedtls_test_psa_setup_key_derivation_wrap(&operation, key, alg, input1, input1_length, input2,
                                                        input2_length, capacity, key_destroyable)) {
            goto exit;
        }

        psa_status_t status = psa_key_derivation_output_bytes(&operation, output, capacity);
...
psa_status_t mbedtls_test_psa_key_agreement_with_self(psa_key_derivation_operation_t* operation,
                                                      mbedtls_svc_key_id_t key,
                                                      int key_destroyable) {
    ...
    status = psa_export_public_key(key, public_key, public_key_length, &public_key_length);
    if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
        /* The key has been destroyed. */
        status = PSA_SUCCESS;
        goto exit;
    ...
    status = psa_key_derivation_key_agreement(operation, PSA_KEY_DERIVATION_INPUT_SECRET, key, public_key,
                                              public_key_length);
    if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
        /* The key has been destroyed. */
        status = PSA_SUCCESS;
...
/* We need two keys to exercise key agreement. Exercise the
 * private key against its own public key. */
psa_status_t mbedtls_test_psa_raw_key_agreement_with_self(psa_algorithm_t alg,
                                                          mbedtls_svc_key_id_t key,
                                                          int key_destroyable) {
    psa_key_type_t private_key_type;
    psa_key_type_t public_key_type;
    ...
    status = psa_raw_key_agreement(alg, key, public_key, public_key_length, output, sizeof(output), &output_length);
    if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
        /* The key has been destroyed. */
        status = PSA_SUCCESS;
        goto exit;
    ...
    if (status == PSA_SUCCESS) {
        TEST_ASSERT(output_length <= PSA_RAW_KEY_AGREEMENT_OUTPUT_SIZE(private_key_type, key_bits));
    ...
    status = psa_key_agreement(key, public_key, public_key_length, alg, &shared_secret_attributes, &shared_secret_id);
    ...
    if (status == PSA_SUCCESS) {
        ...
        exported_size =
            PSA_EXPORT_KEY_OUTPUT_SIZE(psa_get_key_type(&export_attributes), psa_get_key_bits(&export_attributes));
        ...
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            ...
            PSA_ASSERT(status);
            TEST_MEMORY_COMPARE(exported, exported_length, output, output_length);
        }
    }
    ...
    if (PSA_KEY_TYPE_IS_ECC(private_key_type)) {
        ...
        status = psa_key_agreement_iop_setup(&iop_operation, key, public_key, public_key_length, alg,
        ...
        TEST_EQUAL(psa_key_agreement_iop_setup(&iop_operation, key, public_key, public_key_length, alg,
                                               &shared_secret_attributes),
...
static int exercise_raw_key_agreement_key(mbedtls_svc_key_id_t key,
    ...
    if (usage & PSA_KEY_USAGE_DERIVE) {
        /* We need two keys to exercise key agreement. Exercise the
         * private key against its own public key. */
        PSA_ASSERT(mbedtls_test_psa_raw_key_agreement_with_self(alg, key, key_destroyable));
    }
    ok = 1;
...
static int exercise_key_agreement_key(mbedtls_svc_key_id_t key,
    ...
    if (usage & PSA_KEY_USAGE_DERIVE) {
        ...
        if (PSA_ALG_IS_TLS12_PRF(kdf_alg) || PSA_ALG_IS_TLS12_PSK_TO_MS(kdf_alg)) {
            PSA_ASSERT(psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_SEED, input, sizeof(input)));
        }

        if (PSA_ALG_IS_HKDF_EXTRACT(kdf_alg)) {
            PSA_ASSERT(psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_SALT, input, sizeof(input)));
        }
        ...
        TEST_EQUAL(mbedtls_test_psa_key_agreement_with_self(&operation, key, key_destroyable),
        ...
        if (PSA_ALG_IS_TLS12_PRF(kdf_alg) || PSA_ALG_IS_TLS12_PSK_TO_MS(kdf_alg)) {
            PSA_ASSERT(
                psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_LABEL, input, sizeof(input)));
        }
        else if (PSA_ALG_IS_HKDF(kdf_alg) || PSA_ALG_IS_HKDF_EXPAND(kdf_alg)) {
            PSA_ASSERT(psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_INFO, input, sizeof(input)));
        }
        PSA_ASSERT(psa_key_derivation_output_bytes(&operation, output, sizeof(output)));
        PSA_ASSERT(psa_key_derivation_abort(&operation));
    }
    ok = 1;
...
int mbedtls_test_psa_exported_key_sanity_check(psa_key_type_t type,
    ...
    if (PSA_KEY_TYPE_IS_UNSTRUCTURED(type)) {
         ...
         *   }
         */
        TEST_EQUAL(mbedtls_asn1_get_tag(&p, end, &len, MBEDTLS_ASN1_SEQUENCE | MBEDTLS_ASN1_CONSTRUCTED), 0);
        ...
        if (PSA_KEY_TYPE_IS_ECC_KEY_PAIR(type)) {
            ...
            TEST_ASSERT(exported_length <= PSA_EXPORT_PUBLIC_KEY_OUTPUT_SIZE(type, bits));
...
static int exercise_export_key(mbedtls_svc_key_id_t key, psa_key_usage_t usage, int key_destroyable) {
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
    uint8_t* exported = NULL;
    size_t exported_size = 0;
    size_t exported_length = 0;
    ...
    exported_size = PSA_EXPORT_KEY_OUTPUT_SIZE(psa_get_key_type(&attributes), psa_get_key_bits(&attributes));
    TEST_CALLOC(exported, exported_size);
    ...
    else if ((usage & PSA_KEY_USAGE_EXPORT) == 0 && !PSA_KEY_TYPE_IS_PUBLIC_KEY(psa_get_key_type(&attributes))) {
        TEST_EQUAL(status, PSA_ERROR_NOT_PERMITTED);
        ok = 1;
        goto exit;
    }
    PSA_ASSERT(status);
    ok = mbedtls_test_psa_exported_key_sanity_check(psa_get_key_type(&attributes), psa_get_key_bits(&attributes),
...
static int exercise_export_public_key(mbedtls_svc_key_id_t key, int key_destroyable) {
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
    ...
    if (!PSA_KEY_TYPE_IS_ASYMMETRIC(psa_get_key_type(&attributes))) {
        ...
        status = psa_export_public_key(key, exported, exported_size, &exported_length);
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            /* The key has been destroyed. */
            ok = 1;
            goto exit;
    ...
    public_type = PSA_KEY_TYPE_PUBLIC_KEY_OF_KEY_PAIR(psa_get_key_type(&attributes));
    exported_size = PSA_EXPORT_PUBLIC_KEY_OUTPUT_SIZE(public_type, psa_get_key_bits(&attributes));
    TEST_CALLOC(exported, exported_size);
    ...
    PSA_ASSERT(status);
    ok = mbedtls_test_psa_exported_key_sanity_check(public_type, psa_get_key_bits(&attributes), exported,
                                                    exported_length);
...
int mbedtls_test_psa_exercise_key(mbedtls_svc_key_id_t key,
    ...
    if (alg == 0) {
        ...
        ok = exercise_asymmetric_encryption_key(key, usage, alg, key_destroyable);
    }
    ...
    ok = ok && exercise_export_key(key, usage, key_destroyable);
    ok = ok && exercise_export_public_key(key, key_destroyable);

exit:
    return ok;
}

psa_key_usage_t mbedtls_test_psa_usage_to_exercise(psa_key_type_t type, psa_algorithm_t alg) {
    if (PSA_ALG_IS_MAC(alg) || PSA_ALG_IS_SIGN(alg)) {
        if (PSA_ALG_IS_SIGN_HASH(alg)) {
            if (PSA_ALG_SIGN_GET_HASH(alg)) {
                return PSA_KEY_TYPE_IS_PUBLIC_KEY(type) ? PSA_KEY_USAGE_VERIFY_HASH | PSA_KEY_USAGE_VERIFY_MESSAGE
                                                        : PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_VERIFY_HASH |
                                                              PSA_KEY_USAGE_SIGN_MESSAGE | PSA_KEY_USAGE_VERIFY_MESSAGE;
            }
        }
        else if (PSA_ALG_IS_SIGN_MESSAGE(alg)) {
            return PSA_KEY_TYPE_IS_PUBLIC_KEY(type) ? PSA_KEY_USAGE_VERIFY_MESSAGE
                                                    : PSA_KEY_USAGE_SIGN_MESSAGE | PSA_KEY_USAGE_VERIFY_MESSAGE;
        }

        return PSA_KEY_TYPE_IS_PUBLIC_KEY(type) ? PSA_KEY_USAGE_VERIFY_HASH
                                                : PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_VERIFY_HASH;
    }
    else if (PSA_ALG_IS_CIPHER(alg) || PSA_ALG_IS_AEAD(alg) || PSA_ALG_IS_ASYMMETRIC_ENCRYPTION(alg)) {
        return PSA_KEY_TYPE_IS_PUBLIC_KEY(type) ? PSA_KEY_USAGE_ENCRYPT : PSA_KEY_USAGE_ENCRYPT | PSA_KEY_USAGE_DECRYPT;
    }
    else if (PSA_ALG_IS_KEY_DERIVATION(alg) || PSA_ALG_IS_KEY_AGREEMENT(alg)) {
...
#if defined(MBEDTLS_PK_C)
int mbedtls_test_key_consistency_psa_pk(mbedtls_svc_key_id_t psa_key, const mbedtls_pk_context* pk) {
    ...
    TEST_ASSERT(PSA_KEY_TYPE_IS_PUBLIC_KEY(psa_type) || PSA_KEY_TYPE_IS_KEY_PAIR(psa_type));
    ...
    switch (pk_type) {
        ...
        case MBEDTLS_PK_RSA:
            ...
            uint8_t* cursor = end;
            TEST_LE_U(1, mbedtls_rsa_write_pubkey(rsa, pk_public_buffer, &cursor));
            pk_public = cursor;
        ...
        case MBEDTLS_PK_ECDSA:
            ...
            TEST_EQUAL(mbedtls_ecp_write_public_key(ec, MBEDTLS_ECP_PF_UNCOMPRESSED, &pk_public_length,
                                                    pk_public_buffer, sizeof(pk_public_buffer)),
                       0);
        ...
        case MBEDTLS_PK_OPAQUE:
            ...
            TEST_EQUAL(PSA_KEY_TYPE_PUBLIC_KEY_OF_KEY_PAIR(psa_type), PSA_KEY_TYPE_PUBLIC_KEY_OF_KEY_PAIR(pk_psa_type));
            PSA_ASSERT(psa_export_public_key(psa_key, pk_public_buffer, sizeof(pk_public_buffer), &pk_public_length));
            pk_public = pk_public_buffer;
            break;
    ...
    PSA_ASSERT(psa_export_public_key(psa_key, psa_public, sizeof(psa_public), &psa_public_length));
    TEST_MEMORY_COMPARE(pk_public, pk_public_length, psa_public, psa_public_length);
