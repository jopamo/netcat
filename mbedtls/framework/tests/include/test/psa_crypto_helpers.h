 ...
 */
#define TEST_USES_KEY_ID(key_id) TEST_ASSERT(mbedtls_test_uses_key_id(key_id))
 ...
 */
#define RECORD_STATUS(string, expr) mbedtls_test_record_status((expr), string, __FILE__, __LINE__, #expr)
 ...
 */
#if defined(MBEDTLS_AES_ALT) || defined(MBEDTLS_AES_SETKEY_ENC_ALT) || defined(MBEDTLS_PSA_ACCEL_KEY_TYPE_AES)
...
#define MBEDTLS_TEST_PSA_SKIP_IF_ALT_AES_192(key_type, key_bits)                                      \
    do {                                                                                              \
        if ((MBEDTLS_TEST_HAVE_ACCEL_AES) && ((key_type) == PSA_KEY_TYPE_AES) && (key_bits == 192)) { \
...
#define MBEDTLS_TEST_PSA_SKIP_IF_ALT_GCM_NOT_12BYTES_NONCE(alg, nonce_length)                                 \
    do {                                                                                                      \
        if ((MBEDTLS_TEST_HAVE_ACCEL_GCM) &&                                                                  \
            (PSA_ALG_AEAD_WITH_SHORTENED_TAG((alg), 0) == PSA_ALG_AEAD_WITH_SHORTENED_TAG(PSA_ALG_GCM, 0)) && \
            ((nonce_length) != 12)) {                                                                         \
...
#if !defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG) && defined(MBEDTLS_CTR_DRBG_C) && defined(MBEDTLS_CTR_DRBG_USE_PSA_CRYPTO)
 ...
 * psa_crypto_init().
 */
#define MBEDTLS_TEST_PSA_INTERNAL_KEYS MBEDTLS_TEST_PSA_INTERNAL_KEYS_FOR_DRBG
...
#if !defined(MBEDTLS_PSA_STATIC_KEY_SLOTS) || defined(MBEDTLS_TEST_STATIC_KEY_SLOTS_SUPPORT_RSA_4096)
