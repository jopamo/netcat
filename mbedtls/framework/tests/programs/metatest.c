...
void (*volatile do_nothing_with_object_but_the_compiler_does_not_know)(void*) = do_nothing_with_object;
...
static void test_memory_poison(const char* name) {
    ...
    char direction = 'r';
    if (sscanf(name,
               "%*[^0-9]%" MBEDTLS_PRINTF_SIZET "%*[^0-9]%" MBEDTLS_PRINTF_SIZET "%*[^0-9]%" MBEDTLS_PRINTF_SIZET "_%c",
    ...
    if (start > sizeof(aligned.buf)) {
        mbedtls_fprintf(stderr, "%s: start=%" MBEDTLS_PRINTF_SIZET " > size=%" MBEDTLS_PRINTF_SIZET, __func__, start,
                        sizeof(aligned.buf));
        return;
    }
    if (start + count > sizeof(aligned.buf)) {
        mbedtls_fprintf(stderr, "%s: start+count=%" MBEDTLS_PRINTF_SIZET " > size=%" MBEDTLS_PRINTF_SIZET, __func__,
                        start + count, sizeof(aligned.buf));
        return;
    }
    if (offset >= count) {
        mbedtls_fprintf(stderr, "%s: offset=%" MBEDTLS_PRINTF_SIZET " >= count=%" MBEDTLS_PRINTF_SIZET, __func__,
                        offset, count);
        return;
    }
 ...
 * platform values.
 */
metatest_t metatests[] = {{"test_fail", "any", meta_test_fail},
...
int main(int argc, char* argv[]) {
    ...
    for (const metatest_t* p = metatests; p->name != NULL; p++) {
        if (strcmp(argv[1], p->name) == 0) {
            ...
            mbedtls_exit(result == MBEDTLS_TEST_RESULT_SUCCESS ? MBEDTLS_EXIT_SUCCESS : MBEDTLS_EXIT_FAILURE);
        }
    }
