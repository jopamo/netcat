...
#define PSA_KEY_TYPE_IS_SPAKE2P_KEY_PAIR(type) \
    (((type) & ~PSA_KEY_TYPE_ECC_CURVE_MASK) == PSA_KEY_TYPE_SPAKE2P_KEY_PAIR_BASE)

/** Whether a key type is a SPAKE2+ public key type. */
#define PSA_KEY_TYPE_IS_SPAKE2P_PUBLIC_KEY(type) \
    (((type) & ~PSA_KEY_TYPE_ECC_CURVE_MASK) == PSA_KEY_TYPE_SPAKE2P_PUBLIC_KEY_BASE)

/** Whether a key type is a SPAKE2+ key pair or public key type. */
#define PSA_KEY_TYPE_IS_SPAKE2P(type) \
    ((PSA_KEY_TYPE_PUBLIC_KEY_OF_KEY_PAIR(type) & ~PSA_KEY_TYPE_ECC_CURVE_MASK) == PSA_KEY_TYPE_SPAKE2P_PUBLIC_KEY_BASE)
...
#define PSA_ALG_IS_SPAKE2P(alg) \
    (PSA_ALG_IS_SPAKE2P_HMAC(alg) || PSA_ALG_IS_SPAKE2P_CMAC(alg) || (alg) == PSA_ALG_SPAKE2P_MATTER)
...
#define PSA_PAKE_OUTPUT_SIZE(alg, primitive, output_step)                                              \
    (PSA_ALG_IS_JPAKE(alg) &&                                                                          \
             primitive == PSA_PAKE_PRIMITIVE(PSA_PAKE_PRIMITIVE_TYPE_ECC, PSA_ECC_FAMILY_SECP_R1, 256) \
...
#define PSA_PAKE_OPERATION_INIT                                            \
    {                                                                      \
        0, PSA_ALG_NONE, 0, PSA_PAKE_OPERATION_STAGE_SETUP, {0}, { { 0 } } \
    }
#endif
...
typedef enum psa_jpake_round { PSA_JPAKE_FIRST = 0, PSA_JPAKE_SECOND = 1, PSA_JPAKE_FINISHED = 2 } psa_jpake_round_t;

typedef enum psa_jpake_io_mode { PSA_JPAKE_INPUT = 0, PSA_JPAKE_OUTPUT = 1 } psa_jpake_io_mode_t;
