...
#define MBEDTLS_LMOTS_SIG_LEN(type)                                    \
    (MBEDTLS_LMOTS_TYPE_LEN + MBEDTLS_LMOTS_C_RANDOM_VALUE_LEN(type) + \
     (MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT(type) * MBEDTLS_LMOTS_N_HASH_LEN(type)))
...
#define MBEDTLS_LMS_SIG_LEN(type, otstype)                                                 \
    (MBEDTLS_LMOTS_Q_LEAF_ID_LEN + MBEDTLS_LMOTS_SIG_LEN(otstype) + MBEDTLS_LMS_TYPE_LEN + \
     (MBEDTLS_LMS_H_TREE_HEIGHT(type) * MBEDTLS_LMS_M_NODE_BYTES(type)))

#define MBEDTLS_LMS_PUBLIC_KEY_LEN(type) \
    (MBEDTLS_LMS_TYPE_LEN + MBEDTLS_LMOTS_TYPE_LEN + MBEDTLS_LMOTS_I_KEY_ID_LEN + MBEDTLS_LMS_M_NODE_BYTES(type))
 ...
 */
typedef enum { MBEDTLS_LMOTS_SHA256_N32_W8 = 4 } mbedtls_lmots_algorithm_type_t;
...
typedef struct {
    ...
    unsigned char MBEDTLS_PRIVATE(
        q_leaf_identifier[MBEDTLS_LMOTS_Q_LEAF_ID_LEN]);  /*!< Which
...
typedef struct {
    ...
    unsigned char MBEDTLS_PRIVATE(
        T_1_pub_key)[MBEDTLS_LMS_M_NODE_BYTES_MAX]; /*!< The public key, in
