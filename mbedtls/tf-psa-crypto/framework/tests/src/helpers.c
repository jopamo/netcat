...
static void mbedtls_test_set_result_internal(mbedtls_test_result_t result,
                                             const char* test,
                                             int line_no,
                                             const char* filename) {
...
#if defined(MBEDTLS_TEST_HOOKS) && defined(MBEDTLS_PSA_CRYPTO_C) && !defined(MBEDTLS_PSA_ASSUME_EXCLUSIVE_BUFFERS) && \
...
int mbedtls_test_equal(const char* test,
                       int line_no,
                       const char* filename,
    ...
    if (mbedtls_test_info.result != MBEDTLS_TEST_RESULT_FAILED) {
        ...
        mbedtls_test_fail_internal(test, line_no, filename);
        (void)mbedtls_snprintf(buf, sizeof(buf), "lhs = 0x%016llx = %lld", value1, (long long)value1);
        mbedtls_test_set_line1_internal(buf);
        (void)mbedtls_snprintf(buf, sizeof(buf), "rhs = 0x%016llx = %lld", value2, (long long)value2);
...
int mbedtls_test_le_u(const char* test,
                      int line_no,
                      const char* filename,
                      unsigned long long value1,
                      unsigned long long value2) {
    ...
    if (mbedtls_test_info.result != MBEDTLS_TEST_RESULT_FAILED) {
        ...
        char buf[MBEDTLS_TEST_LINE_LENGTH];
        mbedtls_test_fail_internal(test, line_no, filename);
        (void)mbedtls_snprintf(buf, sizeof(buf), "lhs = 0x%016llx = %llu", value1, value1);
        mbedtls_test_set_line1_internal(buf);
        (void)mbedtls_snprintf(buf, sizeof(buf), "rhs = 0x%016llx = %llu", value2, value2);
        mbedtls_test_set_line2_internal(buf);
    }
...
int mbedtls_test_le_s(const char* test, int line_no, const char* filename, long long value1, long long value2) {
    TEST_CF_PUBLIC(&value1, sizeof(value1));
    TEST_CF_PUBLIC(&value2, sizeof(value2));
    ...
    if (mbedtls_test_info.result != MBEDTLS_TEST_RESULT_FAILED) {
        ...
        char buf[MBEDTLS_TEST_LINE_LENGTH];
        mbedtls_test_fail_internal(test, line_no, filename);
        (void)mbedtls_snprintf(buf, sizeof(buf), "lhs = 0x%016llx = %lld", (unsigned long long)value1, value1);
        mbedtls_test_set_line1_internal(buf);
        (void)mbedtls_snprintf(buf, sizeof(buf), "rhs = 0x%016llx = %lld", (unsigned long long)value2, value2);
        mbedtls_test_set_line2_internal(buf);
    }
...
int mbedtls_test_unhexify(unsigned char* obuf, size_t obufmax, const char* ibuf, size_t* len) {
...
void mbedtls_test_err_add_check(int high, int low, const char* file, int line) {
    ...
    if (high > -0x1000 && high != 0) {
         ...
         */
        mbedtls_test_fail("'high' error code is greater than 15 bits", line, file);
    }
    else if ((high & 0x7F) != 0) {
         ...
         */
        mbedtls_test_fail("'low' error code is greater than 7 bits", line, file);
    }
    else if (low > 0) {
