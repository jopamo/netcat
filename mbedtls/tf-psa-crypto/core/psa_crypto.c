...
psa_status_t psa_import_key_into_slot(const psa_key_attributes_t* attributes,
    ...
    if (key_type_is_raw_bytes(type)) {
        ...
        *key_buffer_length = data_length;
        ...
        if (PSA_KEY_TYPE_IS_DH(type)) {
            ...
            return mbedtls_psa_ffdh_import_key(attributes, data, data_length, key_buffer, key_buffer_size,
        ...
        * defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_DH_PUBLIC_KEY) */
        ...
        if (PSA_KEY_TYPE_IS_ECC(type)) {
            return mbedtls_psa_ecp_import_key(attributes, data, data_length, key_buffer, key_buffer_size,
                                              key_buffer_length, bits);
        }
    ...
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
        if (PSA_KEY_TYPE_IS_RSA(type)) {
            return mbedtls_psa_rsa_import_key(attributes, data, data_length, key_buffer, key_buffer_size,
                                              key_buffer_length, bits);
        }
...
static int psa_key_algorithm_permits(psa_key_type_t key_type,
    ...
    if (policy_alg == PSA_ALG_JPAKE_BETA && PSA_ALG_IS_JPAKE(requested_alg)) {
...
psa_status_t psa_export_public_key_internal(const psa_key_attributes_t* attributes,
    ...
    if (PSA_KEY_TYPE_IS_PUBLIC_KEY(type) &&
        ...
        * defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY) */
        ...
        return mbedtls_psa_ffdh_export_public_key(attributes, key_buffer, key_buffer_size, data, data_size,
                                                  data_length);
#else
        return PSA_ERROR_NOT_SUPPORTED;
...
psa_status_t psa_export_public_key(mbedtls_svc_key_id_t key,
    ...
    *data_length = 0;
    ...
    status = psa_driver_wrapper_export_public_key(&slot->attr, slot->key.data, slot->key.bytes, data, data_size,
                                                  data_length);
...
psa_status_t psa_export_public_key_iop_setup(psa_export_public_key_iop_t* operation, mbedtls_svc_key_id_t key) {
    ...
    status = psa_get_and_lock_transparent_key_slot_with_policy(key, &slot, 0, 0);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
    ...
    status =
        mbedtls_psa_ecp_export_public_key_iop_setup(&operation->ctx, slot->key.data, slot->key.bytes, &key_attributes);
...
psa_status_t psa_export_public_key_iop_complete(psa_export_public_key_iop_t* operation,
    ...
    status = mbedtls_psa_ecp_export_public_key_iop_complete(&operation->ctx, data, data_size, data_length);
...
static psa_status_t psa_validate_key_policy(const psa_key_policy_t* policy) {
    ...
    if ((policy->usage & ~(PSA_KEY_USAGE_EXPORT | PSA_KEY_USAGE_COPY | PSA_KEY_USAGE_ENCRYPT | PSA_KEY_USAGE_DECRYPT |
                           ...
                           PSA_KEY_USAGE_VERIFY_HASH | PSA_KEY_USAGE_VERIFY_DERIVATION | PSA_KEY_USAGE_DERIVE)) != 0) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
 ...
 */
static psa_status_t psa_start_key_creation(const psa_key_attributes_t* attributes, psa_key_slot_t** p_slot) {
    psa_status_t status;
 ...
 *         You must call psa_fail_key_creation() to wipe and free the slot.
 */
static psa_status_t psa_finish_key_creation(psa_key_slot_t* slot, mbedtls_svc_key_id_t* key) {
...
psa_status_t psa_import_key(const psa_key_attributes_t* attributes,
    ...
    *key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    bits = slot->attr.bits;
    status = psa_driver_wrapper_import_key(attributes, data, data_length, slot->key.data, slot->key.bytes,
                                           &slot->key.bytes, &bits);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
psa_status_t psa_copy_key(mbedtls_svc_key_id_t source_key,
    ...
    *target_key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    if (psa_key_lifetime_is_external(actual_attributes.lifetime)) {
        ...
        status = psa_driver_wrapper_copy_key(&actual_attributes, source_slot->key.data, source_slot->key.bytes,
                                             target_slot->key.data, target_slot->key.bytes, &target_slot->key.bytes);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
    }
...
psa_status_t psa_hash_compare(psa_algorithm_t alg,
    ...
    LOCAL_INPUT_ALLOC(input_external, input_length, input);
    status = psa_driver_wrapper_hash_compute(alg, input, input_length, actual_hash, sizeof(actual_hash),
                                             &actual_hash_length);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
static psa_status_t psa_mac_compute_internal(mbedtls_svc_key_id_t key,
    ...
    status = psa_driver_wrapper_mac_compute(&slot->attr, slot->key.data, slot->key.bytes, alg, input, input_length, mac,
...
static psa_status_t psa_sign_internal(mbedtls_svc_key_id_t key,
    ...
    *signature_length = 0;
    ...
    if (input_is_message) {
        status = psa_driver_wrapper_sign_message(&slot->attr, slot->key.data, slot->key.bytes, alg, input, input_length,
                                                 signature, signature_size, signature_length);
    }
    else {
        status = psa_driver_wrapper_sign_hash(&slot->attr, slot->key.data, slot->key.bytes, alg, input, input_length,
                                              signature, signature_size, signature_length);
    }
...
static psa_status_t psa_verify_internal(mbedtls_svc_key_id_t key,
    ...
    if (input_is_message) {
        status = psa_driver_wrapper_verify_message(&slot->attr, slot->key.data, slot->key.bytes, alg, input,
                                                   input_length, signature, signature_length);
    }
    else {
        status = psa_driver_wrapper_verify_hash(&slot->attr, slot->key.data, slot->key.bytes, alg, input, input_length,
...
psa_status_t psa_sign_message_builtin(const psa_key_attributes_t* attributes,
    ...
    if (PSA_ALG_IS_SIGN_HASH(alg)) {
        ...
        status = psa_driver_wrapper_hash_compute(PSA_ALG_SIGN_GET_HASH(alg), input, input_length, hash, sizeof(hash),
                                                 &hash_length);
        ...
        return psa_driver_wrapper_sign_hash(attributes, key_buffer, key_buffer_size, alg, hash, hash_length, signature,
...
psa_status_t psa_verify_message_builtin(const psa_key_attributes_t* attributes,
    ...
    if (PSA_ALG_IS_SIGN_HASH(alg)) {
        ...
        return psa_driver_wrapper_verify_hash(attributes, key_buffer, key_buffer_size, alg, hash, hash_length,
                                              signature, signature_length);
    }
...
psa_status_t psa_sign_hash_builtin(const psa_key_attributes_t* attributes,
    ...
    if (attributes->type == PSA_KEY_TYPE_RSA_KEY_PAIR) {
        if (PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg) || PSA_ALG_IS_RSA_PSS(alg)) {
            ...
            return mbedtls_psa_rsa_sign_hash(attributes, key_buffer, key_buffer_size, alg, hash, hash_length, signature,
                                             signature_size, signature_length);
        ...
        * defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PSS) */
        ...
        if (PSA_ALG_IS_ECDSA(alg)) {
            ...
            return mbedtls_psa_ecdsa_sign_hash(attributes, key_buffer, key_buffer_size, alg, hash, hash_length,
                                               signature, signature_size, signature_length);
...
psa_status_t psa_verify_hash_builtin(const psa_key_attributes_t* attributes,
    ...
    if (PSA_KEY_TYPE_IS_RSA(attributes->type)) {
        if (PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg) || PSA_ALG_IS_RSA_PSS(alg)) {
#if defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PKCS1V15_SIGN) || defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PSS)
            return mbedtls_psa_rsa_verify_hash(attributes, key_buffer, key_buffer_size, alg, hash, hash_length,
                                               signature, signature_length);
#endif /* defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PKCS1V15_SIGN) || \
        * defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PSS) */
        }
        ...
        if (PSA_ALG_IS_ECDSA(alg)) {
#if defined(MBEDTLS_PSA_BUILTIN_ALG_ECDSA) || defined(MBEDTLS_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA)
            return mbedtls_psa_ecdsa_verify_hash(attributes, key_buffer, key_buffer_size, alg, hash, hash_length,
                                                 signature, signature_length);
#endif /* defined(MBEDTLS_PSA_BUILTIN_ALG_ECDSA) || \
        * defined(MBEDTLS_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA) */
        }
...
psa_status_t psa_asymmetric_encrypt(mbedtls_svc_key_id_t key,
    ...
    *output_length = 0;
    ...
    status = psa_driver_wrapper_asymmetric_encrypt(&slot->attr, slot->key.data, slot->key.bytes, alg, input,
                                                   input_length, salt, salt_length, output, output_size, output_length);
exit:
    unlock_status = psa_unregister_read_under_mutex(slot);
...
psa_status_t psa_asymmetric_decrypt(mbedtls_svc_key_id_t key,
    ...
    *output_length = 0;
    ...
    status = psa_driver_wrapper_asymmetric_decrypt(&slot->attr, slot->key.data, slot->key.bytes, alg, input,
                                                   input_length, salt, salt_length, output, output_size, output_length);
...
psa_status_t psa_sign_hash_start(psa_sign_hash_interruptible_operation_t* operation,
    ...
    status = psa_driver_wrapper_sign_hash_start(operation, &slot->attr, slot->key.data, slot->key.bytes, alg, hash,
                                                hash_length);
exit:
...
psa_status_t psa_verify_hash_start(psa_verify_hash_interruptible_operation_t* operation,
    ...
    status = psa_driver_wrapper_verify_hash_start(operation, &slot->attr, slot->key.data, slot->key.bytes, alg, hash,
...
#if (defined(MBEDTLS_PSA_BUILTIN_ALG_ECDH) || defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_GENERATE) || \
     defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_IMPORT) ||                                            \
     defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_EXPORT) ||                                            \
     defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY) || defined(MBEDTLS_PSA_BUILTIN_ALG_ECDSA) ||       \
     defined(MBEDTLS_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA)) &&                                                \
...
psa_status_t mbedtls_psa_sign_hash_start(mbedtls_psa_sign_hash_interruptible_operation_t* operation,
    ...
    status = mbedtls_psa_ecp_load_representation(attributes->type, attributes->bits, key_buffer, key_buffer_size,
                                                 &operation->ctx);
...
psa_status_t mbedtls_psa_sign_hash_complete(mbedtls_psa_sign_hash_interruptible_operation_t* operation,
    ...
    if (PSA_ALG_ECDSA_IS_DETERMINISTIC(operation->alg)) {
#if defined(MBEDTLS_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA)
        status = mbedtls_to_psa_error(mbedtls_ecdsa_sign_det_restartable(
            &operation->ctx->grp, &r, &s, &operation->ctx->d, operation->hash, operation->hash_length,
            operation->md_alg, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE, &operation->restart_ctx));
        ...
        status = mbedtls_to_psa_error(
            mbedtls_ecdsa_sign_restartable(&operation->ctx->grp, &r, &s, &operation->ctx->d, operation->hash,
                                           operation->hash_length, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE,
...
psa_status_t mbedtls_psa_verify_hash_complete(mbedtls_psa_verify_hash_interruptible_operation_t* operation) {
    ...
    status = mbedtls_to_psa_error(
        mbedtls_ecdsa_verify_restartable(&operation->ctx->grp, operation->hash, operation->hash_length,
                                         &operation->ctx->Q, &operation->r, &operation->s, &operation->restart_ctx));
...
psa_status_t psa_cipher_encrypt(mbedtls_svc_key_id_t key,
    ...
    status = psa_driver_wrapper_cipher_encrypt(
        &slot->attr, slot->key.data, slot->key.bytes, alg, local_iv, default_iv_length, input, input_length,
        psa_crypto_buffer_offset(output, default_iv_length), output_size - default_iv_length, output_length);
...
psa_status_t psa_cipher_decrypt(mbedtls_svc_key_id_t key,
    ...
    status = psa_driver_wrapper_cipher_decrypt(&slot->attr, slot->key.data, slot->key.bytes, alg, input, input_length,
...
psa_status_t psa_aead_encrypt(mbedtls_svc_key_id_t key,
    ...
    *ciphertext_length = 0;
    ...
    status = psa_driver_wrapper_aead_encrypt(&slot->attr, slot->key.data, slot->key.bytes, alg, nonce, nonce_length,
...
psa_status_t psa_aead_decrypt(mbedtls_svc_key_id_t key,
    ...
    *plaintext_length = 0;
    ...
    status = psa_driver_wrapper_aead_decrypt(&slot->attr, slot->key.data, slot->key.bytes, alg, nonce, nonce_length,
...
psa_status_t psa_aead_finish(psa_aead_operation_t* operation,
    ...
    *tag_length = tag_size;
    ...
    status = psa_driver_wrapper_aead_finish(operation, ciphertext, ciphertext_size, ciphertext_length, tag, tag_size,
                                            tag_length);
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF) || defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
static psa_status_t psa_key_derivation_tls12_prf_generate_next_block(psa_tls12_prf_key_derivation_t* tls12_prf,
                                                                     psa_algorithm_t alg) {
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_ECJPAKE_TO_PMS)
static psa_status_t psa_key_derivation_tls12_ecjpake_to_pms_read(psa_tls12_ecjpake_to_pms_t* ecjpake,
                                                                 uint8_t* output,
                                                                 size_t output_length) {
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    status = psa_hash_compute(PSA_ALG_SHA_256, ecjpake->data, PSA_TLS12_ECJPAKE_TO_PMS_DATA_SIZE, output, output_length,
                              &output_size);
    if (status != PSA_SUCCESS) {
        return status;
    }
...
static psa_status_t psa_key_derivation_pbkdf2_generate_block(psa_pbkdf2_key_derivation_t* pbkdf2,
    ...
    status = psa_driver_wrapper_mac_sign_setup(&mac_operation, attributes, pbkdf2->password, pbkdf2->password_length,
                                               prf_alg);
    if (status != PSA_SUCCESS) {
        goto cleanup;
    }
    ...
    for (i = 1; i < pbkdf2->input_cost; i++) {
        ...
        status = psa_driver_wrapper_mac_compute(attributes, pbkdf2->password, pbkdf2->password_length, prf_alg, U_i,
                                                prf_output_length, U_i, prf_output_length, &mac_output_length);
        if (status != PSA_SUCCESS) {
            goto cleanup;
        }
...
static psa_status_t psa_generate_derived_key_internal(psa_key_slot_t* slot,
                                                      size_t bits,
                                                      psa_key_derivation_operation_t* operation) {
    uint8_t* data = NULL;
    ...
    status = psa_driver_wrapper_import_key(&slot->attr, data, bytes, slot->key.data, slot->key.bytes, &slot->key.bytes,
                                           &bits);
...
psa_status_t psa_key_derivation_output_key(const psa_key_attributes_t* attributes,
                                           psa_key_derivation_operation_t* operation,
                                           mbedtls_svc_key_id_t* key) {
    return psa_key_derivation_output_key_custom(attributes, operation, &default_custom_production, NULL, 0, key);
}
...
static psa_status_t psa_key_derivation_set_maximum_capacity(psa_key_derivation_operation_t* operation,
                                                            psa_algorithm_t kdf_alg) {
...
static psa_status_t psa_tls12_prf_psk_to_ms_set_key(psa_tls12_prf_key_derivation_t* prf,
                                                    ...
                                                    size_t data_length) {
    psa_status_t status;
    const size_t pms_len = (prf->state == PSA_TLS12_PRF_STATE_OTHER_KEY_SET ? 4 + data_length + prf->other_secret_length
                                                                            : 4 + 2 * data_length);
...
static psa_status_t psa_pbkdf2_cmac_set_password(const uint8_t* input,
    ...
    if (input_len != PSA_MAC_LENGTH(PSA_KEY_TYPE_AES, 128U, PSA_ALG_CMAC)) {
        ...
        status =
            psa_driver_wrapper_mac_compute(&attributes, zeros, sizeof(zeros), PSA_ALG_CMAC, input, input_len, output,
                                           PSA_MAC_LENGTH(PSA_KEY_TYPE_AES, 128U, PSA_ALG_CMAC), output_len);
    }
    else {
        memcpy(output, input, input_len);
...
static psa_status_t psa_key_derivation_input_integer_internal(psa_key_derivation_operation_t* operation,
                                                              psa_key_derivation_step_t step,
                                                              uint64_t value) {
    psa_status_t status;
    psa_algorithm_t kdf_alg = psa_key_derivation_get_kdf_alg(operation);
...
psa_status_t psa_key_agreement_raw_builtin(const psa_key_attributes_t* attributes,
    ...
    switch (alg) {
        ...
        case PSA_ALG_ECDH:
            return mbedtls_psa_key_agreement_ecdh(attributes, key_buffer, key_buffer_size, alg, peer_key,
        ...
        case PSA_ALG_FFDH:
            return mbedtls_psa_ffdh_key_agreement(attributes, peer_key, peer_key_length, key_buffer, key_buffer_size,
...
static psa_status_t psa_key_agreement_raw_internal(psa_algorithm_t alg,
    ...
    return psa_driver_wrapper_key_agreement(&private_key->attr, private_key->key.data, private_key->key.bytes, alg,
                                            peer_key, peer_key_length, shared_secret, shared_secret_size,
                                            shared_secret_length);
}
...
static psa_status_t psa_key_agreement_internal(psa_key_derivation_operation_t* operation,
     ...
     * secret. */
    status = psa_key_agreement_raw_internal(ka_alg, private_key, peer_key, peer_key_length, shared_secret,
                                            sizeof(shared_secret), &shared_secret_length);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
psa_status_t psa_key_derivation_key_agreement(psa_key_derivation_operation_t* operation,
    ...
    status =
        psa_get_and_lock_transparent_key_slot_with_policy(private_key, &slot, PSA_KEY_USAGE_DERIVE, operation->alg);
    if (status != PSA_SUCCESS) {
        return status;
    }
...
psa_status_t psa_raw_key_agreement(psa_algorithm_t alg,
    ...
    status = psa_get_and_lock_transparent_key_slot_with_policy(private_key, &slot, PSA_KEY_USAGE_DERIVE, alg);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
static psa_status_t validate_key_agreement_params(const psa_key_attributes_t* attributes, psa_algorithm_t alg) {
    psa_key_type_t key_type;
    ...
    if (key_type != PSA_KEY_TYPE_DERIVE && key_type != PSA_KEY_TYPE_RAW_DATA && key_type != PSA_KEY_TYPE_HMAC &&
...
psa_status_t psa_key_agreement(mbedtls_svc_key_id_t private_key,
    ...
    *key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    status = psa_raw_key_agreement(alg, private_key, peer_key, peer_key_length, shared_secret, sizeof(shared_secret),
                                   &shared_secret_len);
...
psa_status_t psa_key_agreement_iop_setup(psa_key_agreement_iop_t* operation,
    ...
    status = mbedtls_psa_key_agreement_iop_setup(&operation->mbedtls_ctx, &slot->attr, slot->key.data, slot->key.bytes,
...
psa_status_t psa_key_agreement_iop_complete(psa_key_agreement_iop_t* operation, mbedtls_svc_key_id_t* key) {
    ...
    status = mbedtls_psa_key_agreement_iop_complete(&operation->mbedtls_ctx, intermediate_key, sizeof(intermediate_key),
                                                    &key_len);
...
psa_status_t psa_generate_key_internal(const psa_key_attributes_t* attributes,
    ...
    if (key_type_is_raw_bytes(type)) {
        ...
        return mbedtls_psa_rsa_generate_key(attributes, custom_data, custom_data_length, key_buffer, key_buffer_size,
                                            key_buffer_length);
    }
    else
...
psa_status_t psa_generate_key_custom(const psa_key_attributes_t* attributes,
    ...
    *key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    status = psa_driver_wrapper_generate_key(attributes, custom, custom_data, custom_data_length, slot->key.data,
...
#if defined(MBEDTLS_ECP_RESTARTABLE) && defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_GENERATE)
static psa_status_t psa_generate_key_iop_abort_internal(psa_generate_key_iop_t* operation) {
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
psa_status_t psa_generate_key_iop_complete(psa_generate_key_iop_t* operation, mbedtls_svc_key_id_t* key) {
    ...
    status = mbedtls_psa_ecp_generate_key_iop_complete(&operation->ctx, key_data, sizeof(key_data), &key_len);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
    ...
    status = psa_import_key(&operation->attributes, key_data + (sizeof(key_data) - key_len), key_len, key);
...
#if !defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
psa_status_t mbedtls_psa_crypto_configure_entropy_sources(void (*entropy_init)(mbedtls_entropy_context* ctx),
    ...
    switch (subsystem) {
        ...
        case PSA_CRYPTO_SUBSYSTEM_RNG:
            ...
            if (global_data.rng_state == RNG_NOT_INITIALIZED && driver_wrappers_initialized) {
                ...
                if (MBEDTLS_PSA_CRYPTO_RNG_HASH != PSA_ALG_SHA_256 && MBEDTLS_PSA_CRYPTO_RNG_HASH != PSA_ALG_SHA_512) {
...
static psa_status_t psa_pake_set_password_key(psa_pake_operation_t* operation, mbedtls_svc_key_id_t password) {
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    psa_status_t unlock_status = PSA_ERROR_CORRUPTION_DETECTED;
    psa_key_slot_t* slot = NULL;
    psa_key_type_t type;
...
#if defined(PSA_WANT_ALG_JPAKE)
static psa_status_t psa_jpake_prologue(psa_pake_operation_t* operation,
...
psa_status_t psa_pake_get_shared_key(psa_pake_operation_t* operation,
    ...
    status = psa_driver_wrapper_pake_get_shared_key(operation, shared_key, sizeof(shared_key), &shared_key_len);
...
psa_status_t psa_crypto_local_output_free(psa_crypto_local_output_t* local_output) {
    ...
    status = psa_crypto_copy_output(local_output->buffer, local_output->length, local_output->original,
                                    local_output->length);
    if (status != PSA_SUCCESS) {
        return status;
    }
