...
MBEDTLS_STATIC_ASSERT(MBEDTLS_PSA_KEY_ID_BUILTIN_MIN <= MBEDTLS_PSA_KEY_ID_BUILTIN_MAX, "Empty builtin key ID range");
MBEDTLS_STATIC_ASSERT(PSA_KEY_ID_VOLATILE_MIN <= PSA_KEY_ID_VOLATILE_MAX, "Empty volatile key ID range");

MBEDTLS_STATIC_ASSERT(PSA_KEY_ID_USER_MAX < PSA_KEY_ID_VENDOR_MIN || PSA_KEY_ID_VENDOR_MAX < PSA_KEY_ID_USER_MIN,
...
#endif
#define KEY_SLICE_LENGTH_MAX (KEY_SLOT_VOLATILE_SLICE_BASE_LENGTH << (KEY_SLOT_VOLATILE_SLICE_COUNT - 1))
...
static inline psa_key_slot_t* get_volatile_key_slot(psa_key_id_t key_id) {
    MBEDTLS_STATIC_ASSERT(ARRAY_LENGTH(global_data.key_slots) <= PSA_KEY_ID_VOLATILE_MAX - PSA_KEY_ID_VOLATILE_MIN + 1,
...
static psa_status_t psa_get_and_lock_key_slot_in_memory(mbedtls_svc_key_id_t key, psa_key_slot_t** p_slot) {
    ...
    if (psa_key_id_is_volatile(key_id)) {
        ...
        if (slot != NULL && slot->state == PSA_SLOT_FULL && mbedtls_svc_key_id_equal(key, slot->attr.id)) {
            status = PSA_SUCCESS;
        }
        else {
...
psa_status_t psa_initialize_key_slots(void) {
    ...
    global_data.key_slices[KEY_SLOT_CACHE_SLICE_INDEX] =
        mbedtls_calloc(PERSISTENT_KEY_CACHE_COUNT, sizeof(*global_data.key_slices[KEY_SLOT_CACHE_SLICE_INDEX]));
...
static psa_status_t psa_allocate_volatile_key_slot(psa_key_id_t* key_id, psa_key_slot_t** p_slot) {
    ...
    if (global_data.key_slices[slice_idx] == NULL) {
        global_data.key_slices[slice_idx] = mbedtls_calloc(key_slice_length(slice_idx), sizeof(psa_key_slot_t));
        if (global_data.key_slices[slice_idx] == NULL) {
            return PSA_ERROR_INSUFFICIENT_MEMORY;
        }
    }
...
psa_status_t psa_reserve_free_key_slot(psa_key_id_t* volatile_key_id, psa_key_slot_t** p_slot) {
    ...
    for (slot_idx = 0; slot_idx < PERSISTENT_KEY_CACHE_COUNT; slot_idx++) {
        ...
        if ((unused_persistent_key_slot == NULL) && (slot->state == PSA_SLOT_FULL) &&
            (!psa_key_slot_has_readers(slot)) && (!PSA_KEY_LIFETIME_IS_VOLATILE(slot->attr.lifetime))) {
...
static psa_status_t psa_load_builtin_key_into_slot(psa_key_slot_t* slot) {
    ...
    status = psa_driver_wrapper_get_builtin_key(slot_number, &attributes, slot->key.data, slot->key.bytes,
                                                &key_buffer_length);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
psa_status_t psa_unregister_read(psa_key_slot_t* slot) {
    ...
    if ((slot->state == PSA_SLOT_PENDING_DELETION) && (slot->var.occupied.registered_readers == 1)) {
...
psa_status_t psa_purge_key(mbedtls_svc_key_id_t key) {
    ...
    if ((!PSA_KEY_LIFETIME_IS_VOLATILE(slot->attr.lifetime)) && (slot->var.occupied.registered_readers == 1)) {
