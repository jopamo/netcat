...
#define PSA_STORAGE_FLAG_WRITE_ONCE                                                                                   \
    (1 << 0) /**< The data associated with the uid will not be able to be modified or deleted. Intended to be used to \
                set bits in `psa_storage_create_flags_t`*/
...
#define PSA_ITS_API_VERSION_MAJOR                                                                                      \
    1 /**< The major version number of the PSA ITS API. It will be incremented on significant updates that may include \
         breaking changes */
#define PSA_ITS_API_VERSION_MINOR                                                                                    \
    1 /**< The minor version number of the PSA ITS API. It will be incremented in small updates that are unlikely to \
 ...
 * \retval      #PSA_ERROR_NOT_PERMITTED         The operation failed because the provided `uid` value was already
 ...
 * \retval      #PSA_ERROR_INSUFFICIENT_STORAGE  The operation failed because there was insufficient space on the
 * storage medium
 * \retval      #PSA_ERROR_STORAGE_FAILURE       The operation failed because the physical storage has failed (Fatal
 * error)
 ...
 *
 * \retval      #PSA_SUCCESS                 The operation completed successfully
 * \retval      #PSA_ERROR_DOES_NOT_EXIST    The operation failed because the provided `uid` value was not found in the
 * storage
 * \retval      #PSA_ERROR_STORAGE_FAILURE   The operation failed because the physical storage has failed (Fatal error)
 * \retval      #PSA_ERROR_DATA_CORRUPT      The operation failed because stored data has been corrupted
 * \retval      #PSA_ERROR_INVALID_ARGUMENT  The operation failed because one of the provided pointers(`p_data`,
 * `p_data_length`) is invalid. For example is `NULL` or references memory the caller cannot access. In addition, this
 ...
 *                                           is invalid, for example is `NULL` or references memory the caller cannot
 * access
 */
 ...
 *
 * \retval      #PSA_SUCCESS                  The operation completed successfully
 * \retval      #PSA_ERROR_DOES_NOT_EXIST     The operation failed because the provided key value was not found in the
 * storage
 * \retval      #PSA_ERROR_NOT_PERMITTED      The operation failed because the provided key value was created with
