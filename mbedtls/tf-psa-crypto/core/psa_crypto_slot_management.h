...
#define PSA_KEY_ID_VOLATILE_MAX (PSA_KEY_ID_VOLATILE_MIN + MBEDTLS_PSA_KEY_SLOT_COUNT - 1)
 ...
 */
extern size_t (*mbedtls_test_hook_psa_volatile_key_slice_length)(size_t slice_idx);
 ...
 */
static inline psa_status_t psa_key_slot_state_transition(psa_key_slot_t* slot,
...
static inline psa_status_t psa_register_read(psa_key_slot_t* slot) {
    if ((slot->state != PSA_SLOT_FULL) || (slot->var.occupied.registered_readers >= SIZE_MAX)) {
        return PSA_ERROR_CORRUPTION_DETECTED;
    }
...
static inline int psa_key_id_is_user(psa_key_id_t key_id) {
    return (key_id >= PSA_KEY_ID_USER_MIN) && (key_id <= PSA_KEY_ID_USER_MAX);
}
