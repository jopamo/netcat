...
#if !defined(MBEDTLS_PSA_CRYPTO_C) || !defined(MBEDTLS_AES_C) || !defined(MBEDTLS_GCM_C) || \
    !defined(MBEDTLS_CHACHAPOLY_C) || defined(MBEDTLS_PSA_CRYPTO_KEY_ID_ENCODES_OWNER)
int main(void) {
    printf(
 ...
 */
static psa_status_t aead_prepare(const char* info, psa_key_id_t* key, psa_algorithm_t* alg) {
    psa_status_t status;
...
static void aead_info(psa_key_id_t key, psa_algorithm_t alg) {
    ...
    const char* type_str = key_type == PSA_KEY_TYPE_AES ? "AES" : key_type == PSA_KEY_TYPE_CHACHA20 ? "Chacha" : "???";
 ...
 */
static int aead_encrypt(psa_key_id_t key,
                        psa_algorithm_t alg,
    ...
    p += olen;
    PSA_CHECK(psa_aead_finish(&op, p, end - p, &olen, tag, sizeof(tag), &olen_tag));
    p += olen;
...
static psa_status_t aead_demo(const char* info) {
    ...
    PSA_CHECK(aead_encrypt(key, alg, iv1, sizeof(iv1), add_data1, sizeof(add_data1), msg1_part1, sizeof(msg1_part1),
                           msg1_part2, sizeof(msg1_part2)));
    PSA_CHECK(aead_encrypt(key, alg, iv2, sizeof(iv2), add_data2, sizeof(add_data2), msg2_part1, sizeof(msg2_part1),
