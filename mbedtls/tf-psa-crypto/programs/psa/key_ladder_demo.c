...
#if !defined(PSA_WANT_ALG_SHA_256) || !defined(MBEDTLS_MD_C) || !defined(MBEDTLS_AES_C) || !defined(MBEDTLS_CCM_C) || \
    !defined(MBEDTLS_PSA_CRYPTO_C) || !defined(MBEDTLS_FS_IO) || defined(MBEDTLS_PSA_CRYPTO_KEY_ID_ENCODES_OWNER)
int main(void) {
    printf(
...
enum program_mode { MODE_GENERATE, MODE_SAVE, MODE_UNWRAP, MODE_WRAP };
 ...
 */
static psa_status_t derive_key_ladder(const char* ladder[], size_t ladder_depth, psa_key_id_t* key) {
    psa_status_t status = PSA_SUCCESS;
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
    ...
    for (i = 0; i < ladder_depth; i++) {
        ...
        PSA_CHECK(psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_SALT, DERIVE_KEY_SALT,
                                                 DERIVE_KEY_SALT_LENGTH));
        PSA_CHECK(psa_key_derivation_input_key(&operation, PSA_KEY_DERIVATION_INPUT_SECRET, *key));
        PSA_CHECK(psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_INFO, (uint8_t*)ladder[i],
                                                 strlen(ladder[i])));
...
static psa_status_t derive_wrapping_key(psa_key_usage_t usage, psa_key_id_t derived_key, psa_key_id_t* wrapping_key) {
    psa_status_t status = PSA_SUCCESS;
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
    psa_key_derivation_operation_t operation = PSA_KEY_DERIVATION_OPERATION_INIT;
    ...
    *wrapping_key = 0;
     ...
     * the master key. */
    PSA_CHECK(psa_key_derivation_setup(&operation, KDF_ALG));
    PSA_CHECK(psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_SALT, WRAPPING_KEY_SALT,
                                             WRAPPING_KEY_SALT_LENGTH));
    PSA_CHECK(psa_key_derivation_input_key(&operation, PSA_KEY_DERIVATION_INPUT_SECRET, derived_key));
    PSA_CHECK(psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_INFO, NULL, 0));
    ...
    PSA_CHECK(psa_key_derivation_output_key(&attributes, &operation, wrapping_key));
...
static psa_status_t wrap_data(const char* input_file_name, const char* output_file_name, psa_key_id_t wrapping_key) {
    psa_status_t status;
    ...
    buffer_size = PSA_AEAD_ENCRYPT_OUTPUT_SIZE(key_type, WRAPPING_ALG, input_size);
    ...
    PSA_CHECK(psa_aead_encrypt(wrapping_key, WRAPPING_ALG, header.iv, WRAPPING_IV_SIZE, (uint8_t*)&header,
                               sizeof(header), buffer, input_size, buffer, buffer_size, &ciphertext_size));

    /* Write the output. */
    SYS_CHECK((output_file = fopen(output_file_name, "wb")) != NULL);
    /* Ensure no stdio buffering of secrets, as such buffers cannot be wiped. */
    mbedtls_setbuf(output_file, NULL);
    SYS_CHECK(fwrite(&header, 1, sizeof(header), output_file) == sizeof(header));
    SYS_CHECK(fwrite(buffer, 1, ciphertext_size, output_file) == ciphertext_size);
...
static psa_status_t unwrap_data(const char* input_file_name, const char* output_file_name, psa_key_id_t wrapping_key) {
    psa_status_t status;
    FILE* input_file = NULL;
    FILE* output_file = NULL;
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
    ...
    if (memcmp(&header.magic, WRAPPED_DATA_MAGIC, WRAPPED_DATA_MAGIC_LENGTH) != 0) {
    ...
    PSA_CHECK(psa_get_key_attributes(wrapping_key, &attributes));
    key_type = psa_get_key_type(&attributes);
    ciphertext_size = PSA_AEAD_ENCRYPT_OUTPUT_SIZE(key_type, WRAPPING_ALG, header.payload_size);
    /* Check for integer overflow. */
    ...
    /* Unwrap the data. */
    PSA_CHECK(psa_aead_decrypt(wrapping_key, WRAPPING_ALG, header.iv, WRAPPING_IV_SIZE, (uint8_t*)&header,
                               sizeof(header), buffer, ciphertext_size, buffer, ciphertext_size, &plaintext_size));
...
static psa_status_t run(enum program_mode mode,
    ...
    /* Read the master key. */
    PSA_CHECK(
        import_key_from_file(PSA_KEY_USAGE_DERIVE | PSA_KEY_USAGE_EXPORT, KDF_ALG, key_file_name, &derivation_key));
    ...
    switch (mode) {
        case MODE_SAVE:
            ...
            break;
        case MODE_UNWRAP:
            PSA_CHECK(derive_wrapping_key(PSA_KEY_USAGE_DECRYPT, derivation_key, &wrapping_key));
            PSA_CHECK(unwrap_data(input_file_name, output_file_name, wrapping_key));
            break;
        case MODE_WRAP:
            PSA_CHECK(derive_wrapping_key(PSA_KEY_USAGE_ENCRYPT, derivation_key, &wrapping_key));
            PSA_CHECK(wrap_data(input_file_name, output_file_name, wrapping_key));
            break;
        default:
...
int main(int argc, char* argv[]) {
    ...
    if (output_file_name == NULL && (mode == MODE_SAVE || mode == MODE_WRAP || mode == MODE_UNWRAP)) {
        printf("Required argument missing: output\n");
        return DEMO_ERROR;
    }

    status = run(mode, key_file_name, ladder, ladder_depth, input_file_name, output_file_name);
