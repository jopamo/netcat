...
static void append(char** buffer, size_t buffer_size, size_t* required_size, const char* string, size_t length) {
...
static void append_with_curve(char** buffer,
    ...
    if (family_name != NULL) {
        append(buffer, buffer_size, required_size, family_name, strlen(family_name));
    }
    else {
...
static void append_with_group(char** buffer,
    ...
    append(buffer, buffer_size, required_size, "(", 1);
    if (group_name != NULL) {
        append(buffer, buffer_size, required_size, group_name, strlen(group_name));
    }
    else {
...
static int psa_snprint_status(char* buffer, size_t buffer_size, psa_status_t status) {
...
static int psa_snprint_ecc_curve(char* buffer, size_t buffer_size, psa_ecc_family_t curve) {
...
static int psa_snprint_dh_group(char* buffer, size_t buffer_size, psa_dh_family_t group) {
    ...
    for (; *argp != NULL; argp++) {
        ...
        switch (type) {
            case TYPE_ALGORITHM:
                ...
                break;
            case TYPE_ECC_CURVE:
                psa_snprint_ecc_curve(buffer, sizeof(buffer), (psa_ecc_family_t)value);
                break;
            case TYPE_DH_GROUP:
                psa_snprint_dh_group(buffer, sizeof(buffer), (psa_dh_family_t)value);
                break;
            case TYPE_KEY_TYPE:
...
int main(int argc, char* argv[]) {
    ...
    if (!strcmp(argv[1], "error") || !strcmp(argv[1], "status")) {
        ...
        return process_unsigned(TYPE_ALGORITHM, (psa_algorithm_t)(-1), argv + 2);
    }
    else if (!strcmp(argv[1], "curve") || !strcmp(argv[1], "ecc_curve")) {
        return process_unsigned(TYPE_ECC_CURVE, (psa_ecc_family_t)(-1), argv + 2);
    }
        ...
        return process_unsigned(TYPE_KEY_USAGE, (psa_key_usage_t)(-1), argv + 2);
    }
    else {
