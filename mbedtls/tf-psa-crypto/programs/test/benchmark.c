...
#define PRINT_ERROR mbedtls_printf("Error code: %d", ret);
...
#define MEMORY_MEASURE_RESET mbedtls_memory_buffer_alloc_count_get(&prv_alloc, &prv_free);
...
#define MEMORY_MEASURE_PRINT(title_len)                             \
    ...
    mbedtls_printf("%6u heap bytes, %6u allocs", (unsigned)max_bytes, (unsigned)(alloc_cnt - prv_alloc));
...
#if !defined(HAVE_HARDCLOCK) && defined(_WIN32) && !defined(EFIX64) && !defined(EFI32)
...
static unsigned long mbedtls_timing_hardclock(void) {
    ...
    return (tv_cur.tv_sec - tv_init.tv_sec) * 1000000U + (tv_cur.tv_usec - tv_init.tv_usec);
}
...
typedef struct {
    char md5, ripemd160, sha1, sha256, sha512, sha3_224, sha3_256, sha3_384, sha3_512, aes_cbc, aes_cfb128, aes_cfb8,
        aes_ctr, aes_gcm, aes_ccm, aes_xts, chachapoly, aes_cmac, aria, camellia, chacha20, poly1305, ctr_drbg,
        hmac_drbg, rsa, ecdsa, ecdh;
} todo_list;
...
int main(int argc, char* argv[]) {
    ...
    if (todo.aes_cfb128) {
        ...
        for (keysize = 128; keysize <= 256; keysize += 64) {
            ...
            TIME_AND_TSC(title, mbedtls_aes_crypt_cfb128(&aes, MBEDTLS_AES_ENCRYPT, BUFSIZE, &iv_off, tmp, buf, buf));
        }
        mbedtls_aes_free(&aes);
    }
    if (todo.aes_cfb8) {
        ...
        for (keysize = 128; keysize <= 256; keysize += 64) {
            ...
            TIME_AND_TSC(title, mbedtls_aes_crypt_cfb8(&aes, MBEDTLS_AES_ENCRYPT, BUFSIZE, tmp, buf, buf));
        }
        mbedtls_aes_free(&aes);
    }
#endif
    ...
    if (todo.aes_ctr) {
        ...
        for (keysize = 128; keysize <= 256; keysize += 64) {
            ...
            TIME_AND_TSC(title, mbedtls_aes_crypt_ctr(&aes, BUFSIZE, &nc_off, tmp, stream_block, buf, buf));
        }
        mbedtls_aes_free(&aes);
    }
#endif
...
#if defined(MBEDTLS_HMAC_DRBG_C) && (defined(MBEDTLS_SHA1_C) || defined(MBEDTLS_SHA256_C))
    if (todo.hmac_drbg) {
    ...
    if (todo.ecdh) {
        ...
        for (curve_info = curve_list; curve_info->grp_id != MBEDTLS_ECP_DP_NONE; curve_info++) {
            ...
            CHECK_AND_CONTINUE(
                mbedtls_ecdh_make_params(&ecdh_srv, &params_len, buf_srv, sizeof(buf_srv), myrand, NULL));

            mbedtls_snprintf(title, sizeof(title), "ECDHE-%s", curve_info->name);
            TIME_PUBLIC(
                title, "ephemeral handshake", const unsigned char* p_srv = buf_srv; mbedtls_ecdh_init(&ecdh_cli);

                CHECK_AND_CONTINUE(mbedtls_ecdh_read_params(&ecdh_cli, &p_srv, p_srv + params_len)); CHECK_AND_CONTINUE(
                    mbedtls_ecdh_make_public(&ecdh_cli, &publen, buf_cli, sizeof(buf_cli), myrand, NULL));

                CHECK_AND_CONTINUE(
                    mbedtls_ecdh_calc_secret(&ecdh_cli, &seclen, buf_cli, sizeof(buf_cli), myrand, NULL));
        ...
        for (curve_info = curve_list; curve_info->grp_id != MBEDTLS_ECP_DP_NONE; curve_info++) {
            ...
            CHECK_AND_CONTINUE(mbedtls_ecdh_read_params(&ecdh_cli, &p_srv, p_srv + params_len));
            CHECK_AND_CONTINUE(mbedtls_ecdh_make_public(&ecdh_cli, &publen, buf_cli, sizeof(buf_cli), myrand, NULL));

            mbedtls_snprintf(title, sizeof(title), "ECDH-%s", curve_info->name);
            TIME_PUBLIC(title, "static handshake",
                        CHECK_AND_CONTINUE(
                            mbedtls_ecdh_calc_secret(&ecdh_cli, &seclen, buf_cli, sizeof(buf_cli), myrand, NULL)););
