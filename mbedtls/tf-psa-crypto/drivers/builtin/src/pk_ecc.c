...
int mbedtls_pk_ecc_set_group(mbedtls_pk_context* pk, mbedtls_ecp_group_id grp_id) {
    ...
    if ((pk->ec_family != 0 && pk->ec_family != ec_family) || (pk->bits != 0 && pk->bits != ec_bits)) {
 ...
 */
static int pk_ecc_set_pubkey_psa_ecp_fallback(mbedtls_pk_context* pk, const unsigned char* pub, size_t pub_len) {
    ...
    ret = mbedtls_ecp_point_write_binary(&(ecp_key.grp), &ecp_key.Q, MBEDTLS_ECP_PF_UNCOMPRESSED, &pk->pub_raw_len,
                                         pk->pub_raw, sizeof(pk->pub_raw));
