...
/* See the "real" version for documentation */
static int pk_ecc_group_id_from_specified(const mbedtls_asn1_buf* params, mbedtls_ecp_group_id* grp_id) {
    (void)params;
    (void)grp_id;
...
int mbedtls_pk_parse_subpubkey(unsigned char** p, const unsigned char* end, mbedtls_pk_context* pk) {
    ...
    if (pk_alg == MBEDTLS_PK_RSA) {
        ...
        else if ((ret <= MBEDTLS_ERR_ASN1_OUT_OF_DATA) && (ret >= MBEDTLS_ERR_ASN1_BUF_TOO_SMALL)) {
...
static int pk_parse_key_sec1_der(mbedtls_pk_context* pk, const unsigned char* key, size_t keylen) {
    ...
    if (p != end) {
        ...
        if ((ret = mbedtls_asn1_get_tag(&p, end, &len, MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_ASN1_CONSTRUCTED | 0)) ==
            0) {
            if ((ret = pk_get_ecparams(&p, p + len, &params)) != 0 || (ret = pk_use_ecparams(&params, pk)) != 0) {
                return ret;
            }
        }
 ...
 *
 */
MBEDTLS_STATIC_TESTABLE int mbedtls_pk_parse_key_pkcs8_unencrypted_der(mbedtls_pk_context* pk,
    ...
    if (pk_alg == MBEDTLS_PK_RSA) {
        ...
        if (MBEDTLS_PK_IS_RFC8410_GROUP_ID(ec_grp_id)) {
            if ((ret = pk_use_ecparams_rfc8410(&params, ec_grp_id, pk)) != 0 ||
                (ret = pk_parse_key_rfc8410_der(pk, p, len, end)) != 0) {
                mbedtls_pk_free(pk);
                return ret;
            }
        }
...
#endif
        {
            if ((ret = pk_use_ecparams(&params, pk)) != 0 || (ret = pk_parse_key_sec1_der(pk, p, len)) != 0) {
                mbedtls_pk_free(pk);
                return ret;
            }
        }
...
MBEDTLS_STATIC_TESTABLE int mbedtls_pk_parse_key_pkcs8_encrypted_der(mbedtls_pk_context* pk,
    ...
    if (MBEDTLS_OID_CMP(MBEDTLS_OID_PKCS5_PBES2, &pbe_alg_oid) == 0) {
        if ((ret = mbedtls_pkcs5_pbes2_ext(&pbe_params, MBEDTLS_PKCS5_DECRYPT, pwd, pwdlen, p, len, buf, len,
                                           &outlen)) != 0) {
...
int mbedtls_pk_parse_key(mbedtls_pk_context* pk,
    ...
    if (key[keylen - 1] != '\0') {
        ...
        ret = mbedtls_pem_read_buffer(&pem, PEM_BEGIN_PRIVATE_KEY_RSA, PEM_END_PRIVATE_KEY_RSA, key, pwd, pwdlen, &len);
    }
    ...
    if (key[keylen - 1] != '\0') {
        ...
        ret = mbedtls_pem_read_buffer(&pem, PEM_BEGIN_PRIVATE_KEY_EC, PEM_END_PRIVATE_KEY_EC, key, pwd, pwdlen, &len);
    }
    if (ret == 0) {
        pk_info = mbedtls_pk_info_from_type(MBEDTLS_PK_ECKEY);

        if ((ret = mbedtls_pk_setup(pk, pk_info)) != 0 || (ret = pk_parse_key_sec1_der(pk, pem.buf, pem.buflen)) != 0) {
            mbedtls_pk_free(pk);
        }
    ...
    if (key[keylen - 1] != '\0') {
        ...
        ret = mbedtls_pem_read_buffer(&pem, PEM_BEGIN_PRIVATE_KEY_PKCS8, PEM_END_PRIVATE_KEY_PKCS8, key, NULL, 0, &len);
    }
    if (ret == 0) {
        if ((ret = mbedtls_pk_parse_key_pkcs8_unencrypted_der(pk, pem.buf, pem.buflen)) != 0) {
            mbedtls_pk_free(pk);
        }
    ...
    if (key[keylen - 1] != '\0') {
        ...
        ret = mbedtls_pem_read_buffer(&pem, PEM_BEGIN_ENCRYPTED_PRIVATE_KEY_PKCS8, PEM_END_ENCRYPTED_PRIVATE_KEY_PKCS8,
                                      key, NULL, 0, &len);
    }
    if (ret == 0) {
        if ((ret = mbedtls_pk_parse_key_pkcs8_encrypted_der(pk, pem.buf, pem.buflen, pwd, pwdlen)) != 0) {
            mbedtls_pk_free(pk);
        }
    ...
    if (pwdlen != 0) {
        ...
        ret = mbedtls_pk_parse_key_pkcs8_encrypted_der(pk, key_copy, keylen, pwd, pwdlen);
    ...
    pk_info = mbedtls_pk_info_from_type(MBEDTLS_PK_RSA);
    if (mbedtls_pk_setup(pk, pk_info) == 0 && (mbedtls_pk_rsa_set_key(pk, key, keylen) == 0) &&
...
#if defined(PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY)
    pk_info = mbedtls_pk_info_from_type(MBEDTLS_PK_ECKEY);
    if (mbedtls_pk_setup(pk, pk_info) == 0 && pk_parse_key_sec1_der(pk, key, keylen) == 0) {
        return 0;
    }
    mbedtls_pk_free(pk);
#endif /* PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY */
...
int mbedtls_pk_parse_public_key(mbedtls_pk_context* ctx, const unsigned char* key, size_t keylen) {
    ...
    if (key[keylen - 1] != '\0') {
        ...
        ret = mbedtls_pem_read_buffer(&pem, PEM_BEGIN_PUBLIC_KEY_RSA, PEM_END_PUBLIC_KEY_RSA, key, NULL, 0, &len);
    }
    ...
    if (key[keylen - 1] != '\0') {
        ...
        ret = mbedtls_pem_read_buffer(&pem, PEM_BEGIN_PUBLIC_KEY, PEM_END_PUBLIC_KEY, key, NULL, 0, &len);
    }
