 ...
 */
static int convert_raw_to_der_single_int(const unsigned char* raw_buf,
                                         size_t raw_len,
...
int mbedtls_ecdsa_raw_to_der(size_t bits,
    ...
    MBEDTLS_ASN1_CHK_ADD(len, mbedtls_asn1_write_tag(&p, der, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE));
 ...
 */
static int convert_der_to_raw_single_int(unsigned char* der,
                                         size_t der_len,
                                         unsigned char* raw,
...
int mbedtls_ecdsa_der_to_raw(size_t bits,
    ...
    ret = mbedtls_asn1_get_tag(&p, der + der_len, &data_len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
    if (ret != 0) {
        return ret;
    }
