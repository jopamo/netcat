 ...
 * platform and needs.
 */
#if !defined(MBEDTLS_PLATFORM_HAS_EXPLICIT_BZERO) && !(defined(__STDC_LIB_EXT1__) && !defined(__IAR_SYSTEMS_ICC__)) && \
    !defined(_WIN32)
...
mbedtls_ms_time_t mbedtls_ms_time(void) {
    ...
    current_ms = ((mbedtls_ms_time_t)ct.dwLowDateTime + ((mbedtls_ms_time_t)(ct.dwHighDateTime) << 32LL)) / 10000;
    return current_ms;
}
#else
...
int mbedtls_platform_get_entropy(psa_driver_get_entropy_flags_t flags,
    ...
    if (!BCRYPT_SUCCESS(BCryptGenRandom(NULL, output, (unsigned long)output_size, BCRYPT_USE_SYSTEM_PREFERRED_RNG))) {
        return MBEDTLS_ERR_ENTROPY_SOURCE_FAILED;
    }
