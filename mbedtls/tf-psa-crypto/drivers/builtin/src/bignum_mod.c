    ...
    switch (N->int_rep) {
        case MBEDTLS_MPI_MOD_REP_MONTGOMERY:
            if (N->rep.mont.rr != NULL) {
                mbedtls_zeroize_and_free((mbedtls_mpi_uint*)N->rep.mont.rr, N->limbs * sizeof(mbedtls_mpi_uint));
                N->rep.mont.rr = NULL;
            }
            N->rep.mont.mm = 0;
            break;
...
static int set_mont_const_square(const mbedtls_mpi_uint** X, const mbedtls_mpi_uint* A, size_t limbs) {
...
int mbedtls_mpi_mod_modulus_setup(mbedtls_mpi_mod_modulus* N, const mbedtls_mpi_uint* p, size_t p_limbs) {
    int ret = 0;
...
static int mbedtls_mpi_mod_inv_non_mont(mbedtls_mpi_mod_residue* X,
    ...
    mbedtls_mpi_core_to_mont_rep(X->p, A->p, Nmont.p, Nmont.limbs, Nmont.rep.mont.mm, Nmont.rep.mont.rr,
                                 working_memory);

    mbedtls_mpi_mod_raw_inv_prime(X->p, X->p, Nmont.p, Nmont.limbs, Nmont.rep.mont.rr, working_memory);

    /* And convert back from Montgomery form */

    mbedtls_mpi_core_from_mont_rep(X->p, X->p, Nmont.p, Nmont.limbs, Nmont.rep.mont.mm, working_memory);
