 ...
 *          this function.
 */
static int is_alg_compatible_with_key(psa_algorithm_t check_alg, psa_key_type_t key_type, psa_algorithm_t key_alg) {
...
static int is_psa_key_compatible_with_alg_usage(mbedtls_svc_key_id_t key_id,
     ...
     */
    if ((usage == PSA_KEY_USAGE_DERIVE_PUBLIC) && (alg == PSA_ALG_ECDH) && PSA_KEY_TYPE_IS_ECC(key_type)) {
        ret = 1;
        goto exit;
    }
...
int mbedtls_pk_can_do_psa(const mbedtls_pk_context* pk, psa_algorithm_t alg, psa_key_usage_t usage) {
    ...
    int want_private = ((usage & (PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_DECRYPT | PSA_KEY_USAGE_DERIVE)) != 0);
    if ((!has_public && !has_private) || (want_private && !has_private)) {
        return 0;
    }
    ...
    if (mbedtls_pk_get_type(pk) == MBEDTLS_PK_OPAQUE) {
        ...
        switch (pk_type) {
            ...
            case MBEDTLS_PK_RSA:
            case MBEDTLS_PK_RSASSA_PSS:
                if (PSA_ALG_IS_RSA_OAEP(alg) || PSA_ALG_IS_RSA_PSS(alg) || PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg) ||
...
#if defined(PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY)
            case MBEDTLS_PK_ECKEY:
                if (PSA_ALG_IS_ECDH(alg) || (PSA_ALG_IS_ECDSA(alg) && pk->ec_family != PSA_ECC_FAMILY_MONTGOMERY)) {
                    return 1;
                }
                break;
...
int mbedtls_pk_get_psa_attributes(const mbedtls_pk_context* pk,
    ...
    int want_private = !(usage == PSA_KEY_USAGE_VERIFY_MESSAGE || usage == PSA_KEY_USAGE_VERIFY_HASH ||
    ...
    switch (pk_type) {
        ...
        case MBEDTLS_PK_RSA: {
            ...
            psa_set_key_type(attributes, (want_private ? PSA_KEY_TYPE_RSA_KEY_PAIR : PSA_KEY_TYPE_RSA_PUBLIC_KEY));
        ...
        case MBEDTLS_PK_ECDSA: {
            ...
            psa_set_key_type(attributes,
                             (want_private ? PSA_KEY_TYPE_ECC_KEY_PAIR(family) : PSA_KEY_TYPE_ECC_PUBLIC_KEY(family)));
        ...
        case MBEDTLS_PK_OPAQUE: {
            ...
            switch (usage) {
                ...
                case PSA_KEY_USAGE_VERIFY_MESSAGE:
                case PSA_KEY_USAGE_VERIFY_HASH:
                    if (!(PSA_KEY_TYPE_IS_ECC_KEY_PAIR(old_type) || old_type == PSA_KEY_TYPE_RSA_KEY_PAIR)) {
                        return MBEDTLS_ERR_PK_TYPE_MISMATCH;
                    }
                    break;
                case PSA_KEY_USAGE_DECRYPT:
 ...
 */
static int pk_restart_setup(mbedtls_pk_restart_ctx* ctx, const mbedtls_pk_info_t* info, mbedtls_pk_rs_op_t rs_op) {
...
int mbedtls_pk_verify_restartable(mbedtls_pk_context* ctx,
    ...
    if (rs_ctx != NULL && is_restartable_enabled && ctx->pk_info->verify_rs_func != NULL) {
        ...
        ret = ctx->pk_info->verify_rs_func(ctx, md_alg, hash, hash_len, sig, sig_len, rs_ctx->rs_ctx);
...
int mbedtls_pk_sign_restartable(mbedtls_pk_context* ctx,
    ...
    int is_restartable_enabled = psa_interruptible_get_max_ops() != 0;
    /* optimization: use non-restartable version if restart disabled */
    if (rs_ctx != NULL && is_restartable_enabled && ctx->pk_info->sign_rs_func != NULL) {
        int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        ...
        ret = ctx->pk_info->sign_rs_func(ctx, md_alg, hash, hash_len, sig, sig_size, sig_len, rs_ctx->rs_ctx);
    ...
    return ctx->pk_info->sign_func(ctx, md_alg, hash, hash_len, sig, sig_size, sig_len);
}
...
int mbedtls_pk_sign(mbedtls_pk_context* ctx,
                    ...
                    size_t sig_size,
                    size_t* sig_len) {
    return mbedtls_pk_sign_restartable(ctx, md_alg, hash, hash_len, sig, sig_size, sig_len, NULL);
}
...
int mbedtls_pk_sign_ext(mbedtls_pk_sigalg_t pk_type,
    ...
    if (mbedtls_pk_get_type(ctx) == MBEDTLS_PK_OPAQUE) {
        ...
        status = psa_sign_hash(ctx->priv_id, PSA_ALG_RSA_PSS(psa_md_alg), hash, hash_len, sig, sig_size, sig_len);
        if (status == PSA_ERROR_NOT_PERMITTED) {
            status = psa_sign_hash(ctx->priv_id, PSA_ALG_RSA_PSS_ANY_SALT(psa_md_alg), hash, hash_len, sig, sig_size,
                                   sig_len);
        }
        return PSA_PK_RSA_TO_MBEDTLS_ERR(status);
    }

    return mbedtls_pk_psa_rsa_sign_ext(PSA_ALG_RSA_PSS(psa_md_alg), ctx, hash, hash_len, sig, sig_size, sig_len);
#else
    return MBEDTLS_ERR_PK_FEATURE_UNAVAILABLE;
#endif /* PSA_WANT_KEY_TYPE_RSA_PUBLIC_KEY */
}
...
int mbedtls_pk_check_pair(const mbedtls_pk_context* pub, const mbedtls_pk_context* prv) {
    ...
    return prv->pk_info->check_pair_func((mbedtls_pk_context*)pub, (mbedtls_pk_context*)prv);
}
