...
#include "rsa_internal.h"
    ...
    defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PKCS1V15_SIGN) || defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PSS) ||   \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR_IMPORT) ||                                        \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR_EXPORT) || defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
...
psa_status_t mbedtls_psa_rsa_import_key(const psa_key_attributes_t* attributes,
    ...
    *bits = (psa_key_bits_t)PSA_BYTES_TO_BITS(mbedtls_rsa_get_len(rsa));
    ...
    status = mbedtls_psa_rsa_export_key(attributes->type, rsa, key_buffer, key_buffer_size, key_buffer_length);
exit:
...
psa_status_t mbedtls_psa_rsa_export_public_key(const psa_key_attributes_t* attributes,
    ...
    status = mbedtls_psa_rsa_load_representation(attributes->type, key_buffer, key_buffer_size, &rsa);
    if (status == PSA_SUCCESS) {
        status = mbedtls_psa_rsa_export_key(PSA_KEY_TYPE_RSA_PUBLIC_KEY, rsa, data, data_size, data_length);
    }
    mbedtls_rsa_free(rsa);
    mbedtls_free(rsa);
...
psa_status_t mbedtls_psa_rsa_generate_key(const psa_key_attributes_t* attributes,
    ...
    ret = mbedtls_rsa_gen_key(&rsa, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE, (unsigned int)attributes->bits,
                              exponent);
    if (ret != 0) {
        mbedtls_rsa_free(&rsa);
        return mbedtls_to_psa_error(ret);
...
static psa_status_t psa_rsa_decode_md_type(psa_algorithm_t alg, size_t hash_length, mbedtls_md_type_t* md_alg) {
...
psa_status_t mbedtls_psa_rsa_sign_hash(const psa_key_attributes_t* attributes,
    ...
    if (PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg)) {
        ...
        if (ret == 0) {
            ret = mbedtls_rsa_pkcs1_sign(rsa, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE, md_alg,
        ...
        if (ret == 0) {
            ret =
                mbedtls_rsa_rsassa_pss_sign_ext(rsa, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE, MBEDTLS_MD_NONE,
                                                (unsigned int)hash_length, hash, MBEDTLS_RSA_SALT_LEN_ANY, signature);
        }
    }
    else
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PSS)
static int rsa_pss_expected_salt_len(psa_algorithm_t alg, const mbedtls_rsa_context* rsa, size_t hash_length) {
...
psa_status_t mbedtls_psa_rsa_verify_hash(const psa_key_attributes_t* attributes,
    ...
    if (PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg)) {
        ...
        if (ret == 0) {
            ...
            ret = mbedtls_rsa_rsassa_pss_verify_ext(rsa, md_alg, (unsigned)hash_length, hash, md_alg, slen, signature);
        }
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_RSA_PSS */
    ...
    status = (ret == MBEDTLS_ERR_RSA_INVALID_PADDING) ? PSA_ERROR_INVALID_SIGNATURE : mbedtls_to_psa_error(ret);
        ...
        * defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_OAEP) */
        if (alg == PSA_ALG_RSA_PKCS1V15_CRYPT) {
            ...
            status = mbedtls_to_psa_error(mbedtls_rsa_pkcs1_encrypt(
                rsa, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE, input_length, input, output));
#else
            status = PSA_ERROR_NOT_SUPPORTED;
            ...
            status = mbedtls_to_psa_error(mbedtls_rsa_rsaes_oaep_encrypt(
                rsa, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE, salt, salt_length, input_length, input, output));
#else
            status = PSA_ERROR_NOT_SUPPORTED;
        ...
        * defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_OAEP) */
        ...
        if (alg == PSA_ALG_RSA_PKCS1V15_CRYPT) {
            ...
            status = mbedtls_to_psa_error(mbedtls_rsa_pkcs1_decrypt(
                rsa, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE, output_length, input, output, output_size));
#else
            status = PSA_ERROR_NOT_SUPPORTED;
#endif /* MBEDTLS_PSA_BUILTIN_ALG_RSA_PKCS1V15_CRYPT */
        }
            ...
            status = mbedtls_to_psa_error(mbedtls_rsa_rsaes_oaep_decrypt(rsa, mbedtls_psa_get_random,
                                                                         MBEDTLS_PSA_RANDOM_STATE, salt, salt_length,
