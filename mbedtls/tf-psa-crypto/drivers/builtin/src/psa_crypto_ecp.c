...
exit:
    ...
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_EXPORT) || defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY)
...
psa_status_t mbedtls_psa_ecp_import_key(const psa_key_attributes_t* attributes,
    ...
    /* Parse input */
    status = mbedtls_psa_ecp_load_representation(attributes->type, attributes->bits, data, data_length, &ecp);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
     ...
     * copy operation. */
    status = mbedtls_psa_ecp_export_key(attributes->type, ecp, key_buffer, key_buffer_size, key_buffer_length);
exit:
...
psa_status_t mbedtls_psa_ecp_export_key(psa_key_type_t type,
    ...
    if (PSA_KEY_TYPE_IS_PUBLIC_KEY(type)) {
        ...
        status = mbedtls_to_psa_error(mbedtls_ecp_point_write_binary(&ecp->grp, &ecp->Q, MBEDTLS_ECP_PF_UNCOMPRESSED,
...
psa_status_t mbedtls_psa_ecp_export_public_key(const psa_key_attributes_t* attributes,
    ...
    status = mbedtls_psa_ecp_load_representation(attributes->type, attributes->bits, key_buffer, key_buffer_size, &ecp);
    if (status != PSA_SUCCESS) {
        return status;
    }

    status = mbedtls_psa_ecp_export_key(PSA_KEY_TYPE_ECC_PUBLIC_KEY(PSA_KEY_TYPE_ECC_GET_FAMILY(attributes->type)), ecp,
...
psa_status_t mbedtls_psa_ecp_generate_key(const psa_key_attributes_t* attributes,
    ...
    ret = mbedtls_ecp_gen_privkey(&ecp.grp, &ecp.d, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE);
    if (ret != 0) {
        goto exit;
    }
...
psa_status_t mbedtls_psa_ecdsa_sign_hash(const psa_key_attributes_t* attributes,
    ...
    if (PSA_ALG_ECDSA_IS_DETERMINISTIC(alg)) {
        ...
        (void)alg;
        MBEDTLS_MPI_CHK(mbedtls_ecdsa_sign(&ecp->grp, &r, &s, &ecp->d, hash, hash_length, mbedtls_psa_get_random,
...
psa_status_t mbedtls_psa_ecp_load_public_part(mbedtls_ecp_keypair* ecp) {
    ...
    if (mbedtls_ecp_is_zero(&ecp->Q)) {
        ret =
            mbedtls_ecp_mul(&ecp->grp, &ecp->Q, &ecp->d, &ecp->grp.G, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE);
    }
...
psa_status_t mbedtls_psa_ecdsa_verify_hash(const psa_key_attributes_t* attributes,
    ...
    status = mbedtls_to_psa_error(mbedtls_mpi_read_binary(&s, signature + curve_bytes, curve_bytes));
    if (status != PSA_SUCCESS) {
        goto cleanup;
    }
    ...
    status = mbedtls_to_psa_error(mbedtls_ecdsa_verify(&ecp->grp, hash, hash_length, &ecp->Q, &r, &s));
cleanup:
    mbedtls_mpi_free(&r);
    mbedtls_mpi_free(&s);
    mbedtls_ecp_keypair_free(ecp);
...
psa_status_t mbedtls_psa_key_agreement_ecdh(const psa_key_attributes_t* attributes,
    ...
    status = mbedtls_psa_ecp_load_representation(PSA_KEY_TYPE_ECC_PUBLIC_KEY(curve), bits, peer_key, peer_key_length,
                                                 &their_key);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
    ...
    status =
        mbedtls_to_psa_error(mbedtls_ecdh_calc_secret(&ecdh, shared_secret_length, shared_secret, shared_secret_size,
                                                      mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE));
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
uint32_t mbedtls_psa_generate_key_iop_get_num_ops(mbedtls_psa_generate_key_iop_t* operation) {
    return operation->num_ops;
}

psa_status_t mbedtls_psa_ecp_generate_key_iop_setup(mbedtls_psa_generate_key_iop_t* operation,
...
psa_status_t mbedtls_psa_ecp_generate_key_iop_complete(mbedtls_psa_generate_key_iop_t* operation,
                                                       uint8_t* key_output,
    ...
    *key_len = PSA_BITS_TO_BYTES(operation->ecp.grp.nbits);
    ...
    status = mbedtls_ecp_gen_privkey(&operation->ecp.grp, &operation->ecp.d, mbedtls_psa_get_random,
                                     MBEDTLS_PSA_RANDOM_STATE);
...
psa_status_t mbedtls_psa_ecp_generate_key_iop_abort(mbedtls_psa_generate_key_iop_t* operation) {
...
uint32_t mbedtls_psa_ecp_export_public_key_iop_get_num_ops(mbedtls_psa_export_public_key_iop_t* operation) {
    return operation->num_ops;
}

psa_status_t mbedtls_psa_ecp_export_public_key_iop_setup(mbedtls_psa_export_public_key_iop_t* operation,
                                                         uint8_t* key,
                                                         size_t key_len,
                                                         const psa_key_attributes_t* key_attributes) {
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;

    status = mbedtls_psa_ecp_load_representation(psa_get_key_type(key_attributes), psa_get_key_bits(key_attributes),
                                                 key, key_len, &operation->key);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
psa_status_t mbedtls_psa_ecp_export_public_key_iop_complete(mbedtls_psa_export_public_key_iop_t* operation,
                                                            uint8_t* pub_key,
                                                            size_t pub_key_size,
                                                            size_t* pub_key_len) {
    int ret = 0;
    ...
    if (mbedtls_ecp_is_zero(&operation->key->Q)) {
        ...
        ret = mbedtls_ecp_mul_restartable(&operation->key->grp, &operation->key->Q, &operation->key->d,
                                          &operation->key->grp.G, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE,
                                          &operation->restart_ctx);
        operation->num_ops += operation->restart_ctx.ops_done;
    }

    if (ret == 0) {
        ret = mbedtls_ecp_write_public_key(operation->key, MBEDTLS_ECP_PF_UNCOMPRESSED, pub_key_len, pub_key,
                                           pub_key_size);
    }
...
psa_status_t mbedtls_psa_ecp_export_public_key_iop_abort(mbedtls_psa_export_public_key_iop_t* operation) {
...
uint32_t mbedtls_psa_key_agreement_iop_get_num_ops(mbedtls_psa_key_agreement_interruptible_operation_t* operation) {
    return operation->num_ops;
}

psa_status_t mbedtls_psa_key_agreement_iop_setup(mbedtls_psa_key_agreement_interruptible_operation_t* operation,
    ...
    status = mbedtls_psa_ecp_load_representation(psa_get_key_type(private_key_attributes),
                                                 psa_get_key_bits(private_key_attributes), private_key_buffer,
                                                 private_key_buffer_len, &operation->our_key);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = mbedtls_psa_ecp_load_representation(PSA_KEY_TYPE_PUBLIC_KEY_OF_KEY_PAIR(private_key_type),
                                                 psa_get_key_bits(private_key_attributes), peer_key, peer_key_length,
                                                 &operation->their_key);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
psa_status_t mbedtls_psa_key_agreement_iop_complete(mbedtls_psa_key_agreement_interruptible_operation_t* operation,
                                                    uint8_t* shared_secret,
                                                    size_t shared_secret_size,
                                                    size_t* shared_secret_length) {
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    status = mbedtls_to_psa_error(mbedtls_ecp_mul_restartable(&operation->our_key->grp, &secret, &operation->our_key->d,
                                                              &operation->their_key->Q, mbedtls_psa_get_random,
                                                              MBEDTLS_PSA_RANDOM_STATE, &operation->rs));
    ...
    *shared_secret_length = PSA_BITS_TO_BYTES(operation->our_key->grp.pbits);
    ...
    status = mbedtls_to_psa_error(mbedtls_ecp_get_type(&operation->our_key->grp) == MBEDTLS_ECP_TYPE_MONTGOMERY
...
psa_status_t mbedtls_psa_key_agreement_iop_abort(mbedtls_psa_key_agreement_interruptible_operation_t* operation) {
