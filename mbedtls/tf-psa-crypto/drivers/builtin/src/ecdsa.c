...
int mbedtls_ecdsa_sign(mbedtls_ecp_group* grp,
    ...
    return mbedtls_ecdsa_sign_restartable(grp, r, s, d, buf, blen, f_rng, p_rng, f_rng, p_rng, NULL);
}
...
sign:
#endif
    ret = mbedtls_ecdsa_sign_restartable(grp, r, s, d, buf, blen, mbedtls_hmac_drbg_random, p_rng, f_rng_blind,
                                         p_rng_blind, rs_ctx);
...
int mbedtls_ecdsa_sign_det_ext(mbedtls_ecp_group* grp,
                               ...
                               int (*f_rng_blind)(void*, unsigned char*, size_t),
                               void* p_rng_blind) {
    return mbedtls_ecdsa_sign_det_restartable(grp, r, s, d, buf, blen, md_alg, f_rng_blind, p_rng_blind, NULL);
}
#endif /* MBEDTLS_ECDSA_DETERMINISTIC */
...
#if defined(MBEDTLS_ECDSA_DETERMINISTIC)
    MBEDTLS_MPI_CHK(
        mbedtls_ecdsa_sign_det_restartable(&ctx->grp, &r, &s, &ctx->d, hash, hlen, md_alg, f_rng, p_rng, rs_ctx));
#else
    (void)md_alg;

    /* Use the same RNG for both blinding and ephemeral key generation */
    MBEDTLS_MPI_CHK(
        mbedtls_ecdsa_sign_restartable(&ctx->grp, &r, &s, &ctx->d, hash, hlen, f_rng, p_rng, f_rng, p_rng, rs_ctx));
#endif /* MBEDTLS_ECDSA_DETERMINISTIC */
...
int mbedtls_ecdsa_read_signature_restartable(mbedtls_ecdsa_context* ctx,
    ...
    if ((ret = mbedtls_ecdsa_verify_restartable(&ctx->grp, hash, hlen, &ctx->Q, &r, &s, rs_ctx)) != 0) {
        goto cleanup;
    }
