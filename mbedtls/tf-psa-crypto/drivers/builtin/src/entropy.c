...
void mbedtls_entropy_init(mbedtls_entropy_context* ctx) {
    ...
    mbedtls_entropy_add_source(ctx, mbedtls_entropy_poll_platform, NULL, MBEDTLS_ENTROPY_POLL_PLATFORM_MIN,
...
static int entropy_update(mbedtls_entropy_context* ctx,
    ...
    if (use_len > MBEDTLS_ENTROPY_BLOCK_SIZE) {
        if ((ret = mbedtls_md(mbedtls_md_info_from_type(MBEDTLS_ENTROPY_MD), data, len, tmp)) != 0) {
            goto cleanup;
        }
        p = tmp;
...
int mbedtls_entropy_func(void* data, unsigned char* output, size_t len) {
     ...
     */
    if ((ret = mbedtls_md(mbedtls_md_info_from_type(MBEDTLS_ENTROPY_MD), buf, MBEDTLS_ENTROPY_BLOCK_SIZE, buf)) != 0) {
        goto exit;
    }
...
static int mbedtls_entropy_source_self_test_gather(unsigned char* buf, size_t buf_len) {
    ...
    while (attempts > 0 && entropy_len < buf_len) {
        if ((ret = mbedtls_entropy_poll_platform(NULL, buf + entropy_len, buf_len - entropy_len, &olen)) != 0) {
            return ret;
        }
