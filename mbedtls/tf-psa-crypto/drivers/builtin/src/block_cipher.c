...
int mbedtls_block_cipher_setup(mbedtls_block_cipher_context_t* ctx, mbedtls_cipher_id_t cipher_id) {
    ...
    if (psa_key_type != PSA_KEY_TYPE_NONE && psa_is_ready_for_cipher()) {
...
int mbedtls_block_cipher_encrypt(mbedtls_block_cipher_context_t* ctx,
    ...
    if (ctx->engine == MBEDTLS_BLOCK_CIPHER_ENGINE_PSA) {
        ...
        status = psa_cipher_encrypt(ctx->psa_key_id, PSA_ALG_ECB_NO_PADDING, input, 16, output, 16, &olen);
        if (status != PSA_SUCCESS) {
            return mbedtls_cipher_error_from_psa(status);
        }
        return 0;
...
#if defined(MBEDTLS_CAMELLIA_C)
        case MBEDTLS_BLOCK_CIPHER_ID_CAMELLIA:
            return mbedtls_camellia_crypt_ecb(&ctx->ctx.camellia, MBEDTLS_CAMELLIA_ENCRYPT, input, output);
#endif
        default:
            return MBEDTLS_ERR_CIPHER_INVALID_CONTEXT;
    }
