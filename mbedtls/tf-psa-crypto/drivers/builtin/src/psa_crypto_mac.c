...
static psa_status_t psa_hmac_finish_internal(mbedtls_psa_hmac_operation_t* hmac, uint8_t* mac, size_t mac_size) {
...
static psa_status_t cmac_setup(mbedtls_psa_mac_operation_t* operation,
    ...
    const mbedtls_cipher_info_t* cipher_info =
        mbedtls_cipher_info_from_psa(PSA_ALG_CMAC, psa_get_key_type(attributes), psa_get_key_bits(attributes), NULL);
    ...
    ret = mbedtls_cipher_cmac_starts(&operation->ctx.cmac, key_buffer, psa_get_key_bits(attributes));
exit:
...
static psa_status_t psa_mac_setup(mbedtls_psa_mac_operation_t* operation,
    ...
    if (PSA_ALG_FULL_LENGTH_MAC(alg) == PSA_ALG_CMAC) {
        ...
        status = psa_hmac_setup_internal(&operation->ctx.hmac, key_buffer, key_buffer_size, PSA_ALG_HMAC_GET_HASH(alg));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_HMAC */
    {
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_CMAC)
    if (PSA_ALG_FULL_LENGTH_MAC(operation->alg) == PSA_ALG_CMAC) {
        return mbedtls_to_psa_error(mbedtls_cipher_cmac_update(&operation->ctx.cmac, input, input_length));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_CMAC */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_HMAC)
...
static psa_status_t psa_mac_finish_internal(mbedtls_psa_mac_operation_t* operation, uint8_t* mac, size_t mac_size) {
#if defined(MBEDTLS_PSA_BUILTIN_ALG_CMAC)
    if (PSA_ALG_FULL_LENGTH_MAC(operation->alg) == PSA_ALG_CMAC) {
