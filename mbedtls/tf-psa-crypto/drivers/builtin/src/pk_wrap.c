...
static int rsa_verify_wrap(mbedtls_pk_context* pk,
...
static int rsa_sign_wrap(mbedtls_pk_context* pk,
                         mbedtls_md_type_t md_alg,
                         const unsigned char* hash,
                         size_t hash_len,
                         unsigned char* sig,
    ...
    return mbedtls_pk_psa_rsa_sign_ext(PSA_ALG_RSA_PKCS1V15_SIGN(psa_md_alg), pk, hash, hash_len, sig, sig_size,
                                       sig_len);
}
...
static int rsa_check_pair_wrap(mbedtls_pk_context* pub, mbedtls_pk_context* prv) {
    ...
    status = psa_export_public_key(prv->priv_id, exp_pub_key, sizeof(exp_pub_key), &exp_pub_key_len);
    if (status != PSA_SUCCESS) {
        return MBEDTLS_ERR_PK_BAD_INPUT_DATA;
    }
...
static int ecdsa_verify_psa(unsigned char* key,
                            size_t key_len,
    ...
    p = (unsigned char*)sig;
    ret = mbedtls_ecdsa_der_to_raw(curve_bits, p, sig_len, extracted_sig, sizeof(extracted_sig), &converted_sig_len);
    if (ret != 0) {
        goto cleanup;
    }
...
static int ecdsa_verify_wrap(mbedtls_pk_context* pk,
    ...
    return ecdsa_verify_psa(pk->pub_raw, pk->pub_raw_len, curve, curve_bits, hash, hash_len, sig, sig_len);
}
 ...
 */
static int ecdsa_sign_psa(mbedtls_svc_key_id_t key_id,
                          mbedtls_md_type_t md_alg,
                          const unsigned char* hash,
                          size_t hash_len,
                          unsigned char* sig,
    ...
    status = psa_sign_hash(key_id, PSA_ALG_DETERMINISTIC_ECDSA(mbedtls_md_psa_alg_from_type(md_alg)), hash, hash_len,
    ...
    status = psa_sign_hash(key_id, PSA_ALG_ECDSA(mbedtls_md_psa_alg_from_type(md_alg)), hash, hash_len, sig, sig_size,
                           sig_len);
    if (status != PSA_SUCCESS) {
        return PSA_PK_ECDSA_TO_MBEDTLS_ERR(status);
    }
...
#if defined(PSA_HAVE_ALG_ECDSA_VERIFY)
static int eckey_verify_rs_wrap(mbedtls_pk_context* pk,
                                mbedtls_md_type_t md_alg,
                                const unsigned char* hash,
                                size_t hash_len,
                                const unsigned char* sig,
    ...
    ret = mbedtls_ecdsa_der_to_raw(pk->bits, sig, sig_len, raw_sig, sizeof(raw_sig), &raw_sig_len);
    if (ret != 0) {
        return ret;
    }
    ...
    if (psa_verify_hash_get_num_ops(op) == 0) {
        ...
        status = psa_verify_hash_start(op, rs_ctx->pub_id, alg, hash, hash_len, raw_sig, raw_sig_len);
        if (status != PSA_SUCCESS) {
            psa_destroy_key(rs_ctx->pub_id);
            return PSA_PK_TO_MBEDTLS_ERR(status);
        }
...
#if defined(PSA_HAVE_ALG_ECDSA_SIGN)
static int eckey_sign_rs_wrap(mbedtls_pk_context* pk,
                              mbedtls_md_type_t md_alg,
                              const unsigned char* hash,
                              size_t hash_len,
                              unsigned char* sig,
    ...
    if (psa_sign_hash_get_num_ops(op) == 0) {
        psa_algorithm_t alg = MBEDTLS_PK_ALG_ECDSA(mbedtls_md_psa_alg_from_type(md_alg));
