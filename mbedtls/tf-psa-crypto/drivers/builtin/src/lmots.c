 ...
 */
static unsigned short lmots_checksum_calculate(const mbedtls_lmots_parameters_t* params, const unsigned char* digest) {
    size_t idx;
    unsigned sum = 0;
...
static int hash_digit_array(const mbedtls_lmots_parameters_t* params,
    ...
    for (i_digit_idx = 0; i_digit_idx < MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT(params->type); i_digit_idx++) {
...
static int public_key_from_hashed_digit_array(const mbedtls_lmots_parameters_t* params,
    ...
    status = psa_hash_finish(&op, pub_key, MBEDTLS_LMOTS_N_HASH_LEN(params->type), &output_hash_len);
    if (status != PSA_SUCCESS) {
    exit:
        psa_hash_abort(&op);
    }
...
int mbedtls_lmots_import_public_key(mbedtls_lmots_public_t* ctx, const unsigned char* key, size_t key_len) {
    ...
    memcpy(ctx->params.q_leaf_identifier, key + PUBLIC_KEY_Q_LEAF_ID_OFFSET, MBEDTLS_LMOTS_Q_LEAF_ID_LEN);
...
int mbedtls_lmots_export_public_key(const mbedtls_lmots_public_t* ctx,
    ...
    memcpy(key + PUBLIC_KEY_I_KEY_ID_OFFSET, ctx->params.I_key_identifier, MBEDTLS_LMOTS_I_KEY_ID_LEN);

    memcpy(key + PUBLIC_KEY_Q_LEAF_ID_OFFSET, ctx->params.q_leaf_identifier, MBEDTLS_LMOTS_Q_LEAF_ID_LEN);
...
int mbedtls_lmots_calculate_public_key_candidate(const mbedtls_lmots_parameters_t* params,
    ...
    if (sig_size != MBEDTLS_LMOTS_SIG_LEN(params->type) || out_size < MBEDTLS_LMOTS_N_HASH_LEN(params->type)) {
        return MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }

    ret = create_digit_array_with_checksum(params, msg, msg_size, sig + MBEDTLS_LMOTS_SIG_C_RANDOM_OFFSET,
                                           tmp_digit_array);
    if (ret) {
        return ret;
    }

    ret = hash_digit_array(params, sig + MBEDTLS_LMOTS_SIG_SIGNATURE_OFFSET(params->type), tmp_digit_array, NULL,
...
int mbedtls_lmots_verify(const mbedtls_lmots_public_t* ctx,
    ...
    ret = mbedtls_lmots_calculate_public_key_candidate(&ctx->params, msg, msg_size, sig, sig_size,
                                                       ...
                                                       MBEDTLS_LMOTS_N_HASH_LEN(ctx->params.type), NULL);
    if (ret) {
        return MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }
...
int mbedtls_lmots_generate_private_key(mbedtls_lmots_private_t* ctx,
    ...
    for (i_digit_idx = 0; i_digit_idx < MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT(ctx->params.type); i_digit_idx++) {
        status = psa_hash_setup(&op, PSA_ALG_SHA_256);
        if (status != PSA_SUCCESS) {
            goto exit;
        }

        status = psa_hash_update(&op, ctx->params.I_key_identifier, sizeof(ctx->params.I_key_identifier));
        if (status != PSA_SUCCESS) {
            goto exit;
        }
        ...
        status = psa_hash_finish(&op, ctx->private_key[i_digit_idx], MBEDTLS_LMOTS_N_HASH_LEN(ctx->params.type),
                                 &output_hash_len);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
...
int mbedtls_lmots_calculate_public_key(mbedtls_lmots_public_t* ctx, const mbedtls_lmots_private_t* priv_ctx) {
    unsigned char y_hashed_digits[MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT_MAX][MBEDTLS_LMOTS_N_HASH_LEN_MAX];
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    ret = public_key_from_hashed_digit_array(&priv_ctx->params, (unsigned char*)y_hashed_digits, ctx->public_key);
    if (ret) {
        goto exit;
    }
...
int mbedtls_lmots_sign(mbedtls_lmots_private_t* ctx,
    ...
    ret = psa_generate_random(tmp_c_random, MBEDTLS_LMOTS_N_HASH_LEN(ctx->params.type));
    if (ret) {
        return ret;
    }

    ret = create_digit_array_with_checksum(&ctx->params, msg, msg_size, tmp_c_random, tmp_digit_array);
    if (ret) {
        goto exit;
    }
    ...
    memcpy(sig + MBEDTLS_LMOTS_SIG_C_RANDOM_OFFSET, tmp_c_random, MBEDTLS_LMOTS_C_RANDOM_VALUE_LEN(ctx->params.type));

    memcpy(sig + MBEDTLS_LMOTS_SIG_SIGNATURE_OFFSET(ctx->params.type), tmp_sig,
           MBEDTLS_LMOTS_P_SIG_DIGIT_COUNT(ctx->params.type) * MBEDTLS_LMOTS_N_HASH_LEN(ctx->params.type));
