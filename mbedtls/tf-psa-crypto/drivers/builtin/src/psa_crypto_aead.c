...
static psa_status_t psa_aead_setup(mbedtls_psa_aead_operation_t* operation,
    ...
    status = mbedtls_cipher_values_from_psa(alg, attributes->type, &mode, &cipher_id);
    ...
    switch (PSA_ALG_AEAD_WITH_SHORTENED_TAG(alg, 0)) {
        ...
        case PSA_ALG_AEAD_WITH_SHORTENED_TAG(PSA_ALG_CCM, 0):
            ...
            status = mbedtls_to_psa_error(
                mbedtls_ccm_setkey(&operation->ctx.ccm, cipher_id, key_buffer, (unsigned int)attributes->bits));
            if (status != PSA_SUCCESS) {
                return status;
            }
            break;
        ...
        case PSA_ALG_AEAD_WITH_SHORTENED_TAG(PSA_ALG_GCM, 0):
            ...
            status = mbedtls_to_psa_error(
                mbedtls_gcm_setkey(&operation->ctx.gcm, cipher_id, key_buffer, (unsigned int)attributes->bits));
            if (status != PSA_SUCCESS) {
                return status;
            }
            break;
...
psa_status_t mbedtls_psa_aead_encrypt(const psa_key_attributes_t* attributes,
    ...
    if (operation.alg == PSA_ALG_CCM) {
        status = mbedtls_to_psa_error(mbedtls_ccm_encrypt_and_tag(&operation.ctx.ccm, plaintext_length, nonce,
        ...
        status = mbedtls_to_psa_error(mbedtls_chachapoly_encrypt_and_tag(&operation.ctx.chachapoly, plaintext_length,
                                                                         nonce, additional_data, additional_data_length,
...
psa_status_t mbedtls_psa_aead_decrypt(const psa_key_attributes_t* attributes,
    ...
    if (operation.alg == PSA_ALG_CCM) {
        status = mbedtls_to_psa_error(mbedtls_ccm_auth_decrypt(
            &operation.ctx.ccm, ciphertext_length - operation.tag_length, nonce, nonce_length, additional_data,
        ...
        status = mbedtls_to_psa_error(mbedtls_gcm_auth_decrypt(
            &operation.ctx.gcm, ciphertext_length - operation.tag_length, nonce, nonce_length, additional_data,
        ...
        status = mbedtls_to_psa_error(
            mbedtls_chachapoly_auth_decrypt(&operation.ctx.chachapoly, ciphertext_length - operation.tag_length, nonce,
                                            additional_data, additional_data_length, tag, ciphertext, plaintext));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_CHACHA20_POLY1305 */
    {
...
psa_status_t mbedtls_psa_aead_set_nonce(mbedtls_psa_aead_operation_t* operation,
    ...
    if (operation->alg == PSA_ALG_GCM) {
        status = mbedtls_to_psa_error(
            mbedtls_gcm_starts(&operation->ctx.gcm, operation->is_encrypt ? MBEDTLS_GCM_ENCRYPT : MBEDTLS_GCM_DECRYPT,
                               nonce, nonce_length));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_GCM */
        ...
        if (operation->alg == PSA_ALG_CCM) {
        status = mbedtls_to_psa_error(
            mbedtls_ccm_starts(&operation->ctx.ccm, operation->is_encrypt ? MBEDTLS_CCM_ENCRYPT : MBEDTLS_CCM_DECRYPT,
                               nonce, nonce_length));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_CCM */
...
psa_status_t mbedtls_psa_aead_set_lengths(mbedtls_psa_aead_operation_t* operation,
    ...
    if (operation->alg == PSA_ALG_CCM) {
        return mbedtls_to_psa_error(
            mbedtls_ccm_set_lengths(&operation->ctx.ccm, ad_length, plaintext_length, operation->tag_length));
    }
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_GCM)
    if (operation->alg == PSA_ALG_GCM) {
        status = mbedtls_to_psa_error(mbedtls_gcm_update_ad(&operation->ctx.gcm, input, input_length));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_GCM */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_CCM)
        if (operation->alg == PSA_ALG_CCM) {
        status = mbedtls_to_psa_error(mbedtls_ccm_update_ad(&operation->ctx.ccm, input, input_length));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_CCM */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_CHACHA20_POLY1305)
        if (operation->alg == PSA_ALG_CHACHA20_POLY1305) {
        status = mbedtls_to_psa_error(mbedtls_chachapoly_update_aad(&operation->ctx.chachapoly, input, input_length));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_CHACHA20_POLY1305 */
    {
...
psa_status_t mbedtls_psa_aead_update(mbedtls_psa_aead_operation_t* operation,
    ...
    if (operation->alg == PSA_ALG_GCM) {
        status = mbedtls_to_psa_error(
            mbedtls_gcm_update(&operation->ctx.gcm, input, input_length, output, output_size, &update_output_length));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_GCM */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_CCM)
        ...
        status = mbedtls_to_psa_error(
            mbedtls_ccm_update(&operation->ctx.ccm, input, input_length, output, output_size, &update_output_length));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_CCM */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_CHACHA20_POLY1305)
        ...
        status =
            mbedtls_to_psa_error(mbedtls_chachapoly_update(&operation->ctx.chachapoly, input_length, input, output));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_CHACHA20_POLY1305 */
    {
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_GCM)
    if (operation->alg == PSA_ALG_GCM) {
        status = mbedtls_to_psa_error(mbedtls_gcm_finish(&operation->ctx.gcm, ciphertext, ciphertext_size,
        ...
        status = mbedtls_to_psa_error(mbedtls_ccm_finish(&operation->ctx.ccm, tag, operation->tag_length));
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_CCM */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_CHACHA20_POLY1305)
