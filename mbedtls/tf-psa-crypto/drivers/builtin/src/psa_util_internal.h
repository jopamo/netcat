...
#define MBEDTLS_PSA_MAX_FFDH_PUBKEY_LENGTH PSA_KEY_EXPORT_FFDH_PUBLIC_KEY_MAX_SIZE(PSA_VENDOR_FFDH_MAX_KEY_BITS)
...
#define PSA_TO_MBEDTLS_ERR_LIST(status, error_list, fallback_f) \
    psa_status_to_mbedtls(status, error_list, sizeof(error_list) / sizeof(error_list[0]), fallback_f)
