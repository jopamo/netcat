...
static psa_status_t psa_pake_ecjpake_setup(mbedtls_psa_pake_operation_t* operation) {
    ...
    ret = mbedtls_ecjpake_setup(&operation->ctx.jpake, operation->role, MBEDTLS_MD_SHA256, MBEDTLS_ECP_DP_SECP256R1,
...
psa_status_t mbedtls_psa_pake_setup(mbedtls_psa_pake_operation_t* operation,
    ...
    status = psa_crypto_driver_pake_get_password(inputs, operation->password, password_len, &actual_password_len);
    if (status != PSA_SUCCESS) {
        goto error;
    }
    ...
    if (PSA_ALG_IS_JPAKE(cipher_suite.algorithm)) {
        if (cipher_suite.type != PSA_PAKE_PRIMITIVE_TYPE_ECC || cipher_suite.family != PSA_ECC_FAMILY_SECP_R1 ||
            cipher_suite.bits != 256 || PSA_ALG_GET_HASH(cipher_suite.algorithm) != PSA_ALG_SHA_256) {
...
static psa_status_t mbedtls_psa_pake_output_internal(mbedtls_psa_pake_operation_t* operation,
    ...
    if (PSA_ALG_IS_JPAKE(operation->alg)) {
        ...
        if (step == PSA_JPAKE_X1_STEP_KEY_SHARE) {
            ret = mbedtls_ecjpake_write_round_one(&operation->ctx.jpake, operation->buffer, sizeof(operation->buffer),
                                                  &operation->buffer_length, mbedtls_psa_get_random,
            ...
            ret = mbedtls_ecjpake_write_round_two(&operation->ctx.jpake, operation->buffer, sizeof(operation->buffer),
                                                  &operation->buffer_length, mbedtls_psa_get_random,
                                                  MBEDTLS_PSA_RANDOM_STATE);
            if (ret != 0) {
                return mbedtls_ecjpake_to_psa_error(ret);
...
psa_status_t mbedtls_psa_pake_output(mbedtls_psa_pake_operation_t* operation,
                                     ...
                                     size_t output_size,
                                     size_t* output_length) {
    psa_status_t status = mbedtls_psa_pake_output_internal(operation, step, output, output_size, output_length);
...
static psa_status_t mbedtls_psa_pake_input_internal(mbedtls_psa_pake_operation_t* operation,
    ...
    if (PSA_ALG_IS_JPAKE(operation->alg)) {
        ...
        if (step == PSA_JPAKE_X4S_STEP_KEY_SHARE && operation->role == MBEDTLS_ECJPAKE_CLIENT) {
            ...
            memcpy(operation->buffer + operation->buffer_length, ecparameters, sizeof(ecparameters));
        ...
        if (step == PSA_JPAKE_X2_STEP_ZK_PROOF) {
            ret = mbedtls_ecjpake_read_round_one(&operation->ctx.jpake, operation->buffer, operation->buffer_length);
            ...
            ret = mbedtls_ecjpake_read_round_two(&operation->ctx.jpake, operation->buffer, operation->buffer_length);
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_JPAKE)
    if (PSA_ALG_IS_JPAKE(operation->alg)) {
        ret = mbedtls_ecjpake_write_shared_key(&operation->ctx.jpake, output, output_size, output_length,
