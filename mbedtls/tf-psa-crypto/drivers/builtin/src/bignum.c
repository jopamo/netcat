 ...
 * values.)
 */
static inline signed short mbedtls_ct_mpi_sign_if(mbedtls_ct_condition_t cond, signed short sign1, signed short sign2) {
...
int mbedtls_mpi_lt_mpi_ct(const mbedtls_mpi* X, const mbedtls_mpi* Y, unsigned* ret) {
     ...
     */
    result = mbedtls_ct_bool_or(result, mbedtls_ct_bool_and(mbedtls_ct_bool_not(different_sign), lt));
 ...
 */
#if defined(_MSC_VER) && defined(MBEDTLS_PLATFORM_IS_WINDOWS_ON_ARM64) && (_MSC_FULL_VER < 193131103)
/*
 ...
 */
static int mbedtls_mpi_exp_mod_optionally_safe(mbedtls_mpi* X,
                                               const mbedtls_mpi* A,
                                               const mbedtls_mpi* E,
                                               int E_public,
                                               const mbedtls_mpi* N,
...
int mbedtls_mpi_exp_mod_unsafe(mbedtls_mpi* X,
                               const mbedtls_mpi* A,
                               const mbedtls_mpi* E,
                               const mbedtls_mpi* N,
                               mbedtls_mpi* prec_RR) {
...
int mbedtls_mpi_gcd_modinv_odd(mbedtls_mpi* G, mbedtls_mpi* I, const mbedtls_mpi* A, const mbedtls_mpi* N) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_mpi local_g;
    ...
    if (mbedtls_mpi_cmp_int(A, 0) < 0 || mbedtls_mpi_cmp_mpi(A, N) > 0 || mbedtls_mpi_get_bit(N, 0) != 1 ||
...
static const unsigned char small_prime_gaps[] = {
    ...
    14, 4,  6, 2,  10, 2,  6, 6,  4,  6, 6,  2,  10, 2,  4, 2,  12, 12, 4, 2, 4,  6, 2,  10, 6,  6,  6, 2,
    6,  4,  2, 10, 14, 4,  2, 4,  14, 6, 10, 2,  4,  6,  8, 6,  6,  4,  6, 8, 4,  8, 10, 2,  10, 2,  6, 4,
    6,  8,  4, 2,  4,  12, 8, 4,  8,  4, 6,  12, 2,  18, 6, 10, 6,  6,  2, 6, 10, 6, 6,  2,  6,  6,  4, 2,
    12, 10, 2, 4,  6,  6,  2, 12, 4,  6, 8,  10, 8,  10, 8, 6,  6,  4,  8, 6, 4,  8, 4,  14, 10, 12, 2, 10,
    2,  4,  2, 10, 14, 4,  2, 4,  14, 4, 2,  4,  20, 4,  8, 10, 8,  4,  6, 6, 14, 4, 6,  6,  8,  6, /*reaches 997*/
