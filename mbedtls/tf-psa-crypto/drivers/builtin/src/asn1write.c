...
#if defined(MBEDTLS_ASN1_WRITE_C) || defined(MBEDTLS_ASN1_PARSE_C) || defined(PSA_HAVE_ALG_SOME_ECDSA)
...
int mbedtls_asn1_write_algorithm_identifier_ext(unsigned char** p,
    ...
    return mbedtls_asn1_write_len_and_tag(p, start, len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
}
...
#else
#define asn1_find_named_data(list, oid, len) ((mbedtls_asn1_named_data*)mbedtls_asn1_find_named_data(list, oid, len))
#endif
...
int mbedtls_asn1_write_integer(unsigned char** p,
    ...
    while ((number_of_leading_zeros < integer_length) && (integer[number_of_leading_zeros] == 0x0)) {
