...
const mbedtls_cipher_info_t* mbedtls_cipher_info_from_values(const mbedtls_cipher_id_t cipher_id,
    ...
    for (def = mbedtls_cipher_definitions; def->info != NULL; def++) {
        if (mbedtls_cipher_get_base(def->info)->cipher == cipher_id &&
            mbedtls_cipher_info_get_key_bitlen(def->info) == (unsigned)key_bitlen && def->info->mode == mode) {
            return def->info;
        }
    }
...
int mbedtls_cipher_setkey(mbedtls_cipher_context_t* ctx,
    ...
    if (MBEDTLS_MODE_ECB == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode) && MBEDTLS_DECRYPT == operation) {
     ...
     */
    if (MBEDTLS_ENCRYPT == operation || MBEDTLS_MODE_CFB == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode) ||
        MBEDTLS_MODE_OFB == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode) ||
        MBEDTLS_MODE_CTR == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode)) {
        return mbedtls_cipher_get_base(ctx->cipher_info)->setkey_enc_func(ctx->cipher_ctx, key, ctx->key_bitlen);
    }

    if (MBEDTLS_DECRYPT == operation) {
        return mbedtls_cipher_get_base(ctx->cipher_info)->setkey_dec_func(ctx->cipher_ctx, key, ctx->key_bitlen);
    }
#else
    if (operation == MBEDTLS_ENCRYPT || operation == MBEDTLS_DECRYPT) {
        return mbedtls_cipher_get_base(ctx->cipher_info)->setkey_enc_func(ctx->cipher_ctx, key, ctx->key_bitlen);
...
int mbedtls_cipher_set_iv(mbedtls_cipher_context_t* ctx, const unsigned char* iv, size_t iv_len) {
...
#if defined(MBEDTLS_CHACHAPOLY_C)
    if (((mbedtls_cipher_type_t)ctx->cipher_info->type) == MBEDTLS_CIPHER_CHACHA20_POLY1305 && iv_len != 12) {
        return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
    }
#endif
#endif
    ...
    if (MBEDTLS_MODE_GCM == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode)) {
        return mbedtls_gcm_starts((mbedtls_gcm_context*)ctx->cipher_ctx, ctx->operation, iv, iv_len);
    }
#endif
    ...
    if (MBEDTLS_MODE_CCM_STAR_NO_TAG == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode)) {
        ...
        set_lengths_result = mbedtls_ccm_set_lengths((mbedtls_ccm_context*)ctx->cipher_ctx, 0, 0, 0);
    ...
    *olen = 0;
    ...
    if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_ECB) {
        ...
        *olen = ilen;
        ...
        if (0 !=
            (ret =
                 mbedtls_cipher_get_base(ctx->cipher_info)->ecb_func(ctx->cipher_ctx, ctx->operation, input, output))) {
            return ret;
        }
    ...
    if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_GCM) {
        return mbedtls_gcm_update((mbedtls_gcm_context*)ctx->cipher_ctx, input, ilen, output, ilen, olen);
    }
#endif

#if defined(MBEDTLS_CCM_C)
    if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_CCM_STAR_NO_TAG) {
        return mbedtls_ccm_update((mbedtls_ccm_context*)ctx->cipher_ctx, input, ilen, output, ilen, olen);
    }
#endif
    ...
    if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_CBC) {
        ...
        if (0 != ctx->unprocessed_len) {
            ...
            if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)
                                ->cbc_func(ctx->cipher_ctx, ctx->operation, block_size, ctx->iv, ctx->unprocessed_data,
                                           output))) {
                return ret;
            }
        ...
        if (ilen) {
            if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)
                                ->cbc_func(ctx->cipher_ctx, ctx->operation, ilen, ctx->iv, input, output))) {
                return ret;
            }
    ...
    if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_CFB) {
        if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)
                            ->cfb_func(ctx->cipher_ctx, ctx->operation, ilen, &ctx->unprocessed_len, ctx->iv, input,
                                       output))) {
            return ret;
        }
    ...
    if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_OFB) {
        if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)
                            ->ofb_func(ctx->cipher_ctx, ilen, &ctx->unprocessed_len, ctx->iv, input, output))) {
            return ret;
        }
    ...
    if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_CTR) {
        if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)
                            ->ctr_func(ctx->cipher_ctx, ilen, &ctx->unprocessed_len, ctx->iv, ctx->unprocessed_data,
                                       input, output))) {
            return ret;
        }
    ...
    if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_STREAM) {
        if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)->stream_func(ctx->cipher_ctx, ilen, input, output))) {
            return ret;
        }
 ...
 */
static int get_no_padding(unsigned char* input, size_t input_len, size_t* data_len, size_t* invalid_padding) {
    if (NULL == input || NULL == data_len) {
        return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
    }
...
int mbedtls_cipher_finish_padded(mbedtls_cipher_context_t* ctx,
    ...
    *invalid_padding = 0;
    ...
    if (MBEDTLS_MODE_CBC == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode)) {
        ...
        /* cipher block */
        if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)
                            ->cbc_func(ctx->cipher_ctx, ctx->operation, mbedtls_cipher_get_block_size(ctx), ctx->iv,
        ...
        if (MBEDTLS_DECRYPT == ctx->operation) {
            return ctx->get_padding(output, mbedtls_cipher_get_block_size(ctx), olen, invalid_padding);
        }
...
int mbedtls_cipher_set_padding_mode(mbedtls_cipher_context_t* ctx, mbedtls_cipher_padding_t mode) {
    if (NULL == ctx->cipher_info || MBEDTLS_MODE_CBC != ((mbedtls_cipher_mode_t)ctx->cipher_info->mode)) {
        return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
    }
...
int mbedtls_cipher_write_tag(mbedtls_cipher_context_t* ctx, unsigned char* tag, size_t tag_len) {
    ...
    if (MBEDTLS_MODE_GCM == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode)) {
        ...
        return mbedtls_gcm_finish((mbedtls_gcm_context*)ctx->cipher_ctx, NULL, 0, &output_length, tag, tag_len);
    }
#endif
...
int mbedtls_cipher_check_tag(mbedtls_cipher_context_t* ctx, const unsigned char* tag, size_t tag_len) {
    ...
    if (MBEDTLS_MODE_GCM == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode)) {
        ...
        if (0 != (ret = mbedtls_gcm_finish((mbedtls_gcm_context*)ctx->cipher_ctx, NULL, 0, &output_length, check_tag,
                                           tag_len))) {
            return ret;
        }
...
int mbedtls_cipher_crypt(mbedtls_cipher_context_t* ctx,
    ...
    size_t invalid_padding = 0;
    if ((ret = mbedtls_cipher_finish_padded(ctx, output + *olen, &finish_olen, &invalid_padding)) != 0) {
        return ret;
    }
    *olen += finish_olen;
...
static int mbedtls_cipher_aead_encrypt(mbedtls_cipher_context_t* ctx,
    ...
    if (MBEDTLS_MODE_GCM == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode)) {
        *olen = ilen;
        return mbedtls_gcm_crypt_and_tag(ctx->cipher_ctx, MBEDTLS_GCM_ENCRYPT, ilen, iv, iv_len, ad, ad_len, input,
                                         output, tag_len, tag);
    }
#endif /* MBEDTLS_GCM_C */
#if defined(MBEDTLS_CCM_C)
...
int mbedtls_cipher_auth_encrypt_ext(mbedtls_cipher_context_t* ctx,
    ...
    int ret =
        mbedtls_cipher_aead_encrypt(ctx, iv, iv_len, ad, ad_len, input, ilen, output, olen, output + ilen, tag_len);
    *olen += tag_len;
    return ret;
}
...
int mbedtls_cipher_auth_decrypt_ext(mbedtls_cipher_context_t* ctx,
    ...
    return mbedtls_cipher_aead_decrypt(ctx, iv, iv_len, ad, ad_len, input, ilen - tag_len, output, olen,
