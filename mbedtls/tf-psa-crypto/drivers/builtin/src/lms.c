...
int mbedtls_lms_import_public_key(mbedtls_lms_public_t* ctx, const unsigned char* key, size_t key_size) {
    ...
    memcpy(ctx->T_1_pub_key, key + PUBLIC_KEY_ROOT_NODE_OFFSET, MBEDTLS_LMS_M_NODE_BYTES(ctx->params.type));
...
int mbedtls_lms_export_public_key(const mbedtls_lms_public_t* ctx,
    ...
    memcpy(key + PUBLIC_KEY_ROOT_NODE_OFFSET, ctx->T_1_pub_key, MBEDTLS_LMS_M_NODE_BYTES(ctx->params.type));
...
int mbedtls_lms_verify(const mbedtls_lms_public_t* ctx,
    ...
    if (MBEDTLS_GET_UINT32_BE(sig, SIG_OTS_SIG_OFFSET + MBEDTLS_LMOTS_SIG_TYPE_OFFSET) != MBEDTLS_LMOTS_SHA256_N32_W8) {
        return MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }
    ...
    if (MBEDTLS_GET_UINT32_BE(sig, SIG_TYPE_OFFSET(ctx->params.otstype)) != MBEDTLS_LMS_SHA256_M32_H10) {
        return MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }
    ...
    ret = mbedtls_lmots_calculate_public_key_candidate(
        &ots_params, msg, msg_size, sig + SIG_OTS_SIG_OFFSET, MBEDTLS_LMOTS_SIG_LEN(ctx->params.otstype),
    ...
    for (height = 0; height < MBEDTLS_LMS_H_TREE_HEIGHT(ctx->params.type); height++) {
        ...
        if (curr_node_id & 1) {
            left_node =
                sig + SIG_PATH_OFFSET(ctx->params.otstype) + height * MBEDTLS_LMS_M_NODE_BYTES(ctx->params.type);
        ...
        ret = create_merkle_internal_value(&ctx->params, left_node, right_node, parent_node_id, Tc_candidate_root_node);
        if (ret != 0) {
            return MBEDTLS_ERR_LMS_VERIFY_FAILED;
        }
        curr_node_id /= 2;
    ...
    if (memcmp(Tc_candidate_root_node, ctx->T_1_pub_key, MBEDTLS_LMS_M_NODE_BYTES(ctx->params.type))) {
        return MBEDTLS_ERR_LMS_VERIFY_FAILED;
    }
...
static int calculate_merkle_tree(const mbedtls_lms_private_t* ctx, unsigned char* tree) {
    ...
    for (priv_key_idx = 0; priv_key_idx < MERKLE_TREE_INTERNAL_NODE_AM(ctx->params.type); priv_key_idx++) {
        r_node_idx = MERKLE_TREE_INTERNAL_NODE_AM(ctx->params.type) + priv_key_idx;

        ret = create_merkle_leaf_value(&ctx->params, ctx->ots_public_keys[priv_key_idx].public_key, r_node_idx,
    ...
    for (r_node_idx = MERKLE_TREE_INTERNAL_NODE_AM(ctx->params.type) - 1; r_node_idx > 0; r_node_idx--) {
 ...
 */
static int get_merkle_path(mbedtls_lms_private_t* ctx, unsigned int leaf_node_id, unsigned char* path) {
...
int mbedtls_lms_generate_private_key(mbedtls_lms_private_t* ctx,
    ...
    ret = psa_generate_random(ctx->params.I_key_identifier, MBEDTLS_LMOTS_I_KEY_ID_LEN);
    if (ret != 0) {
        goto exit;
    }
    ...
    ctx->ots_private_keys =
        mbedtls_calloc((size_t)MERKLE_TREE_LEAF_NODE_AM(ctx->params.type), sizeof(*ctx->ots_private_keys));
    if (ctx->ots_private_keys == NULL) {
     ...
     * platforms (particularly Windows) */
    ctx->ots_public_keys =
        mbedtls_calloc((size_t)MERKLE_TREE_LEAF_NODE_AM(ctx->params.type), sizeof(*ctx->ots_public_keys));
    if (ctx->ots_public_keys == NULL) {
        ret = MBEDTLS_ERR_LMS_ALLOC_FAILED;
        goto exit;
    }
    ...
    for (idx = 0; idx < MERKLE_TREE_LEAF_NODE_AM(ctx->params.type); idx++) {
        ret = mbedtls_lmots_generate_private_key(&ctx->ots_private_keys[idx], otstype, ctx->params.I_key_identifier,
                                                 idx, seed, seed_size);
        if (ret != 0) {
            goto exit;
        }

        ret = mbedtls_lmots_calculate_public_key(&ctx->ots_public_keys[idx], &ctx->ots_private_keys[idx]);
        if (ret != 0) {
            goto exit;
        }
    }
...
int mbedtls_lms_sign(mbedtls_lms_private_t* ctx,
    ...
    ret = mbedtls_lmots_sign(&ctx->ots_private_keys[q_leaf_identifier], msg, msg_size, sig + SIG_OTS_SIG_OFFSET,
