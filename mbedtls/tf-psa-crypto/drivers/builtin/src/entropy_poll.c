...
#error \
    "MBEDTLS_PSA_DRIVER_GET_ENTROPY and MBEDTLS_PSA_BUILTIN_GET_ENTROPY " \
...
int mbedtls_entropy_poll_platform(void* data, unsigned char* output, size_t len, size_t* olen) {
    ...
    *olen = len;
    ...
    ret = mbedtls_platform_get_entropy(PSA_DRIVER_GET_ENTROPY_FLAGS_NONE, &estimate_bits, output, len);
    if (ret != 0) {
        return ret;
    }
