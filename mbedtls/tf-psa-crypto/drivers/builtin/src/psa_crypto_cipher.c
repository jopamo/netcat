...
#if defined(MBEDTLS_PSA_BUILTIN_CIPHER) || defined(MBEDTLS_PSA_BUILTIN_AEAD) ||                \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_AES) || defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ARIA) || \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_CAMELLIA) || defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_CHACHA20)
...
static psa_status_t mbedtls_cipher_validate_values(psa_algorithm_t alg, psa_key_type_t key_type) {
    ...
    if (alg == PSA_ALG_STREAM_CIPHER || alg == PSA_ALG_AEAD_WITH_SHORTENED_TAG(PSA_ALG_CHACHA20_POLY1305, 0)) {
    ...
    if (alg == PSA_ALG_AEAD_WITH_SHORTENED_TAG(PSA_ALG_CCM, 0) ||
        ...
        if (key_type == PSA_KEY_TYPE_AES || key_type == PSA_KEY_TYPE_ARIA || key_type == PSA_KEY_TYPE_CAMELLIA) {
            return PSA_SUCCESS;
        }
    }

    if (alg == PSA_ALG_CTR || alg == PSA_ALG_CFB || alg == PSA_ALG_OFB || alg == PSA_ALG_XTS ||
        alg == PSA_ALG_ECB_NO_PADDING || alg == PSA_ALG_CBC_NO_PADDING || alg == PSA_ALG_CBC_PKCS7 ||
        alg == PSA_ALG_CMAC) {
        if (key_type == PSA_KEY_TYPE_AES || key_type == PSA_KEY_TYPE_ARIA || key_type == PSA_KEY_TYPE_CAMELLIA) {
            return PSA_SUCCESS;
        }
...
psa_status_t mbedtls_cipher_values_from_psa(psa_algorithm_t alg,
...
psa_status_t mbedtls_psa_cipher_encrypt_setup(mbedtls_psa_cipher_operation_t* operation,
                                              ...
                                              size_t key_buffer_size,
                                              psa_algorithm_t alg) {
    return psa_cipher_setup(operation, attributes, key_buffer, key_buffer_size, alg, MBEDTLS_ENCRYPT);
}
...
psa_status_t mbedtls_psa_cipher_decrypt_setup(mbedtls_psa_cipher_operation_t* operation,
                                              ...
                                              size_t key_buffer_size,
                                              psa_algorithm_t alg) {
    return psa_cipher_setup(operation, attributes, key_buffer, key_buffer_size, alg, MBEDTLS_DECRYPT);
}
...
static psa_status_t psa_cipher_update_ecb(mbedtls_cipher_context_t* ctx,
    ...
    *output_length = 0;
    ...
    if (ctx->unprocessed_len > 0) {
        ...
        if (ctx->unprocessed_len == block_size) {
            status = mbedtls_to_psa_error(
                mbedtls_cipher_update(ctx, ctx->unprocessed_data, block_size, output, &internal_output_length));
    ...
    while (input_length >= block_size) {
        ...
        status = mbedtls_to_psa_error(mbedtls_cipher_update(ctx, input, block_size, output, &internal_output_length));
...
psa_status_t mbedtls_psa_cipher_update(mbedtls_psa_cipher_operation_t* operation,
    ...
    if (!PSA_ALG_IS_STREAM_CIPHER(operation->alg)) {
        ...
        expected_output_size =
            (operation->ctx.cipher.unprocessed_len + input_length) / operation->block_length * operation->block_length;
    }
    else {
    ...
    if (operation->alg == PSA_ALG_ECB_NO_PADDING) {
        ...
        status = psa_cipher_update_ecb(&operation->ctx.cipher, input, input_length, output, output_length);
    }
    else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_ECB_NO_PADDING */
        if (input_length == 0) {
...
psa_status_t mbedtls_psa_cipher_finish(mbedtls_psa_cipher_operation_t* operation,
    ...
    status = mbedtls_to_psa_error(
        mbedtls_cipher_finish_padded(&operation->ctx.cipher, temp_output_buffer, output_length, &invalid_padding));
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
psa_status_t mbedtls_psa_cipher_encrypt(const psa_key_attributes_t* attributes,
    ...
    status = mbedtls_psa_cipher_encrypt_setup(&operation, attributes, key_buffer, key_buffer_size, alg);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
    ...
    status = mbedtls_psa_cipher_update(&operation, input, input_length, output, output_size, &update_output_length);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = mbedtls_psa_cipher_finish(&operation, mbedtls_buffer_offset(output, update_output_length),
...
psa_status_t mbedtls_psa_cipher_decrypt(const psa_key_attributes_t* attributes,
    ...
    status = mbedtls_psa_cipher_decrypt_setup(&operation, attributes, key_buffer, key_buffer_size, alg);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
    ...
    status = mbedtls_psa_cipher_update(&operation, mbedtls_buffer_offset_const(input, operation.iv_length),
    ...
    status = mbedtls_psa_cipher_finish(&operation, mbedtls_buffer_offset(output, accumulated_length),
