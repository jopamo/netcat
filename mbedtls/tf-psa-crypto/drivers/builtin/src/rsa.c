...
int mbedtls_rsa_write_key(const mbedtls_rsa_context* rsa, unsigned char* start, unsigned char** p) {
    ...
    /* Export QP */
    if ((ret = mbedtls_rsa_export_crt(rsa, NULL, NULL, &T)) != 0 || (ret = mbedtls_asn1_write_mpi(p, start, &T)) < 0) {
        goto end_of_export;
    }
    len += ret;

    /* Export DQ */
    if ((ret = mbedtls_rsa_export_crt(rsa, NULL, &T, NULL)) != 0 || (ret = mbedtls_asn1_write_mpi(p, start, &T)) < 0) {
        goto end_of_export;
    }
    len += ret;

    /* Export DP */
    if ((ret = mbedtls_rsa_export_crt(rsa, &T, NULL, NULL)) != 0 || (ret = mbedtls_asn1_write_mpi(p, start, &T)) < 0) {
        goto end_of_export;
    }
    len += ret;
...
static int mbedtls_ct_rsaes_pkcs1_v15_unpadding(unsigned char* input,
    ...
    plaintext_size = mbedtls_ct_uint_if(bad, (unsigned)plaintext_max_size, (unsigned)(ilen - pad_count - 3));
    ...
    plaintext_size = mbedtls_ct_uint_if(output_too_large, (unsigned)plaintext_max_size, (unsigned)plaintext_size);
     ...
     * information. */
    mbedtls_ct_memmove_left(input + ilen - plaintext_max_size, plaintext_max_size, plaintext_max_size - plaintext_size);
 ...
 */
static int rsa_apply_crt(mbedtls_mpi* T, const mbedtls_mpi* TP, const mbedtls_mpi* TQ, const mbedtls_rsa_context* ctx) {
    int ret;
 ...
 */
static int compute_hash(mbedtls_md_type_t md_alg, const unsigned char* input, size_t ilen, unsigned char* output) {
    const mbedtls_md_info_t* md_info;
...
int mbedtls_rsa_rsaes_oaep_encrypt(mbedtls_rsa_context* ctx,
    ...
    *p++ = 1;
    ...
    if ((ret = mgf_mask(output + hlen + 1, olen - hlen - 1, output + 1, hlen, (mbedtls_md_type_t)ctx->hash_id)) != 0) {
        return ret;
    }

    /* maskedSeed: Apply seedMask to seed */
    if ((ret = mgf_mask(output + 1, hlen, output + hlen + 1, olen - hlen - 1, (mbedtls_md_type_t)ctx->hash_id)) != 0) {
        return ret;
    }
...
int mbedtls_rsa_rsaes_oaep_decrypt(mbedtls_rsa_context* ctx,
    ...
    if ((ret = mgf_mask(buf + 1, hlen, buf + hlen + 1, ilen - hlen - 1, (mbedtls_md_type_t)ctx->hash_id)) != 0 ||
        /* DB: Apply dbMask to maskedDB */
        (ret = mgf_mask(buf + hlen + 1, ilen - hlen - 1, buf + 1, hlen, (mbedtls_md_type_t)ctx->hash_id)) != 0) {
        goto cleanup;
    }
...
#if defined(MBEDTLS_PKCS1_V21)
        case MBEDTLS_RSA_PKCS_V21:
            return mbedtls_rsa_rsassa_pss_sign_ext(ctx, f_rng, p_rng, md_alg, hashlen, hash, MBEDTLS_RSA_SALT_LEN_ANY,
                                                   sig);
#endif
...
int mbedtls_rsa_self_test(int verbose) {
    ...
    if (mbedtls_md(mbedtls_md_info_from_type(MBEDTLS_MD_SHA1), rsa_plaintext, PT_LEN, sha1sum) != 0) {
        if (verbose != 0) {
            mbedtls_printf("failed\n");
        }
