...
psa_status_t mbedtls_nist_kw_wrap(mbedtls_svc_key_id_t key,
    ...
    *output_length = 0;
    ...
    if (mode == MBEDTLS_KW_MODE_KW) {
        ...
        MBEDTLS_PUT_UINT32_BE((input_length & 0xffffffff), output, KW_SEMIBLOCK_LENGTH / 2);
    ...
    if (mode == MBEDTLS_KW_MODE_KWP && input_length <= KW_SEMIBLOCK_LENGTH) {
        memcpy(inbuff, output, 16);
        ret = psa_cipher_update(&wrap_operation, inbuff, 16, output, output_size, &update_output_length);
        if (ret != PSA_SUCCESS) {
            goto cleanup;
        }
        ret = psa_cipher_finish(&wrap_operation, mbedtls_buffer_offset(output, update_output_length),
        ...
        *output_length = update_output_length + finish_output_length;
        ...
        for (t = 1; t <= s; t++) {
            ...
            ret = psa_cipher_update(&wrap_operation, inbuff, 16, outbuff, sizeof(outbuff), &olen);
            if (ret != PSA_SUCCESS) {
                goto cleanup;
            }
 ...
 */
static int unwrap(const unsigned char* input,
                  size_t semiblocks,
    ...
    *output_length = 0;
    ...
    for (t = s; t >= 1; t--) {
        ...
        ret = psa_cipher_update(operation, inbuff, 16, outbuff, sizeof(outbuff), output_length);
        if (ret != PSA_SUCCESS) {
            goto cleanup;
        }
...
psa_status_t mbedtls_nist_kw_unwrap(mbedtls_svc_key_id_t key,
    ...
    *output_length = 0;
    ...
    if (mode == MBEDTLS_KW_MODE_KW) {
        ...
        ret = unwrap(input, input_length / KW_SEMIBLOCK_LENGTH, A, output, output_length, &unwrap_operation);
        if (ret != PSA_SUCCESS) {
            goto cleanup;
        }
        ...
        if (input_length == KW_SEMIBLOCK_LENGTH * 2) {
            unsigned char outbuff[KW_SEMIBLOCK_LENGTH * 2];
            ret = psa_cipher_update(&unwrap_operation, input, 16, outbuff, sizeof(outbuff), output_length);
            if (ret != PSA_SUCCESS) {
                goto cleanup;
            }
            ret = psa_cipher_finish(&unwrap_operation, outbuff + *output_length, sizeof(outbuff) - *output_length,
                                    &part_length);
            if (ret != PSA_SUCCESS) {
                goto cleanup;
            }
        ...
        ret = mbedtls_ct_error_if(mbedtls_ct_uint_gt(padlen, 7), PSA_ERROR_INVALID_SIGNATURE, ret);
        padlen &= 7;

        /* Check padding in "constant-time" */
        const uint8_t zero[KW_SEMIBLOCK_LENGTH] = {0};
        diff = mbedtls_ct_memcmp_partial(&output[*output_length - KW_SEMIBLOCK_LENGTH], zero, KW_SEMIBLOCK_LENGTH,
                                         KW_SEMIBLOCK_LENGTH - padlen, 0);
