...
void mbedtls_test_ssl_log_analyzer(void* ctx, int level, const char* file, int line, const char* str) {
...
void mbedtls_test_init_handshake_options(mbedtls_test_handshake_test_options* opts) {
    ...
    TEST_EQUAL(mbedtls_ssl_cache_get_timeout(opts->cache), MBEDTLS_SSL_CACHE_DEFAULT_TIMEOUT);
#endif
exit:
    return;
#endif
...
int mbedtls_test_ssl_buffer_setup(mbedtls_test_ssl_buffer* buf, size_t capacity) {
    buf->buffer = (unsigned char*)mbedtls_calloc(capacity, sizeof(unsigned char));
    if (NULL == buf->buffer) {
...
int mbedtls_test_ssl_buffer_put(mbedtls_test_ssl_buffer* buf, const unsigned char* input, size_t input_len) {
    size_t overflow = 0;
    ...
    if (buf->start + buf->content_length < buf->capacity) {
        ...
        if (buf->start + buf->content_length + input_len > buf->capacity) {
            overflow = (buf->start + buf->content_length + input_len) % buf->capacity;
        }

        memcpy(buf->buffer + buf->start + buf->content_length, input, input_len - overflow);
        memcpy(buf->buffer, input + input_len - overflow, overflow);
    }
    else {
        /* The buffer has come full circle and free space is in the middle */
        memcpy(buf->buffer + buf->start + buf->content_length - buf->capacity, input, input_len);
    }
...
int mbedtls_test_ssl_message_queue_setup(mbedtls_test_ssl_message_queue* queue, size_t capacity) {
...
int mbedtls_test_ssl_message_queue_push_info(mbedtls_test_ssl_message_queue* queue, size_t len) {
    int place;
    if (queue == NULL) {
...
int mbedtls_test_ssl_message_queue_pop_info(mbedtls_test_ssl_message_queue* queue, size_t buf_len) {
    ...
    return (message_length > INT_MAX && buf_len > INT_MAX) ? INT_MAX
           : (message_length > buf_len)                    ? (int)buf_len
                                                           : (int)message_length;
}
 ...
 */
static int test_ssl_message_queue_peek_info(mbedtls_test_ssl_message_queue* queue, size_t buf_len, size_t* msg_len) {
...
int mbedtls_test_mock_socket_connect(mbedtls_test_mock_socket* peer1, mbedtls_test_mock_socket* peer2, size_t bufsize) {
    int ret = -1;

    peer1->output = (mbedtls_test_ssl_buffer*)mbedtls_calloc(1, sizeof(mbedtls_test_ssl_buffer));
    ...
    peer2->output = (mbedtls_test_ssl_buffer*)mbedtls_calloc(1, sizeof(mbedtls_test_ssl_buffer));
...
int mbedtls_test_mock_tcp_send_msg(void* ctx, const unsigned char* buf, size_t len) {
    ...
    if (context == NULL || context->socket == NULL || context->queue_output == NULL) {
...
int mbedtls_test_mock_tcp_recv_msg(void* ctx, unsigned char* buf, size_t buf_len) {
    ...
    if (context == NULL || context->socket == NULL || context->queue_input == NULL) {
        return MBEDTLS_TEST_ERROR_CONTEXT_ERROR;
    }
...
#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED) && defined(MBEDTLS_SSL_HANDSHAKE_WITH_PSK_ENABLED) && \
    defined(MBEDTLS_SSL_SRV_C)
static int psk_dummy_callback(void* p_info, mbedtls_ssl_context* ssl, const unsigned char* name, size_t name_len) {
    (void)p_info;
    (void)ssl;
    (void)name;
    (void)name_len;
...
static int set_ciphersuite(mbedtls_test_ssl_endpoint* ep, const char* cipher) {
    ...
    const mbedtls_ssl_ciphersuite_t* ciphersuite_info = mbedtls_ssl_ciphersuite_from_id(ep->ciphersuites[0]);
...
static int load_endpoint_rsa(mbedtls_test_ssl_endpoint* ep) {
    ...
    if (ep->conf.endpoint == MBEDTLS_SSL_IS_SERVER) {
        ...
        TEST_EQUAL(ret, 0);
        ret = mbedtls_pk_parse_key(ep->pkey, (const unsigned char*)mbedtls_test_srv_key_rsa_der,
                                   mbedtls_test_srv_key_rsa_der_len, NULL, 0);
        TEST_EQUAL(ret, 0);
    }
    else {
        ret = mbedtls_x509_crt_parse(ep->cert, (const unsigned char*)mbedtls_test_cli_crt_rsa_der,
                                     mbedtls_test_cli_crt_rsa_der_len);
        TEST_EQUAL(ret, 0);
        ret = mbedtls_pk_parse_key(ep->pkey, (const unsigned char*)mbedtls_test_cli_key_rsa_der,
...
static int load_endpoint_ecc(mbedtls_test_ssl_endpoint* ep) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    if (ep->conf.endpoint == MBEDTLS_SSL_IS_SERVER) {
        ret = mbedtls_x509_crt_parse(ep->cert, (const unsigned char*)mbedtls_test_srv_crt_ec_der,
                                     mbedtls_test_srv_crt_ec_der_len);
        TEST_EQUAL(ret, 0);
        ret = mbedtls_pk_parse_key(ep->pkey, (const unsigned char*)mbedtls_test_srv_key_ec_der,
                                   mbedtls_test_srv_key_ec_der_len, NULL, 0);
        TEST_EQUAL(ret, 0);
    }
    else {
        ret = mbedtls_x509_crt_parse(ep->cert, (const unsigned char*)mbedtls_test_cli_crt_ec_der,
                                     mbedtls_test_cli_crt_ec_len);
        TEST_EQUAL(ret, 0);
        ret = mbedtls_pk_parse_key(ep->pkey, (const unsigned char*)mbedtls_test_cli_key_ec_der,
...
int mbedtls_test_ssl_endpoint_certificate_init(mbedtls_test_ssl_endpoint* ep,
    ...
    for (i = 0; mbedtls_test_cas_der[i] != NULL; i++) {
        ret = mbedtls_x509_crt_parse_der(ep->ca_chain, (const unsigned char*)mbedtls_test_cas_der[i],
    ...
    if (opaque_alg != 0) {
        ...
        TEST_EQUAL(mbedtls_pk_get_psa_attributes(ep->pkey, PSA_KEY_USAGE_SIGN_HASH, &key_attr), 0);
...
int mbedtls_test_ssl_endpoint_init_conf(mbedtls_test_ssl_endpoint* ep,
                                        int endpoint_type,
    ...
    (void)mbedtls_test_rnd_std_rand(NULL, (void*)&ep->user_data_cookie, sizeof(ep->user_data_cookie));
    ...
    ret = mbedtls_ssl_config_defaults(&(ep->conf), endpoint_type,
                                      options->dtls ? MBEDTLS_SSL_TRANSPORT_DATAGRAM : MBEDTLS_SSL_TRANSPORT_STREAM,
                                      MBEDTLS_SSL_PRESET_DEFAULT);
    TEST_EQUAL(ret, 0);

    if (MBEDTLS_SSL_IS_CLIENT == endpoint_type) {
        if (options->client_min_version != MBEDTLS_SSL_VERSION_UNKNOWN) {
            mbedtls_ssl_conf_min_tls_version(&(ep->conf), options->client_min_version);
        }

        if (options->client_max_version != MBEDTLS_SSL_VERSION_UNKNOWN) {
            mbedtls_ssl_conf_max_tls_version(&(ep->conf), options->client_max_version);
        }
    }
    else {
        if (options->server_min_version != MBEDTLS_SSL_VERSION_UNKNOWN) {
            mbedtls_ssl_conf_min_tls_version(&(ep->conf), options->server_min_version);
        }

        if (options->server_max_version != MBEDTLS_SSL_VERSION_UNKNOWN) {
            mbedtls_ssl_conf_max_tls_version(&(ep->conf), options->server_max_version);
        }
    }
    ...
    if (endpoint_type == MBEDTLS_SSL_IS_SERVER && (options->max_early_data_size >= 0)) {
        mbedtls_ssl_conf_max_early_data_size(&(ep->conf), options->max_early_data_size);
    }
#endif
    ...
    if (options->renegotiate) {
        mbedtls_ssl_conf_renegotiation(&ep->conf, MBEDTLS_SSL_RENEGOTIATION_ENABLED);
    ...
    if (endpoint_type == MBEDTLS_SSL_IS_SERVER && options->cache != NULL) {
        mbedtls_ssl_conf_session_cache(&(ep->conf), options->cache, mbedtls_ssl_cache_get, mbedtls_ssl_cache_set);
    }
#endif

#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
    TEST_EQUAL(mbedtls_ssl_conf_max_frag_len(&ep->conf, (unsigned char)options->mfl), 0);
#else
...
#if defined(MBEDTLS_SSL_SRV_C)
    if (endpoint_type == MBEDTLS_SSL_IS_SERVER && options->srv_log_fun != NULL) {
        mbedtls_ssl_conf_dbg(&(ep->conf), options->srv_log_fun, options->srv_log_obj);
    }
#endif
#if defined(MBEDTLS_SSL_CLI_C)
    if (endpoint_type == MBEDTLS_SSL_IS_CLIENT && options->cli_log_fun != NULL) {
        mbedtls_ssl_conf_dbg(&(ep->conf), options->cli_log_fun, options->cli_log_obj);
    }
#endif
#endif /* MBEDTLS_DEBUG_C */

    ret = mbedtls_test_ssl_endpoint_certificate_init(ep, options->pk_alg, options->opaque_alg, options->opaque_alg2,
                                                     options->opaque_usage);
    TEST_EQUAL(ret, 0);

#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_PSK_ENABLED)
    if (options->psk_str != NULL && options->psk_str->len > 0) {
        TEST_EQUAL(mbedtls_ssl_conf_psk(&ep->conf, options->psk_str->x, options->psk_str->len,
    ...
    TEST_EQUAL(mbedtls_ssl_conf_get_user_data_n(&ep->conf), ep->user_data_cookie);
...
int mbedtls_test_ssl_endpoint_init_ssl(mbedtls_test_ssl_endpoint* ep,
                                       const mbedtls_test_handshake_test_options* options) {
    ...
    if (options->dtls) {
        mbedtls_ssl_set_bio(&(ep->ssl), &ep->dtls_context, mbedtls_test_mock_tcp_send_msg,
                            mbedtls_test_mock_tcp_recv_msg, NULL);
#if defined(MBEDTLS_TIMING_C)
        mbedtls_ssl_set_timer_cb(&ep->ssl, &ep->timer, mbedtls_timing_set_delay, mbedtls_timing_get_delay);
#endif
    }
    else {
        mbedtls_ssl_set_bio(&(ep->ssl), &(ep->socket), mbedtls_test_mock_tcp_send_nb, mbedtls_test_mock_tcp_recv_nb,
                            NULL);
    }
...
int mbedtls_test_ssl_dtls_join_endpoints(mbedtls_test_ssl_endpoint* client, mbedtls_test_ssl_endpoint* server) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    ret = mbedtls_test_message_socket_setup(&client->queue_input, &server->queue_input, 100, &(client->socket),
                                            &client->dtls_context);
    TEST_EQUAL(ret, 0);

    ret = mbedtls_test_message_socket_setup(&server->queue_input, &client->queue_input, 100, &(server->socket),
                                            &server->dtls_context);
    TEST_EQUAL(ret, 0);
...
int mbedtls_test_move_handshake_to_state(mbedtls_ssl_context* ssl, mbedtls_ssl_context* second_ssl, int state) {
    enum { BUFFSIZE = 1024 };
    int max_steps = 1000;
    int ret = 0;
...
static int mbedtls_ssl_write_fragment(mbedtls_ssl_context* ssl,
    ...
    if (expected_fragments == 0) {
        ...
        TEST_ASSERT(ret == buf_len || ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE);
    }
    else {
        /* Used for TLS and the message size larger than MFL */
        TEST_ASSERT(expected_fragments > 1);
        TEST_ASSERT((ret >= 0 && ret <= buf_len) || ret == MBEDTLS_ERR_SSL_WANT_READ ||
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2) && defined(PSA_WANT_ALG_CBC_NO_PADDING) && defined(PSA_WANT_KEY_TYPE_AES)
...
int mbedtls_test_ssl_build_transforms(mbedtls_ssl_transform* t_in,
    ...
    maclen = 0;
    mbedtls_test_ssl_cipher_info_from_type((mbedtls_cipher_type_t)cipher_type, &cipher_mode, &key_bits, &ivlen);
    ...
    if (cipher_mode == MBEDTLS_MODE_CBC || cipher_mode == MBEDTLS_MODE_STREAM) {
        ...
        CHK(psa_import_key(&attributes, md0, maclen, &t_in->psa_mac_enc) == PSA_SUCCESS);

        CHK(psa_import_key(&attributes, md1, maclen, &t_out->psa_mac_enc) == PSA_SUCCESS);

        if (cipher_mode == MBEDTLS_MODE_STREAM || etm == MBEDTLS_SSL_ETM_DISABLED) {
        ...
        CHK(psa_import_key(&attributes, md1, maclen, &t_in->psa_mac_dec) == PSA_SUCCESS);

        CHK(psa_import_key(&attributes, md0, maclen, &t_out->psa_mac_dec) == PSA_SUCCESS);
    }
#else
    ((void)hash_id);
    ...
    if (alg != MBEDTLS_SSL_NULL_CIPHER) {
        ...
        status = psa_import_key(&attributes, key0, PSA_BITS_TO_BYTES(key_bits), &t_in->psa_key_enc);
        ...
        status = psa_import_key(&attributes, key1, PSA_BITS_TO_BYTES(key_bits), &t_out->psa_key_enc);
        ...
        status = psa_import_key(&attributes, key1, PSA_BITS_TO_BYTES(key_bits), &t_in->psa_key_dec);
        ...
        status = psa_import_key(&attributes, key0, PSA_BITS_TO_BYTES(key_bits), &t_out->psa_key_dec);
...
#if defined(MBEDTLS_SSL_SOME_SUITES_USE_MAC)
int mbedtls_test_ssl_prepare_record_mac(mbedtls_record* record, mbedtls_ssl_transform* transform_out) {
    ...
    TEST_EQUAL(PSA_SUCCESS, psa_mac_sign_setup(&operation, transform_out->psa_mac_enc, transform_out->psa_mac_alg));
    TEST_EQUAL(PSA_SUCCESS, psa_mac_update(&operation, add_data, 13));
    TEST_EQUAL(PSA_SUCCESS, psa_mac_update(&operation, record->buf + record->data_offset, record->data_len));
    ...
    TEST_EQUAL(PSA_SUCCESS, psa_mac_sign_finish(&operation, mac, sizeof(mac), &sign_mac_length));
...
int mbedtls_test_ssl_tls12_populate_session(mbedtls_ssl_session* session,
    ...
    TEST_ASSERT(endpoint_type == MBEDTLS_SSL_IS_CLIENT || endpoint_type == MBEDTLS_SSL_IS_SERVER);
    ...
    if (crt_file != NULL && strlen(crt_file) != 0) {
        ...
        *session->peer_cert = tmp_crt;
        ...
        session->peer_cert_digest = mbedtls_calloc(1, MBEDTLS_SSL_PEER_CERT_DIGEST_DFL_LEN);
        if (session->peer_cert_digest == NULL) {
            return -1;
        }

        psa_algorithm_t psa_alg = mbedtls_md_psa_alg_from_type(MBEDTLS_SSL_PEER_CERT_DIGEST_DFL_TYPE);
        size_t hash_size = 0;
        psa_status_t status = psa_hash_compute(psa_alg, tmp_crt.raw.p, tmp_crt.raw.len, session->peer_cert_digest,
                                               MBEDTLS_SSL_PEER_CERT_DIGEST_DFL_LEN, &hash_size);
        ret = PSA_TO_MBEDTLS_ERR(status);
        if (ret != 0) {
            return ret;
        }
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
int mbedtls_test_ssl_tls13_populate_session(mbedtls_ssl_session* session, int ticket_len, int endpoint_type) {
    ((void)ticket_len);
    session->tls_version = MBEDTLS_SSL_VERSION_TLS1_3;
    session->endpoint = endpoint_type == MBEDTLS_SSL_IS_CLIENT ? MBEDTLS_SSL_IS_CLIENT : MBEDTLS_SSL_IS_SERVER;
    session->ciphersuite = 0xabcd;
...
int mbedtls_test_ssl_exchange_data(mbedtls_ssl_context* ssl_1,
                                   int msg_len_1,
    ...
    for (msg_type = 0; msg_type < 2; msg_type++) {
        ...
        while (read_1 < msg_len_2 || read_2 < msg_len_1) {
            ...
            if (msg_len_1 > written_1) {
                ret = mbedtls_ssl_write_fragment(ssl_1, msg_buf_1, msg_len_1, &written_1, expected_fragments_1);
            ...
            if (msg_len_2 > written_2) {
                ret = mbedtls_ssl_write_fragment(ssl_2, msg_buf_2, msg_len_2, &written_2, expected_fragments_2);
            ...
            if (read_1 < msg_len_2) {
                ret =
                    mbedtls_ssl_read_fragment(ssl_1, in_buf_1, msg_len_2, &read_1, &fragments_2, expected_fragments_2);
                TEST_EQUAL(ret, 0);
            }
            ...
            if (read_2 < msg_len_1) {
                ret =
                    mbedtls_ssl_read_fragment(ssl_2, in_buf_2, msg_len_1, &read_2, &fragments_1, expected_fragments_1);
                TEST_EQUAL(ret, 0);
            }
        }
 ...
 */
#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED) && \
    (defined(MBEDTLS_SSL_RENEGOTIATION) || defined(MBEDTLS_SSL_VARIABLE_BUFFER_LENGTH))
static int exchange_data(mbedtls_ssl_context* ssl_1, mbedtls_ssl_context* ssl_2) {
    return mbedtls_test_ssl_exchange_data(ssl_1, 256, 1, ssl_2, 256, 1);
}
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED && \
...
static int check_ssl_version(mbedtls_ssl_protocol_version expected_negotiated_version,
    ...
    mbedtls_ssl_protocol_version version_number = mbedtls_ssl_get_version_number(client);
...
#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
int mbedtls_test_ssl_do_handshake_with_endpoints(mbedtls_test_ssl_endpoint* server_ep,
...
static int test_renegotiation(const mbedtls_test_handshake_test_options* options,
...
#endif
    TEST_ASSERT(ret == 0 || ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE);
    TEST_EQUAL(server->ssl.renego_status, MBEDTLS_SSL_RENEGOTIATION_DONE);
    ...
    if (options->resize_buffers != 0) {
        TEST_EQUAL(client->ssl.out_buf_len, mbedtls_ssl_get_output_buflen(&client->ssl));
        TEST_EQUAL(client->ssl.in_buf_len, mbedtls_ssl_get_input_buflen(&client->ssl));
        TEST_EQUAL(server->ssl.out_buf_len, mbedtls_ssl_get_output_buflen(&server->ssl));
        TEST_EQUAL(server->ssl.in_buf_len, mbedtls_ssl_get_input_buflen(&server->ssl));
    }
...
static int test_serialization(const mbedtls_test_handshake_test_options* options,
    ...
    TEST_EQUAL(mbedtls_ssl_context_save(&(server->ssl), NULL, 0, &context_buf_len), MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL);

    context_buf = mbedtls_calloc(1, context_buf_len);
    TEST_ASSERT(context_buf != NULL);

    TEST_EQUAL(mbedtls_ssl_context_save(&(server->ssl), context_buf, context_buf_len, &context_buf_len), 0);
    ...
    mbedtls_ssl_set_bio(&(server->ssl), &server->dtls_context, mbedtls_test_mock_tcp_send_msg,
                        mbedtls_test_mock_tcp_recv_msg, NULL);

    mbedtls_ssl_set_user_data_p(&server->ssl, server);

#if defined(MBEDTLS_TIMING_C)
    mbedtls_ssl_set_timer_cb(&server->ssl, &server->timer, mbedtls_timing_set_delay, mbedtls_timing_get_delay);
#endif
#if defined(MBEDTLS_SSL_VARIABLE_BUFFER_LENGTH)
    if (options->resize_buffers != 0) {
        /* Ensure that the buffer sizes are appropriate before resizes */
...
#endif
    TEST_EQUAL(mbedtls_ssl_context_load(&(server->ssl), context_buf, context_buf_len), 0);
    ...
    if (options->cli_msg_len != 0 || options->srv_msg_len != 0) {
        TEST_EQUAL(
            mbedtls_test_ssl_exchange_data(&(client->ssl), options->cli_msg_len, options->expected_cli_fragments,
                                           &(server->ssl), options->srv_msg_len, options->expected_srv_fragments),
            0);
    }
...
int mbedtls_test_ssl_perform_connection(const mbedtls_test_handshake_test_options* options,
                                        mbedtls_test_ssl_endpoint* client,
                                        mbedtls_test_ssl_endpoint* server) {
    enum { BUFFSIZE = 17000 };
    ...
    TEST_EQUAL(mbedtls_test_mock_socket_connect(&(client->socket), &(server->socket), BUFFSIZE), 0);
    ...
    TEST_EQUAL(mbedtls_test_move_handshake_to_state(&(client->ssl), &(server->ssl), MBEDTLS_SSL_HANDSHAKE_OVER),
    ...
    TEST_EQUAL(mbedtls_test_move_handshake_to_state(&(server->ssl), &(client->ssl), MBEDTLS_SSL_HANDSHAKE_OVER), 0);

    TEST_EQUAL(mbedtls_ssl_is_handshake_over(&server->ssl), 1);

    /* Check that both sides have negotiated the expected version. */
    TEST_ASSERT(check_ssl_version(options->expected_negotiated_version, &client->ssl, &server->ssl));

    if (options->expected_ciphersuite != 0) {
        TEST_EQUAL(server->ssl.session->ciphersuite, options->expected_ciphersuite);
    }
...
void mbedtls_test_ssl_perform_handshake(const mbedtls_test_handshake_test_options* options) {
    ...
    /* Client side */
    TEST_EQUAL(mbedtls_test_ssl_endpoint_init(client, MBEDTLS_SSL_IS_CLIENT, options), 0);

    /* Server side */
    TEST_EQUAL(mbedtls_test_ssl_endpoint_init(server, MBEDTLS_SSL_IS_SERVER, options), 0);
...
#if defined(MBEDTLS_TEST_HOOKS)
int mbedtls_test_tweak_tls13_certificate_msg_vector_len(unsigned char* buf,
                                                        unsigned char** end,
                                                        int tweak,
                                                        int* expected_result,
    ...
    *expected_result = MBEDTLS_ERR_SSL_DECODE_ERROR;
    ...
    switch (tweak) {
        ...
        case 2:
             ...
             */
            *p_certificate_request_context_len = (unsigned char)certificate_request_context_len + 1;
            reset_chk_buf_ptr_args(args);
            break;

        case 3:
            /* Failure when checking if certificate_list data can be read. */
            MBEDTLS_PUT_UINT24_BE(certificate_list_len + 1, p_certificate_list_len, 0);
            set_chk_buf_ptr_args(args, certificate_list, *end, certificate_list_len + 1);
            break;
        ...
        case 5:
            ...
            MBEDTLS_PUT_UINT24_BE(certificate_list_len - 3 + 1, p_cert_data_len, 0);
            set_chk_buf_ptr_args(args, cert_data, certificate_list + certificate_list_len,
                                 certificate_list_len - 3 + 1);
            break;

        case 6:
            /* Failure when checking if the extensions length can be read. */
            MBEDTLS_PUT_UINT24_BE(certificate_list_len - extensions_len - 1, p_certificate_list_len, 0);
            set_chk_buf_ptr_args(args, p_extensions_len, certificate_list + certificate_list_len - extensions_len - 1,
                                 2);
            break;
        ...
        case 7:
            ...
            set_chk_buf_ptr_args(args, extensions, certificate_list + certificate_list_len, extensions_len + 1);
            break;
 ...
 */
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
int mbedtls_test_ticket_write(void* p_ticket,
    ...
    if ((ret = mbedtls_ssl_session_save(session, start, end - start, tlen)) != 0) {
        return ret;
    }
...
int mbedtls_test_ticket_parse(void* p_ticket, mbedtls_ssl_session* session, unsigned char* buf, size_t len) {
    ((void)p_ticket);
    ...
    defined(MBEDTLS_SSL_SESSION_TICKETS) && defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
int mbedtls_test_get_tls13_ticket(mbedtls_test_handshake_test_options* client_options,
    ...
    ret = mbedtls_test_ssl_endpoint_init(&client_ep, MBEDTLS_SSL_IS_CLIENT, client_options);
    TEST_EQUAL(ret, 0);

    ret = mbedtls_test_ssl_endpoint_init(&server_ep, MBEDTLS_SSL_IS_SERVER, server_options);
    TEST_EQUAL(ret, 0);

    mbedtls_ssl_conf_session_tickets_cb(&server_ep.conf, mbedtls_test_ticket_write, mbedtls_test_ticket_parse, NULL);

    ret = mbedtls_test_mock_socket_connect(&(client_ep.socket), &(server_ep.socket), 1024);
    TEST_EQUAL(ret, 0);

    TEST_EQUAL(mbedtls_test_move_handshake_to_state(&(server_ep.ssl), &(client_ep.ssl), MBEDTLS_SSL_HANDSHAKE_OVER), 0);
