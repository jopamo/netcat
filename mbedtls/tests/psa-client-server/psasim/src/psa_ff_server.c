                            ...
                            switch (msg.message_text.psa_type) {
                                ...
                                default:
                                    ...
                                    if (msg.message_text.psa_type >= 0) {
                                        ...
                                        FATAL("UNKNOWN MESSAGE TYPE RECEIVED %li", msg.message_type);
                                    }
                                    break;
                            }
                            messages[i].handle = i;
...
void psa_write(psa_handle_t msg_handle, uint32_t outvec_idx, const void* buffer, size_t num_bytes) {
    ...
    if (num_bytes > messages[msg_handle].out_size[outvec_idx]) {
        ERROR("Program tried to write too much data %lu/%lu", num_bytes, messages[msg_handle].out_size[outvec_idx]);
    }
...
void psa_notify(int32_t partition_id) {
    ...
    INFO("psa_notify: notifying partition %u using %s", partition_id, pathname);
