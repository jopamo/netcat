...
int psa_mac_main(void) {
    ...
    const uint8_t mbedtls_test_hmac_sha256[] = {0xae, 0x72, 0x34, 0x5a, 0x10, 0x36, 0xfb, 0x71, 0x35, 0x3c, 0x7d,
                                                0x6c, 0x81, 0x98, 0x52, 0x86, 0x00, 0x4a, 0x43, 0x7c, 0x2d, 0xb3,
                                                0x1a, 0xd8, 0x67, 0xb1, 0xad, 0x11, 0x4d, 0x18, 0x49, 0x8b};
    ...
    psa_set_key_usage_flags(&attributes,
                            PSA_KEY_USAGE_VERIFY_MESSAGE | PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_SIGN_MESSAGE);
    ...
    status =
        psa_mac_compute(key_id, PSA_ALG_HMAC(PSA_ALG_SHA_256), input, sizeof(input), mac, sizeof(mac), &mac_size_real);
    if (status != PSA_SUCCESS) {
    ...
    if (safer_memcmp(mac, mbedtls_test_hmac_sha256, mac_size_real) != 0) {
    ...
    status = psa_mac_verify(key_id, PSA_ALG_HMAC(PSA_ALG_SHA_256), input, sizeof(input), mbedtls_test_hmac_sha256,
