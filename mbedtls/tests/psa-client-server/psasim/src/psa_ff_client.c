...
static psa_status_t process_response(int rx_qid, vectors_t* vecs, int type, int* internal_server_qid) {
    ...
    while (1) {
        ...
        switch (response.message_type) {
            ...
            default:
                FATAL("   ERROR: unknown internal message type: %ld", response.message_type);
        }
    }
}

static psa_status_t send(int rx_qid,
                         int server_qid,
                         int* internal_server_qid,
                         int32_t type,
...
psa_handle_t psa_connect(uint32_t sid, uint32_t minor_version) {
    ...
    if (idx >= 0) {
        ...
        if (get_queue_info(pathname, &handles[idx].client_qid, &handles[idx].server_qid) >= 0) {
            ret = send(handles[idx].client_qid, handles[idx].server_qid, &handles[idx].internal_server_qid,
                       PSA_IPC_CONNECT, minor_version, NULL);
...
psa_status_t psa_call(psa_handle_t handle,
    ...
    return send(handles[handle].client_qid, handles[handle].server_qid, &handles[handle].internal_server_qid, type, 0,
                &vecs);
}

void psa_close(psa_handle_t handle) {
    handle_is_valid(handle);
    if (send(handles[handle].client_qid, handles[handle].server_qid, &handles[handle].internal_server_qid,
             PSA_IPC_DISCONNECT, 0, NULL)) {
