...
int psasim_serialise_uint16_t(uint8_t** pos, size_t* remaining, uint16_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_deserialise_uint16_t(uint8_t** pos, size_t* remaining, uint16_t* value) {
    if (*remaining < sizeof(*value)) {
        return 0;
    }
...
int psasim_serialise_uint32_t(uint8_t** pos, size_t* remaining, uint32_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_deserialise_uint32_t(uint8_t** pos, size_t* remaining, uint32_t* value) {
    if (*remaining < sizeof(*value)) {
        return 0;
    }
...
int psasim_serialise_uint64_t(uint8_t** pos, size_t* remaining, uint64_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_deserialise_uint64_t(uint8_t** pos, size_t* remaining, uint64_t* value) {
    if (*remaining < sizeof(*value)) {
        return 0;
    }
...
size_t psasim_serialise_psa_custom_key_parameters_t_needs(psa_custom_key_parameters_t value) {
    return sizeof(value);
}

int psasim_serialise_psa_custom_key_parameters_t(uint8_t** pos, size_t* remaining, psa_custom_key_parameters_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_serialise_psa_key_derivation_step_t(uint8_t** pos, size_t* remaining, psa_key_derivation_step_t value) {
    return psasim_serialise_uint16_t(pos, remaining, value);
}

int psasim_deserialise_psa_key_derivation_step_t(uint8_t** pos, size_t* remaining, psa_key_derivation_step_t* value) {
...
int psasim_server_serialise_psa_hash_operation_t(uint8_t** pos,
    ...
    if (completed) {
        memset(&hash_operations[slot], 0, sizeof(psa_hash_operation_t));
...
int psasim_server_deserialise_psa_hash_operation_t(uint8_t** pos, size_t* remaining, psa_hash_operation_t** operation) {
    psasim_operation_t client_operation;
...
int psasim_serialise_psa_aead_operation_t(uint8_t** pos, size_t* remaining, psa_aead_operation_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_deserialise_psa_aead_operation_t(uint8_t** pos, size_t* remaining, psa_aead_operation_t* value) {
    if (*remaining < sizeof(*value)) {
        return 0;
    }
...
int psasim_server_serialise_psa_aead_operation_t(uint8_t** pos,
    ...
    if (completed) {
        memset(&aead_operations[slot], 0, sizeof(psa_aead_operation_t));
...
int psasim_server_deserialise_psa_aead_operation_t(uint8_t** pos, size_t* remaining, psa_aead_operation_t** operation) {
    psasim_operation_t client_operation;
...
int psasim_serialise_psa_key_attributes_t(uint8_t** pos, size_t* remaining, psa_key_attributes_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_deserialise_psa_key_attributes_t(uint8_t** pos, size_t* remaining, psa_key_attributes_t* value) {
    if (*remaining < sizeof(*value)) {
        return 0;
    }
...
int psasim_serialise_psa_mac_operation_t(uint8_t** pos, size_t* remaining, psa_mac_operation_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_deserialise_psa_mac_operation_t(uint8_t** pos, size_t* remaining, psa_mac_operation_t* value) {
    if (*remaining < sizeof(*value)) {
        return 0;
    }
...
int psasim_server_serialise_psa_mac_operation_t(uint8_t** pos,
    ...
    if (completed) {
        memset(&mac_operations[slot], 0, sizeof(psa_mac_operation_t));
...
int psasim_server_deserialise_psa_mac_operation_t(uint8_t** pos, size_t* remaining, psa_mac_operation_t** operation) {
    psasim_operation_t client_operation;
...
int psasim_serialise_psa_cipher_operation_t(uint8_t** pos, size_t* remaining, psa_cipher_operation_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_deserialise_psa_cipher_operation_t(uint8_t** pos, size_t* remaining, psa_cipher_operation_t* value) {
    if (*remaining < sizeof(*value)) {
        return 0;
    }
...
int psasim_server_serialise_psa_cipher_operation_t(uint8_t** pos,
    ...
    if (completed) {
        memset(&cipher_operations[slot], 0, sizeof(psa_cipher_operation_t));
...
int psasim_server_serialise_psa_key_derivation_operation_t(uint8_t** pos,
    ...
    if (completed) {
        memset(&key_derivation_operations[slot], 0, sizeof(psa_key_derivation_operation_t));
...
int psasim_server_serialise_psa_sign_hash_interruptible_operation_t(uint8_t** pos,
    ...
    if (completed) {
        memset(&sign_hash_interruptible_operations[slot], 0, sizeof(psa_sign_hash_interruptible_operation_t));
...
int psasim_server_deserialise_psa_sign_hash_interruptible_operation_t(
    uint8_t** pos,
    size_t* remaining,
    psa_sign_hash_interruptible_operation_t** operation) {
    psasim_operation_t client_operation;
...
int psasim_server_serialise_psa_verify_hash_interruptible_operation_t(
    uint8_t** pos,
    size_t* remaining,
    psa_verify_hash_interruptible_operation_t* operation,
    int completed) {
    ...
    if (completed) {
        memset(&verify_hash_interruptible_operations[slot], 0, sizeof(psa_verify_hash_interruptible_operation_t));
...
int psasim_server_deserialise_psa_verify_hash_interruptible_operation_t(
    uint8_t** pos,
    size_t* remaining,
    psa_verify_hash_interruptible_operation_t** operation) {
    psasim_operation_t client_operation;
...
int psasim_serialise_mbedtls_svc_key_id_t(uint8_t** pos, size_t* remaining, mbedtls_svc_key_id_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_deserialise_mbedtls_svc_key_id_t(uint8_t** pos, size_t* remaining, mbedtls_svc_key_id_t* value) {
    if (*remaining < sizeof(*value)) {
        return 0;
    }
...
size_t psasim_serialise_psa_key_agreement_iop_t_needs(psa_key_agreement_iop_t value) {
    return sizeof(value);
}

int psasim_serialise_psa_key_agreement_iop_t(uint8_t** pos, size_t* remaining, psa_key_agreement_iop_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_deserialise_psa_key_agreement_iop_t(uint8_t** pos, size_t* remaining, psa_key_agreement_iop_t* value) {
    if (*remaining < sizeof(*value)) {
        return 0;
    }
...
size_t psasim_serialise_psa_generate_key_iop_t_needs(psa_generate_key_iop_t value) {
    return sizeof(value);
}

int psasim_serialise_psa_generate_key_iop_t(uint8_t** pos, size_t* remaining, psa_generate_key_iop_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
...
int psasim_deserialise_psa_generate_key_iop_t(uint8_t** pos, size_t* remaining, psa_generate_key_iop_t* value) {
    if (*remaining < sizeof(*value)) {
        return 0;
    }
...
size_t psasim_serialise_psa_export_public_key_iop_t_needs(psa_export_public_key_iop_t value) {
    return sizeof(value);
}

int psasim_serialise_psa_export_public_key_iop_t(uint8_t** pos, size_t* remaining, psa_export_public_key_iop_t value) {
    if (*remaining < sizeof(value)) {
        return 0;
    }
