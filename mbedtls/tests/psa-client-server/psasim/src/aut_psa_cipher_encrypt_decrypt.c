...
int psa_cipher_encrypt_decrypt_main(void) {
    ...
    const uint8_t key_bytes[32] MBEDTLS_ATTRIBUTE_UNTERMINATED_STRING = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    ...
    status = psa_cipher_encrypt(key_id, PSA_ALG_ECB_NO_PADDING, original, sizeof(original), encrypt, sizeof(encrypt),
                                &encrypted_length);
    if (status != PSA_SUCCESS) {
    ...
    status = psa_cipher_decrypt(key_id, PSA_ALG_ECB_NO_PADDING, encrypt, encrypted_length, decrypt, sizeof(decrypt),
                                &decrypted_length);
    if (status != PSA_SUCCESS) {
