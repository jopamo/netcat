...
#define PSA_TO_MBEDTLS_ERR(status) PSA_TO_MBEDTLS_ERR_LIST(status, psa_to_ssl_errors, psa_generic_status_to_mbedtls)
...
#if defined(MBEDTLS_TEST_HAS_TLS1_3_AES_256_GCM_SHA384) || defined(MBEDTLS_TEST_HAS_TLS1_3_AES_128_GCM_SHA256) ||   \
...
enum {
#define MBEDTLS_SSL_TLS1_3_LABEL(name, string) tls13_label_##name,
...
#define ECJPAKE_TEST_SET_PASSWORD(exp_ret_val)                                               \
    ret = (use_opaque_arg) ? mbedtls_ssl_set_hs_ecjpake_password_opaque(&ssl, pwd_slot)      \
...
#define TEST_AVAILABLE_ECC(tls_id_, group_id_, psa_family_, psa_bits_)                                  \
    TEST_EQUAL(mbedtls_ssl_get_ecp_group_id_from_tls_id(tls_id_), group_id_);                           \
    TEST_EQUAL(mbedtls_ssl_get_tls_id_from_ecp_group_id(group_id_), tls_id_);                           \
    TEST_EQUAL(mbedtls_ssl_get_psa_curve_info_from_tls_id(tls_id_, &psa_type, &psa_bits), PSA_SUCCESS); \
...
#define TEST_UNAVAILABLE_ECC(tls_id_, group_id_, psa_family_, psa_bits_)                \
    TEST_EQUAL(mbedtls_ssl_get_ecp_group_id_from_tls_id(tls_id_), MBEDTLS_ECP_DP_NONE); \
    TEST_EQUAL(mbedtls_ssl_get_tls_id_from_ecp_group_id(group_id_), 0);                 \
    TEST_EQUAL(mbedtls_ssl_get_psa_curve_info_from_tls_id(tls_id_, &psa_type, &psa_bits), PSA_ERROR_NOT_SUPPORTED);
