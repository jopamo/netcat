...
static int pkcs7_get_next_content_len(unsigned char** p, unsigned char* end, size_t* len) {
    ...
    if (ret != 0) {
        ...
        ret = MBEDTLS_ERROR_ADD(MBEDTLS_ERR_PKCS7_INVALID_CONTENT_INFO, MBEDTLS_ERR_ASN1_LENGTH_MISMATCH);
    }
 ...
 **/
static int pkcs7_get_content_info_type(unsigned char** p,
                                       unsigned char* end,
...
static int pkcs7_get_certificates(unsigned char** p, unsigned char* end, mbedtls_x509_crt* certs) {
    ...
    ret = mbedtls_asn1_get_tag(p, end, &len1, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_CONTEXT_SPECIFIC);
 ...
 **/
static int pkcs7_get_signer_info(unsigned char** p,
                                 unsigned char* end,
    ...
    asn1_ret = mbedtls_asn1_get_tag(p, end_signer, &len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
    if (asn1_ret != 0) {
        goto out;
    }
    ...
    asn1_ret = mbedtls_asn1_get_tag(p, end_issuer_and_sn, &len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
    if (asn1_ret != 0) {
        goto out;
    }
 ...
 * Return negative error code for failure.
 **/
static int pkcs7_get_signers_info_set(unsigned char** p,
                                      unsigned char* end,
 ...
 */
static int pkcs7_get_signed_data(unsigned char* buf, size_t buflen, mbedtls_pkcs7_signed_data* signed_data) {
    unsigned char* p = buf;
    ...
    if (p + len != end) {
        return MBEDTLS_ERROR_ADD(MBEDTLS_ERR_PKCS7_INVALID_FORMAT, MBEDTLS_ERR_ASN1_LENGTH_MISMATCH);
    }
    ...
    if (p != end_content_info) {
        ...
        ret =
            mbedtls_asn1_get_tag(&p, end_content_info, &len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_CONTEXT_SPECIFIC);
        if (ret != 0) {
            return MBEDTLS_ERROR_ADD(MBEDTLS_ERR_PKCS7_INVALID_CONTENT_INFO, ret);
        }
        p += len;
...
static int mbedtls_pkcs7_data_or_hash_verify(mbedtls_pkcs7* pkcs7,
    ...
    if (mbedtls_x509_time_is_past(&cert->valid_to) || mbedtls_x509_time_is_future(&cert->valid_from)) {
    ...
    for (signer = &pkcs7->signed_data.signers; signer; signer = signer->next) {
        ret = mbedtls_pk_verify_ext(cert->sig_pk, &pk_cxt, md_alg, hash, mbedtls_md_get_size(md_info), signer->sig.p,
                                    signer->sig.len);
