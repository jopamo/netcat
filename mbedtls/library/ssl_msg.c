...
#define PSA_CHK(func_call)         \
    ...
    for (offset = min_data_len; offset <= max_data_len; offset++) {
        ...
        mbedtls_ct_memcpy_if(mbedtls_ct_uint_eq(offset, data_len_secret), output, aux_out, NULL, hash_size);
...
static int ssl_parse_record_header(mbedtls_ssl_context const* ssl, unsigned char* buf, size_t len, mbedtls_record* rec);
...
static int ssl_double_retransmit_timeout(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(3,
                          ("update timeout value to %lu millisecs", (unsigned long)ssl->handshake->retransmit_timeout));
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_inner_plaintext(unsigned char const* content, size_t* content_size, uint8_t* rec_type) {
...
int mbedtls_ssl_encrypt_buf(mbedtls_ssl_context* ssl, mbedtls_ssl_transform* transform, mbedtls_record* rec) {
    ...
    if (rec->data_len > MBEDTLS_SSL_OUT_CONTENT_LEN) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("Record content %" MBEDTLS_PRINTF_SIZET " too large, maximum %" MBEDTLS_PRINTF_SIZET,
    ...
    if (transform->tls_version == MBEDTLS_SSL_VERSION_TLS1_3) {
        size_t padding = ssl_compute_padding_length(rec->data_len, MBEDTLS_SSL_CID_TLS1_3_PADDING_GRANULARITY);
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_STREAM || ssl_mode == MBEDTLS_SSL_MODE_CBC) {
        ...
        ssl_extract_add_data_from_record(add_data, &add_data_len, rec, transform->tls_version, transform->taglen);
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_STREAM) {
        ...
        ssl_build_record_nonce(iv, sizeof(iv), transform->iv_enc, transform->fixed_ivlen, dynamic_iv, dynamic_iv_len);
        ...
        MBEDTLS_SSL_DEBUG_BUF(4, "IV used (transmitted)", dynamic_iv, dynamic_iv_is_explicit ? dynamic_iv_len : 0);
        ...
        status =
            psa_aead_encrypt(transform->psa_key_enc, transform->psa_alg, iv, transform->ivlen, add_data, add_data_len,
                             data, rec->data_len, data, rec->buf_len - (data - rec->buf), &rec->data_len);
        ...
        MBEDTLS_SSL_DEBUG_BUF(4, "after encrypt: tag", data + rec->data_len - transform->taglen, transform->taglen);
        ...
        MBEDTLS_SSL_DEBUG_MSG(
            3, ("before encrypt: msglen = %" MBEDTLS_PRINTF_SIZET ", "
                "including %" MBEDTLS_PRINTF_SIZET " bytes of IV and %" MBEDTLS_PRINTF_SIZET " bytes of padding",
...
int mbedtls_ssl_decrypt_buf(mbedtls_ssl_context const* ssl, mbedtls_ssl_transform* transform, mbedtls_record* rec) {
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_STREAM) {
        ...
        if (ssl_transform_aead_dynamic_iv_is_explicit(transform) == 1) {
            if (rec->data_len < dynamic_iv_len) {
                MBEDTLS_SSL_DEBUG_MSG(
                    1, ("msglen (%" MBEDTLS_PRINTF_SIZET " ) < explicit_iv_len (%" MBEDTLS_PRINTF_SIZET ") ",
                        rec->data_len, dynamic_iv_len));
                return MBEDTLS_ERR_SSL_INVALID_MAC;
            }
            dynamic_iv = data;
        ...
        if (rec->data_len < transform->taglen) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") < taglen (%" MBEDTLS_PRINTF_SIZET ") ",
         ...
         */
        ssl_build_record_nonce(iv, sizeof(iv), transform->iv_dec, transform->fixed_ivlen, dynamic_iv, dynamic_iv_len);
         ...
         */
        status =
            psa_aead_decrypt(transform->psa_key_dec, transform->psa_alg, iv, transform->ivlen, add_data, add_data_len,
                             data, rec->data_len + transform->taglen, data, rec->buf_len - (data - rec->buf), &olen);
        ...
        if (rec->data_len < minlen + transform->ivlen || rec->data_len < minlen + transform->maclen + 1) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") < max( ivlen(%" MBEDTLS_PRINTF_SIZET
        ...
        if (ssl_mode == MBEDTLS_SSL_MODE_CBC_ETM) {
            ...
            MBEDTLS_SSL_DEBUG_BUF(4, "MAC'd meta-data", add_data, add_data_len);
            status = psa_mac_verify_setup(&operation, transform->psa_mac_dec, transform->psa_mac_alg);
            if (status != PSA_SUCCESS) {
                goto hmac_failed_etm_enabled;
            }
        ...
        if (rec->data_len % transform->ivlen != 0) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") %% ivlen (%" MBEDTLS_PRINTF_SIZET ") != 0",
                                      rec->data_len, transform->ivlen));
            return MBEDTLS_ERR_SSL_INVALID_MAC;
        }
        ...
        if (auth_done == 1) {
            ...
            if (rec->data_len < transform->maclen + padlen + 1) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") < maclen (%" MBEDTLS_PRINTF_SIZET
                                          ") + padlen (%" MBEDTLS_PRINTF_SIZET ")",
                                          rec->data_len, transform->maclen, padlen + 1));
            }
#endif
            const mbedtls_ct_condition_t ge = mbedtls_ct_uint_ge(rec->data_len, transform->maclen + padlen + 1);
            correct = mbedtls_ct_bool_and(ge, correct);
            padlen = mbedtls_ct_size_if_else_0(ge, padlen);
        }
    ...
    if (auth_done == 0) {
        ...
        ret = mbedtls_ct_hmac(transform->psa_mac_dec, transform->psa_mac_alg, add_data, add_data_len, data,
                              rec->data_len, min_len, max_len, mac_expect);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ct_hmac", ret);
            goto hmac_failed_etm_disabled;
        }

        mbedtls_ct_memcpy_offset(mac_peer, data, rec->data_len, min_len, max_len, transform->maclen);
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
...
int mbedtls_ssl_fetch_input(mbedtls_ssl_context* ssl, size_t nb_want) {
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (ssl->next_record_offset != 0) {
            ...
            if (ssl->in_left != 0) {
                MBEDTLS_SSL_DEBUG_MSG(
                    2, ("next record in same datagram, offset: %" MBEDTLS_PRINTF_SIZET, ssl->next_record_offset));
        ...
        MBEDTLS_SSL_DEBUG_MSG(
            2, ("in_left: %" MBEDTLS_PRINTF_SIZET ", nb_want: %" MBEDTLS_PRINTF_SIZET, ssl->in_left, nb_want));
...
#endif
    {
        ...
        while (ssl->in_left < nb_want) {
            ...
            if ((size_t)ret > len) {
                MBEDTLS_SSL_DEBUG_MSG(
                    1, ("f_recv returned %d bytes but only %" MBEDTLS_PRINTF_SIZET " were requested", ret, len));
                return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
            }
...
int mbedtls_ssl_flush_output(mbedtls_ssl_context* ssl) {
    ...
    while (ssl->out_left > 0) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("message length: %" MBEDTLS_PRINTF_SIZET ", out_left: %" MBEDTLS_PRINTF_SIZET,
        ...
        if ((size_t)ret > ssl->out_left) {
            MBEDTLS_SSL_DEBUG_MSG(
                1, ("f_send returned %d bytes but only %" MBEDTLS_PRINTF_SIZET " bytes were sent", ret, ssl->out_left));
            return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
        }
...
static int ssl_flight_append(mbedtls_ssl_context* ssl) {
    ...
    if ((msg = mbedtls_calloc(1, sizeof(mbedtls_ssl_flight_item))) == NULL) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("alloc %" MBEDTLS_PRINTF_SIZET " bytes failed", sizeof(mbedtls_ssl_flight_item)));
...
int mbedtls_ssl_flight_transmit(mbedtls_ssl_context* ssl) {
    ...
    while (ssl->handshake->cur_msg != NULL) {
        ...
        int const force_flush = ssl->disable_datagram_packing == 1 ? SSL_FORCE_FLUSH : SSL_DONT_FORCE_FLUSH;
        ...
        if (cur->type == MBEDTLS_SSL_MSG_CHANGE_CIPHER_SPEC) {
            ...
            if (frag_off == 0 && cur_hs_frag_len != hs_len) {
                MBEDTLS_SSL_DEBUG_MSG(2, ("fragmenting handshake message (%u > %u)", (unsigned)cur_hs_frag_len,
...
int mbedtls_ssl_prepare_handshake_record(mbedtls_ssl_context* ssl) {
    if (ssl->in_hsfraglen == 0) {
        ...
        if (ssl->in_msglen < mbedtls_ssl_hs_hdr_len(ssl)) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("handshake message too short: %" MBEDTLS_PRINTF_SIZET, ssl->in_msglen));
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }
    ...
    if (ssl->transform_in != NULL) {
        MBEDTLS_SSL_DEBUG_MSG(
            4, ("decrypted handshake message:"
                " iv-buf=%d hdr-buf=%d hdr-buf=%d",
                (int)(ssl->in_iv - ssl->in_buf), (int)(ssl->in_hdr - ssl->in_buf), (int)(ssl->in_msg - ssl->in_buf)));
    }
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (ssl->handshake != NULL &&
            ((mbedtls_ssl_is_handshake_over(ssl) == 0 && recv_msg_seq != ssl->handshake->in_msg_seq) ||
             (mbedtls_ssl_is_handshake_over(ssl) == 1 && ssl->in_msg[0] != MBEDTLS_SSL_HS_CLIENT_HELLO))) {
...
#endif /* MBEDTLS_SSL_PROTO_DTLS */
    {
        unsigned char* const reassembled_record_start = ssl->in_buf + MBEDTLS_SSL_SEQUENCE_NUMBER_LEN;
        ...
        MBEDTLS_SSL_DEBUG_MSG(
            3, ("%s handshake fragment: %" MBEDTLS_PRINTF_SIZET ", %" MBEDTLS_PRINTF_SIZET "..%" MBEDTLS_PRINTF_SIZET
                " of %" MBEDTLS_PRINTF_SIZET,
                (ssl->in_hsfraglen != 0                  ? "subsequent"
                 : hs_this_fragment_len == ssl->in_hslen ? "sole"
                                                         : "initial"),
                ssl->in_msglen, ssl->in_hsfraglen, ssl->in_hsfraglen + hs_this_fragment_len, ssl->in_hslen));
        ...
        if (payload_end + ssl->in_msglen > ssl->in_buf + in_buf_len) {
            MBEDTLS_SSL_DEBUG_MSG(
                1, ("Shouldn't happen: no room to move handshake fragment %" MBEDTLS_PRINTF_SIZET
                    " from %p to %p (buf=%p len=%" MBEDTLS_PRINTF_SIZET ")",
                    ssl->in_msglen, (void*)ssl->in_msg, (void*)payload_end, (void*)ssl->in_buf, in_buf_len));
        ...
        if (ssl->in_hsfraglen < ssl->in_hslen) {
            MBEDTLS_SSL_DEBUG_MSG(
                3, ("Prepare: waiting for more handshake fragments %" MBEDTLS_PRINTF_SIZET "/%" MBEDTLS_PRINTF_SIZET,
            ...
            if (ssl->in_msglen > 0xffff) {
                MBEDTLS_SSL_DEBUG_MSG(
                    1, ("Shouldn't happen: in_msglen=%" MBEDTLS_PRINTF_SIZET " > 0xffff", ssl->in_msglen));
                return MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
            }
...
static inline uint64_t ssl_load_six_bytes(unsigned char* buf) {
    return ((uint64_t)buf[0] << 40) | ((uint64_t)buf[1] << 32) | ((uint64_t)buf[2] << 24) | ((uint64_t)buf[3] << 16) |
...
int mbedtls_ssl_check_dtls_clihlo_cookie(mbedtls_ssl_context* ssl,
    ...
    if (in[0] != MBEDTLS_SSL_MSG_HANDSHAKE || epoch != 0 || fragment_offset != 0) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(
            4, ("    type=%u epoch=%u fragment_offset=%u", in[0], (unsigned)epoch, (unsigned)fragment_offset));
        return MBEDTLS_ERR_SSL_DECODE_ERROR;
    }

    sid_len = in[59];
    if (59 + 1 + sid_len + 1 > in_len) {
        MBEDTLS_SSL_DEBUG_MSG(4, ("check cookie: sid_len=%u > %u", (unsigned)sid_len, (unsigned)in_len - 61));
        return MBEDTLS_ERR_SSL_DECODE_ERROR;
    }
    ...
    if (59 + 1 + sid_len + 1 + cookie_len > in_len) {
        MBEDTLS_SSL_DEBUG_MSG(
            4, ("check cookie: cookie_len=%u > %u", (unsigned)cookie_len, (unsigned)(in_len - sid_len - 61)));
        return MBEDTLS_ERR_SSL_DECODE_ERROR;
    }
...
static int ssl_handle_possible_reconnect(mbedtls_ssl_context* ssl) {
    ...
    ret = mbedtls_ssl_check_dtls_clihlo_cookie(ssl, ssl->cli_id, ssl->cli_id_len, ssl->in_buf, ssl->in_left,
...
static int ssl_check_record_type(uint8_t record_type) {
    if (record_type != MBEDTLS_SSL_MSG_HANDSHAKE && record_type != MBEDTLS_SSL_MSG_ALERT &&
        record_type != MBEDTLS_SSL_MSG_CHANGE_CIPHER_SPEC && record_type != MBEDTLS_SSL_MSG_APPLICATION_DATA) {
        return MBEDTLS_ERR_SSL_INVALID_RECORD;
    }
...
static int ssl_parse_record_header(mbedtls_ssl_context const* ssl,
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM && ssl->conf->cid_len != 0 &&
        ...
        if (len < rec_hdr_len_offset + rec_hdr_len_len) {
            MBEDTLS_SSL_DEBUG_MSG(
                1, ("datagram of length %u too small to hold DTLS record header including CID, length %u",
    ...
    tls_version =
        (mbedtls_ssl_protocol_version)mbedtls_ssl_read_version(buf + rec_hdr_version_offset, ssl->conf->transport);

    if (tls_version > ssl->conf->max_tls_version) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("TLS version mismatch: got %u, expected max %u", (unsigned)tls_version,
...
static int ssl_check_client_reconnect(mbedtls_ssl_context* ssl) {
     ...
     */
    if (rec_epoch == 0 && ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER && mbedtls_ssl_is_handshake_over(ssl) == 1 &&
...
static int ssl_prepare_record_content(mbedtls_ssl_context* ssl, mbedtls_record* rec) {
    ...
    if (!done && ssl->transform_in != NULL) {
        ...
        if ((ret = mbedtls_ssl_decrypt_buf(ssl, ssl->transform_in, rec)) != 0) {
            ...
            if ((old_msg_type == MBEDTLS_SSL_MSG_APPLICATION_DATA) &&
                (ssl->discard_early_data_record == MBEDTLS_SSL_EARLY_DATA_TRY_TO_DEPROTECT_AND_DISCARD)) {
        ...
        MBEDTLS_SSL_DEBUG_BUF(4, "input payload after decrypt", rec->buf + rec->data_offset, rec->data_len);
        ...
        if (rec->data_len == 0) {
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
            if (ssl->tls_version == MBEDTLS_SSL_VERSION_TLS1_2 && rec->type != MBEDTLS_SSL_MSG_APPLICATION_DATA) {
    ...
    if (ssl->discard_early_data_record == MBEDTLS_SSL_EARLY_DATA_DISCARD) {
        if (rec->type == MBEDTLS_SSL_MSG_APPLICATION_DATA) {
            ...
            MBEDTLS_SSL_DEBUG_MSG(3, ("EarlyData: Ignore application message before 2nd ClientHello"));
...
static int ssl_load_buffered_message(mbedtls_ssl_context* ssl) {
        ...
        for (offset = 1; offset < MBEDTLS_SSL_MAX_BUFFERED_HS; offset++) {
            ...
            if (hs_buf->is_valid == 1) {
                MBEDTLS_SSL_DEBUG_MSG(2, ("Future message with sequence number %u %s buffered.",
                                          hs->in_msg_seq + offset, hs_buf->is_complete ? "fully" : "partially"));
            }
        }
    }
    ...
    if ((hs_buf->is_valid == 1) && (hs_buf->is_complete == 1)) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(2, ("Next handshake message %u not or only partially buffered", hs->in_msg_seq));
    }
    ...
    switch (ssl->in_msgtype) {
        ...
        case MBEDTLS_SSL_MSG_HANDSHAKE: {
            ...
            if (recv_msg_seq_offset >= MBEDTLS_SSL_MAX_BUFFERED_HS) {
                ...
                MBEDTLS_SSL_DEBUG_MSG(2, ("Ignore future HS message with sequence number %u, "
            ...
            if (!hs_buf->is_valid) {
                ...
                if (reassembly_buf_sz > (MBEDTLS_SSL_DTLS_MAX_BUFFERING - hs->buffering.total_bytes_buffered)) {
                    if (recv_msg_seq_offset > 0) {
                        ...
                        MBEDTLS_SSL_DEBUG_MSG(
                            2, ("Buffering of future message of size %" MBEDTLS_PRINTF_SIZET
                                " would exceed the compile-time limit %" MBEDTLS_PRINTF_SIZET
                                " (already %" MBEDTLS_PRINTF_SIZET " bytes buffered) -- ignore\n",
                                msg_len, (size_t)MBEDTLS_SSL_DTLS_MAX_BUFFERING, hs->buffering.total_bytes_buffered));
                        goto exit;
                    }
                    else {
                        MBEDTLS_SSL_DEBUG_MSG(
                ...
                MBEDTLS_SSL_DEBUG_MSG(2, ("initialize reassembly, total length = %" MBEDTLS_PRINTF_SIZET, msg_len));
            ...
            if (!hs_buf->is_complete) {
                ...
                MBEDTLS_SSL_DEBUG_MSG(
                    2, ("adding fragment, offset = %" MBEDTLS_PRINTF_SIZET ", length = %" MBEDTLS_PRINTF_SIZET,
                        frag_off, frag_len));
...
static int ssl_consume_current_message(mbedtls_ssl_context* ssl) {
    ...
    if (ssl->in_hslen != 0) {
        ...
        if (ssl->in_hsfraglen != 0) {
            ...
            MBEDTLS_SSL_DEBUG_MSG(
                3, ("Consume: waiting for more handshake fragments %" MBEDTLS_PRINTF_SIZET "/%" MBEDTLS_PRINTF_SIZET,
                    ssl->in_hsfraglen, ssl->in_hslen));
            return 0;
        }
...
static int ssl_buffer_future_record(mbedtls_ssl_context* ssl, mbedtls_record const* rec) {
    ...
    if (rec->buf_len > (MBEDTLS_SSL_DTLS_MAX_BUFFERING - hs->buffering.total_bytes_buffered)) {
        MBEDTLS_SSL_DEBUG_MSG(
            2, ("Buffering of future epoch record of size %" MBEDTLS_PRINTF_SIZET
                " would exceed the compile-time limit %" MBEDTLS_PRINTF_SIZET " (already %" MBEDTLS_PRINTF_SIZET
                " bytes buffered) -- ignore\n",
                rec->buf_len, (size_t)MBEDTLS_SSL_DTLS_MAX_BUFFERING, hs->buffering.total_bytes_buffered));
        return 0;
    }
...
int mbedtls_ssl_handle_message_type(mbedtls_ssl_context* ssl) {
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_STREAM && ssl->in_hsfraglen != 0 &&
        ssl->in_msgtype != MBEDTLS_SSL_MSG_HANDSHAKE) {
    ...
    if (ssl->in_msgtype == MBEDTLS_SSL_MSG_CHANGE_CIPHER_SPEC) {
        if (ssl->in_msglen != 1) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("invalid CCS message, len: %" MBEDTLS_PRINTF_SIZET, ssl->in_msglen));
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }
    ...
    if (ssl->in_msgtype == MBEDTLS_SSL_MSG_ALERT) {
        if (ssl->in_msglen != 2) {
            ...
            MBEDTLS_SSL_DEBUG_MSG(1, ("invalid alert message, len: %" MBEDTLS_PRINTF_SIZET, ssl->in_msglen));
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }

        MBEDTLS_SSL_DEBUG_MSG(2, ("got an alert message, type: [%u:%u]", ssl->in_msg[0], ssl->in_msg[1]));
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (ssl->in_msgtype == MBEDTLS_SSL_MSG_APPLICATION_DATA && mbedtls_ssl_is_handshake_over(ssl) == 0
...
int mbedtls_ssl_send_fatal_handshake_failure(mbedtls_ssl_context* ssl) {
    return mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE);
}
...
int mbedtls_ssl_get_record_expansion(const mbedtls_ssl_context* ssl) {
    ...
    if (transform->psa_alg == PSA_ALG_GCM || transform->psa_alg == PSA_ALG_CCM ||
        ...
        transform->psa_alg == PSA_ALG_CHACHA20_POLY1305 || transform->psa_alg == MBEDTLS_SSL_NULL_CIPHER) {
...
static int ssl_check_ctr_renegotiate(mbedtls_ssl_context* ssl) {
    ...
    if (mbedtls_ssl_is_handshake_over(ssl) == 0 || ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_PENDING ||
        ssl->conf->disable_renegotiation == MBEDTLS_SSL_RENEGOTIATION_DISABLED) {
        return 0;
    }

    in_ctr_cmp =
        memcmp(ssl->in_ctr + ep_len, &ssl->conf->renego_period[ep_len], MBEDTLS_SSL_SEQUENCE_NUMBER_LEN - ep_len);
    out_ctr_cmp =
        memcmp(&ssl->cur_out_ctr[ep_len], &ssl->conf->renego_period[ep_len], sizeof(ssl->cur_out_ctr) - ep_len);
...
static int ssl_tls13_is_new_session_ticket(mbedtls_ssl_context* ssl) {
    if ((ssl->in_hslen == mbedtls_ssl_hs_hdr_len(ssl)) || (ssl->in_msg[0] != MBEDTLS_SSL_HS_NEW_SESSION_TICKET)) {
        return 0;
    }
...
int mbedtls_ssl_write_early_data(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
    ...
    if ((!mbedtls_ssl_conf_is_tls13_enabled(conf)) || (conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) ||
