...
#define SSL_TLS1_3_KEY_SCHEDULE_MAX_HKDF_LABEL_LEN                                                                   \
    SSL_TLS1_3_KEY_SCHEDULE_HKDF_LABEL_LEN(sizeof(tls13_label_prefix) + MBEDTLS_SSL_TLS1_3_HKDF_LABEL_MAX_LABEL_LEN, \
                                           MBEDTLS_SSL_TLS1_3_KEY_SCHEDULE_MAX_CONTEXT_LEN)
...
#error \
    "The desired key length must fit into an uint16 but \
...
static int ssl_tls13_make_traffic_key(psa_algorithm_t hash_alg,
    ...
    ret = mbedtls_ssl_tls13_hkdf_expand_label(hash_alg, secret, secret_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(key), NULL,
                                              0, key, key_len);
    if (ret != 0) {
        return ret;
    }

    ret = mbedtls_ssl_tls13_hkdf_expand_label(hash_alg, secret, secret_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(iv), NULL,
                                              0, iv, iv_len);
    return ret;
}
...
int mbedtls_ssl_tls13_make_traffic_keys(psa_algorithm_t hash_alg,
    ...
    ret = ssl_tls13_make_traffic_key(hash_alg, client_secret, secret_len, keys->client_write_key, key_len,
                                     keys->client_write_iv, iv_len);
    if (ret != 0) {
        return ret;
    }

    ret = ssl_tls13_make_traffic_key(hash_alg, server_secret, secret_len, keys->server_write_key, key_len,
...
int mbedtls_ssl_tls13_derive_secret(psa_algorithm_t hash_alg,
    ...
    return mbedtls_ssl_tls13_hkdf_expand_label(hash_alg, secret, secret_len, label, label_len, hashed_context, ctx_len,
                                               dstbuf, dstbuf_len);
}
...
int mbedtls_ssl_tls13_evolve_secret(psa_algorithm_t hash_alg,
     ...
     * on the old secret. */
    if (secret_old != NULL) {
        ret = mbedtls_ssl_tls13_derive_secret(hash_alg, secret_old, hlen, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(derived),
                                              NULL, 0, /* context */
...
int mbedtls_ssl_tls13_derive_early_secrets(psa_algorithm_t hash_alg,
    ...
    ret = mbedtls_ssl_tls13_derive_secret(
        hash_alg, early_secret, hash_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(c_e_traffic), transcript, transcript_len,
    ...
    ret = mbedtls_ssl_tls13_derive_secret(
        hash_alg, early_secret, hash_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(e_exp_master), transcript, transcript_len,
...
int mbedtls_ssl_tls13_derive_handshake_secrets(psa_algorithm_t hash_alg,
    ...
    ret = mbedtls_ssl_tls13_derive_secret(
        hash_alg, handshake_secret, hash_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(c_hs_traffic), transcript, transcript_len,
    ...
    ret = mbedtls_ssl_tls13_derive_secret(
        hash_alg, handshake_secret, hash_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(s_hs_traffic), transcript, transcript_len,
...
int mbedtls_ssl_tls13_derive_application_secrets(psa_algorithm_t hash_alg,
    ...
    ret = mbedtls_ssl_tls13_derive_secret(
        hash_alg, application_secret, hash_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(c_ap_traffic), transcript,
        transcript_len, MBEDTLS_SSL_TLS1_3_CONTEXT_HASHED, derived->client_application_traffic_secret_N, hash_len);
    if (ret != 0) {
        return ret;
    }

    ret = mbedtls_ssl_tls13_derive_secret(
        hash_alg, application_secret, hash_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(s_ap_traffic), transcript,
        transcript_len, MBEDTLS_SSL_TLS1_3_CONTEXT_HASHED, derived->server_application_traffic_secret_N, hash_len);
    if (ret != 0) {
        return ret;
    }
...
int mbedtls_ssl_tls13_derive_resumption_master_secret(psa_algorithm_t hash_alg,
    ...
    ret = mbedtls_ssl_tls13_derive_secret(
        hash_alg, application_secret, hash_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(res_master), transcript, transcript_len,
...
static int ssl_tls13_key_schedule_stage_application(mbedtls_ssl_context* ssl) {
    ...
    psa_algorithm_t const hash_alg = mbedtls_md_psa_alg_from_type((mbedtls_md_type_t)handshake->ciphersuite_info->mac);

    /*
     * Compute MasterSecret
     */
    ret = mbedtls_ssl_tls13_evolve_secret(hash_alg, handshake->tls13_master_secrets.handshake, NULL, 0,
...
static int ssl_tls13_calc_finished_core(psa_algorithm_t hash_alg,
    ...
    ret = mbedtls_ssl_tls13_hkdf_expand_label(hash_alg, base_key, hash_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(finished),
...
int mbedtls_ssl_tls13_populate_transform(mbedtls_ssl_transform* transform,
    ...
    if (alg != MBEDTLS_SSL_NULL_CIPHER) {
        ...
        if ((status = psa_import_key(&attributes, key_enc, PSA_BITS_TO_BYTES(key_bits), &transform->psa_key_enc)) !=
            PSA_SUCCESS) {
        ...
        if ((status = psa_import_key(&attributes, key_dec, PSA_BITS_TO_BYTES(key_bits), &transform->psa_key_dec)) !=
            PSA_SUCCESS) {
            MBEDTLS_SSL_DEBUG_RET(1, "psa_import_key", PSA_TO_MBEDTLS_ERR(status));
            return PSA_TO_MBEDTLS_ERR(status);
        }
...
static int ssl_tls13_get_cipher_key_info(const mbedtls_ssl_ciphersuite_t* ciphersuite_info,
    ...
    status =
        mbedtls_ssl_cipher_to_psa((mbedtls_cipher_type_t)ciphersuite_info->cipher, taglen, &alg, &key_type, &key_bits);
    if (status != PSA_SUCCESS) {
        return PSA_TO_MBEDTLS_ERR(status);
    }
...
static int ssl_tls13_generate_early_key(mbedtls_ssl_context* ssl, mbedtls_ssl_key_set* traffic_keys) {
    ...
    ret = mbedtls_ssl_tls13_derive_early_secrets(hash_alg, handshake->tls13_master_secrets.early, transcript,
                                                 transcript_len, &tls13_early_secrets);
    if (ret != 0) {
    ...
    ret = ssl_tls13_make_traffic_key(hash_alg, tls13_early_secrets.client_early_traffic_secret, hash_len,
                                     traffic_keys->client_write_key, key_len, traffic_keys->client_write_iv, iv_len);
    if (ret != 0) {
...
int mbedtls_ssl_tls13_key_schedule_stage_early(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "mbedtls_ssl_tls13_key_schedule_stage_early", handshake->tls13_master_secrets.early,
...
static int ssl_tls13_generate_handshake_keys(mbedtls_ssl_context* ssl, mbedtls_ssl_key_set* traffic_keys) {
    ...
    ret = mbedtls_ssl_tls13_derive_handshake_secrets(hash_alg, handshake->tls13_master_secrets.handshake, transcript,
                                                     transcript_len, tls13_hs_secrets);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ssl_tls13_derive_handshake_secrets", ret);
        return ret;
    }

    MBEDTLS_SSL_DEBUG_BUF(4, "Client handshake traffic secret", tls13_hs_secrets->client_handshake_traffic_secret,
                          hash_len);
    MBEDTLS_SSL_DEBUG_BUF(4, "Server handshake traffic secret", tls13_hs_secrets->server_handshake_traffic_secret,
                          hash_len);
     ...
     */
    if (ssl->f_export_keys != NULL) {
        ssl->f_export_keys(ssl->p_export_keys, MBEDTLS_SSL_KEY_EXPORT_TLS1_3_CLIENT_HANDSHAKE_TRAFFIC_SECRET,
                           tls13_hs_secrets->client_handshake_traffic_secret, hash_len, handshake->randbytes,
                           handshake->randbytes + MBEDTLS_CLIENT_HELLO_RANDOM_LEN,
                           MBEDTLS_SSL_TLS_PRF_NONE /* TODO: FIX! */);

        ssl->f_export_keys(ssl->p_export_keys, MBEDTLS_SSL_KEY_EXPORT_TLS1_3_SERVER_HANDSHAKE_TRAFFIC_SECRET,
                           tls13_hs_secrets->server_handshake_traffic_secret, hash_len, handshake->randbytes,
                           handshake->randbytes + MBEDTLS_CLIENT_HELLO_RANDOM_LEN,
                           MBEDTLS_SSL_TLS_PRF_NONE /* TODO: FIX! */);
    }

    ret = mbedtls_ssl_tls13_make_traffic_keys(hash_alg, tls13_hs_secrets->client_handshake_traffic_secret,
                                              tls13_hs_secrets->server_handshake_traffic_secret, hash_len, key_len,
                                              iv_len, traffic_keys);
    if (ret != 0) {
...
static int ssl_tls13_key_schedule_stage_handshake(mbedtls_ssl_context* ssl) {
    ...
    if (mbedtls_ssl_tls13_key_exchange_mode_with_ephemeral(ssl)) {
        if (mbedtls_ssl_tls13_named_group_is_ecdhe(handshake->offered_group_id) ||
            ...
            psa_algorithm_t alg =
                mbedtls_ssl_tls13_named_group_is_ecdhe(handshake->offered_group_id) ? PSA_ALG_ECDH : PSA_ALG_FFDH;
            ...
            status = psa_raw_key_agreement(alg, handshake->xxdh_psa_privkey, handshake->xxdh_psa_peerkey,
                                           handshake->xxdh_psa_peerkey_len, shared_secret, shared_secret_len,
                                           &shared_secret_len);
            if (status != PSA_SUCCESS) {
                ret = PSA_TO_MBEDTLS_ERR(status);
     ...
     */
    ret = mbedtls_ssl_tls13_evolve_secret(hash_alg, handshake->tls13_master_secrets.early, shared_secret,
                                          shared_secret_len, handshake->tls13_master_secrets.handshake);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ssl_tls13_evolve_secret", ret);
        goto cleanup;
    }
...
static int ssl_tls13_generate_application_keys(mbedtls_ssl_context* ssl, mbedtls_ssl_key_set* traffic_keys) {
    ...
    ret = mbedtls_ssl_tls13_derive_application_secrets(hash_alg, handshake->tls13_master_secrets.app, transcript,
                                                       transcript_len, app_secrets);
    if (ret != 0) {
    ...
    ret = mbedtls_ssl_tls13_make_traffic_keys(hash_alg, app_secrets->client_application_traffic_secret_N,
                                              app_secrets->server_application_traffic_secret_N, hash_len, key_len,
                                              iv_len, traffic_keys);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ssl_tls13_make_traffic_keys", ret);
        goto cleanup;
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "Client application traffic secret", app_secrets->client_application_traffic_secret_N,
                          hash_len);

    MBEDTLS_SSL_DEBUG_BUF(4, "Server application traffic secret", app_secrets->server_application_traffic_secret_N,
                          hash_len);
...
int mbedtls_ssl_tls13_compute_resumption_master_secret(mbedtls_ssl_context* ssl) {
    ...
    ret = mbedtls_ssl_tls13_derive_resumption_master_secret(mbedtls_md_psa_alg_from_type(md_type),
                                                            handshake->tls13_master_secrets.app, transcript,
                                                            transcript_len, &ssl->session_negotiate->app_secrets);
    if (ret != 0) {
        return ret;
    }
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "Resumption master secret", ssl->session_negotiate->app_secrets.resumption_master_secret,
...
int mbedtls_ssl_tls13_exporter(const psa_algorithm_t hash_alg,
    ...
    ret =
        mbedtls_ssl_tls13_derive_secret(hash_alg, hkdf_secret, hash_len, MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(exporter),
                                        context_value, context_len, MBEDTLS_SSL_TLS1_3_CONTEXT_UNHASHED, out, out_len);
