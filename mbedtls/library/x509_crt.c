 ...
 */
static int x509_profile_check_pk_alg(const mbedtls_x509_crt_profile* profile, mbedtls_pk_sigalg_t pk_alg) {
 ...
 */
static int x509_get_subject_key_id(unsigned char** p, const unsigned char* end, mbedtls_x509_buf* subject_key_id) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len = 0u;
...
static int x509_crt_parse_der_core(mbedtls_x509_crt* crt,
    ...
    if ((ret = mbedtls_x509_get_sig_alg(&crt->sig_oid, &sig_params1, &crt->sig_md, &crt->sig_pk)) != 0) {
        mbedtls_x509_crt_free(crt);
        return ret;
    }
...
int mbedtls_x509_crt_info(char* buf, size_t size, const char* prefix, const mbedtls_x509_crt* crt) {
    ...
    ret = mbedtls_snprintf(p, n, "\n%s%-" MBEDTLS_BEFORE_COLON_STR "s: %d bits", prefix, key_size_str,
...
static int x509_crt_verifycrl(mbedtls_x509_crt* crt,
    ...
    while (crl_list != NULL) {
        ...
        if (mbedtls_pk_verify_ext(crl_list->sig_pk, &ca->pk, crl_list->sig_md, hash, hash_length, crl_list->sig.p,
                                  crl_list->sig.len) != 0) {
            flags |= MBEDTLS_X509_BADCRL_NOT_TRUSTED;
            break;
        }
...
static int x509_crt_check_signature(const mbedtls_x509_crt* child,
    ...
    if (!mbedtls_pk_can_do_psa(&parent->pk, mbedtls_psa_alg_from_pk_sigalg(child->sig_pk, hash_alg),
    ...
    return mbedtls_pk_verify_ext(child->sig_pk, &parent->pk, child->sig_md, hash, hash_len, child->sig.p,
                                 child->sig.len);
}
