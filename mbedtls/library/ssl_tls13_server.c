...
static const mbedtls_ssl_ciphersuite_t* ssl_tls13_validate_peer_ciphersuite(mbedtls_ssl_context* ssl,
    ...
    if ((mbedtls_ssl_validate_ciphersuite(ssl, ciphersuite_info, ssl->tls_version, ssl->tls_version) != 0)) {
        return NULL;
    }
...
static int ssl_tls13_offered_psks_check_identity_match_ticket(mbedtls_ssl_context* ssl,
    ...
    if (server_age > MBEDTLS_SSL_TLS1_3_MAX_ALLOWED_TICKET_LIFETIME * 1000) {
        MBEDTLS_SSL_DEBUG_MSG(3, ("Ticket age exceeds limitation ticket_age = %" MBEDTLS_PRINTF_MS_TIME, server_age));
        goto exit;
    }
    ...
    if (age_diff < -MBEDTLS_SSL_TLS1_3_TICKET_AGE_TOLERANCE || age_diff > MBEDTLS_SSL_TLS1_3_TICKET_AGE_TOLERANCE) {
        MBEDTLS_SSL_DEBUG_MSG(3,
                              ("Ticket age outside tolerance window ( diff = %" MBEDTLS_PRINTF_MS_TIME ")", age_diff));
        goto exit;
    }
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    ret =
        ssl_tls13_offered_psks_check_identity_match_ticket(ssl, identity, identity_len, obfuscated_ticket_age, session);
    if (ret == SSL_TLS1_3_PSK_IDENTITY_MATCH) {
        *psk_type = MBEDTLS_SSL_TLS1_3_PSK_RESUMPTION;
        ...
        MBEDTLS_SSL_DEBUG_BUF(4, "Ticket-resumed PSK:", session->resumption_key, session->resumption_key_len);
        MBEDTLS_SSL_DEBUG_MSG(4, ("ticket: obfuscated_ticket_age: %u", (unsigned)obfuscated_ticket_age));
...
static int ssl_tls13_offered_psks_check_binder_match(mbedtls_ssl_context* ssl,
    ...
    ret = mbedtls_ssl_get_handshake_transcript(ssl, mbedtls_md_type_from_psa_alg(psk_hash_alg), transcript,
                                               sizeof(transcript), &transcript_len);
    if (ret != 0) {
        return ret;
    }
...
static int ssl_tls13_parse_pre_shared_key_ext(mbedtls_ssl_context* ssl,
    ...
    ret = ssl->handshake->update_checksum(ssl, pre_shared_key_ext, identities_end - pre_shared_key_ext);
    if (0 != ret) {
    ...
    while (p_identity_len < identities_end && p_binder_len < binders_end) {
        ...
        ret = ssl_tls13_offered_psks_check_identity_match(ssl, identity, identity_len, obfuscated_ticket_age,
                                                          &psk->type, &session);
        if (ret != SSL_TLS1_3_PSK_IDENTITY_MATCH) {
            continue;
        }
        ...
        switch (psk->type) {
            ...
            case MBEDTLS_SSL_TLS1_3_PSK_RESUMPTION:
                ...
                allowed_key_exchange_modes = session.ticket_flags & MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_ALL;
                break;
#endif
            default:
                return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
        ...
        ssl_tls13_select_ciphersuite(ssl, ciphersuites, ciphersuites_end, psk_ciphersuite_id, psk_hash_alg,
    ...
    ret = ssl->handshake->update_checksum(ssl, identities_end, (size_t)(binders_end - identities_end));
    if (0 != ret) {
        MBEDTLS_SSL_DEBUG_RET(1, ("update_checksum"), ret);
        return ret;
    }
...
static int ssl_tls13_parse_supported_versions_ext(mbedtls_ssl_context* ssl,
    ...
    while (p < versions_end) {
        ...
        if ((MBEDTLS_SSL_VERSION_TLS1_2 == tls_version) && mbedtls_ssl_conf_is_tls12_enabled(ssl->conf)) {
            found_supported_version = 1;
            break;
        }
    }
    ...
    if (!found_supported_version) {
        ...
        MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_PROTOCOL_VERSION, MBEDTLS_ERR_SSL_BAD_PROTOCOL_VERSION);
...
static int ssl_tls13_parse_supported_groups_ext(mbedtls_ssl_context* ssl,
    ...
    while (p < named_group_list_end) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(2,
                              ("got named group: %s(%04x)", mbedtls_ssl_named_group_to_str(named_group), named_group));

        if (!mbedtls_ssl_named_group_is_offered(ssl, named_group) ||
            !mbedtls_ssl_named_group_is_supported(named_group) || ssl->handshake->hrr_selected_group != 0) {
            continue;
        }
...
static int ssl_tls13_parse_key_shares_ext(mbedtls_ssl_context* ssl,
    ...
    while (p < client_shares_end) {
         ...
         */
        if (!mbedtls_ssl_named_group_is_offered(ssl, group) || !mbedtls_ssl_named_group_is_supported(group) ||
            ssl->handshake->offered_group_id != 0) {
            continue;
        }
         ...
         */
        if (mbedtls_ssl_tls13_named_group_is_ecdhe(group) || mbedtls_ssl_tls13_named_group_is_ffdh(group)) {
            MBEDTLS_SSL_DEBUG_MSG(2, ("ECDH/FFDH group: %s (%04x)", mbedtls_ssl_named_group_to_str(group), group));
            ret = mbedtls_ssl_tls13_read_public_xxdhe_share(ssl, key_exchange - 2, key_exchange_len + 2);
            if (ret != 0) {
                return ret;
            }
        }
...
static int ssl_tls13_client_hello_has_exts_for_ephemeral_key_exchange(mbedtls_ssl_context* ssl) {
    return ssl_tls13_client_hello_has_exts(
        ssl, MBEDTLS_SSL_EXT_MASK(SUPPORTED_GROUPS) | MBEDTLS_SSL_EXT_MASK(KEY_SHARE) | MBEDTLS_SSL_EXT_MASK(SIG_ALG));
}
...
static int ssl_tls13_client_hello_has_exts_for_psk_key_exchange(mbedtls_ssl_context* ssl) {
    return ssl_tls13_client_hello_has_exts(
        ssl, MBEDTLS_SSL_EXT_MASK(PRE_SHARED_KEY) | MBEDTLS_SSL_EXT_MASK(PSK_KEY_EXCHANGE_MODES));
}
#endif /* MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_ENABLED */

#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_EPHEMERAL_ENABLED)
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_client_hello_has_exts_for_psk_ephemeral_key_exchange(mbedtls_ssl_context* ssl) {
    return ssl_tls13_client_hello_has_exts(
        ssl, MBEDTLS_SSL_EXT_MASK(SUPPORTED_GROUPS) | MBEDTLS_SSL_EXT_MASK(KEY_SHARE) |
...
static int ssl_tls13_key_exchange_is_psk_ephemeral_available(mbedtls_ssl_context* ssl) {
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_EPHEMERAL_ENABLED)
    return mbedtls_ssl_conf_tls13_is_psk_ephemeral_enabled(ssl) && mbedtls_ssl_tls13_is_psk_ephemeral_supported(ssl) &&
...
static int ssl_tls13_pick_key_cert(mbedtls_ssl_context* ssl) {
    ...
    for (; *sig_alg != MBEDTLS_TLS1_3_SIG_NONE; sig_alg++) {
        ...
        for (key_cert = key_cert_list; key_cert != NULL; key_cert = key_cert->next) {
             ...
             */
            if (mbedtls_x509_crt_check_key_usage(key_cert->cert, MBEDTLS_X509_KU_DIGITAL_SIGNATURE) != 0 ||
            ...
            if (mbedtls_ssl_tls13_check_sig_alg_cert_key_match(*sig_alg, &key_cert->cert->pk) &&
                ...
                mbedtls_pk_can_do_psa(&key_cert->cert->pk, psa_alg, PSA_KEY_USAGE_VERIFY_HASH) == 1) {
                ssl->handshake->key_cert = key_cert;
                MBEDTLS_SSL_DEBUG_MSG(3, ("ssl_tls13_pick_key_cert:"
                                          "selected signature algorithm"
                                          " %s [%04x]",
                                          mbedtls_ssl_sig_alg_to_str(*sig_alg), *sig_alg));
                MBEDTLS_SSL_DEBUG_CRT(3, "selected certificate (chain)", ssl->handshake->key_cert->cert);
                return 0;
            }
        }
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_parse_client_hello(mbedtls_ssl_context* ssl, const unsigned char* buf, const unsigned char* end) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    const unsigned char* p = buf;
     ...
     */
    ret = mbedtls_ssl_tls13_is_supported_versions_ext_present_in_exts(ssl, p + 1 + p[0], end, &supported_versions_data,
                                                                      &supported_versions_data_end);
    if (ret < 0) {
        MBEDTLS_SSL_DEBUG_RET(1, ("mbedtls_ssl_tls13_is_supported_versions_ext_present_in_exts"), ret);
        return ret;
    }
    ...
    if (ret == 1) {
        ret = ssl_tls13_parse_supported_versions_ext(ssl, supported_versions_data, supported_versions_data_end);
        if (ret < 0) {
     ...
     */
    MBEDTLS_SSL_DEBUG_BUF(3, "client hello, list of cipher suites", cipher_suites, cipher_suites_len);

    ssl_tls13_select_ciphersuite(ssl, cipher_suites, cipher_suites_end, 0, PSA_ALG_NONE, &handshake->ciphersuite_info);

    if (handshake->ciphersuite_info == NULL) {
        MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE, MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE);
        return MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
    }
    ssl->session_negotiate->ciphersuite = handshake->ciphersuite_info->id;

    MBEDTLS_SSL_DEBUG_MSG(2, ("selected ciphersuite: %04x - %s", ((unsigned)handshake->ciphersuite_info->id),
    ...
    while (p < extensions_end) {
        ...
        ret =
            mbedtls_ssl_tls13_check_received_extension(ssl, MBEDTLS_SSL_HS_CLIENT_HELLO, extension_type, allowed_exts);
        if (ret != 0) {
            return ret;
        }
        ...
        switch (extension_type) {
            ...
            case MBEDTLS_TLS_EXT_PRE_SHARED_KEY:
                ...
                if ((handshake->received_extensions & MBEDTLS_SSL_EXT_MASK(PSK_KEY_EXCHANGE_MODES)) == 0) {
                    MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER,
                                                 MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER);
                    return MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
                }
    ...
    if (handshake->received_extensions & MBEDTLS_SSL_EXT_MASK(PRE_SHARED_KEY)) {
        ...
        ret = ssl_tls13_parse_pre_shared_key_ext(ssl, pre_shared_key_ext, pre_shared_key_ext_end, cipher_suites,
                                                 cipher_suites_end, &psk);
        if (ret == 0) {
            got_psk = 1;
        }
    ...
    if (got_psk && (psk.key_exchange_mode == MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_EPHEMERAL)) {
        ...
        MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_MISSING_EXTENSION, MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER);
        return MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
    }
    ...
    if (handshake->key_exchange_mode & MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_ALL) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(
            2, ("Select PSK ciphersuite: %04x - %s", ((unsigned)psk.ciphersuite_info->id), psk.ciphersuite_info->name));
...
static int ssl_tls13_check_early_data_requirements(mbedtls_ssl_context* ssl) {
    ...
    if (ssl->conf->early_data_enabled == MBEDTLS_SSL_EARLY_DATA_DISABLED) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("EarlyData: rejected, feature disabled in server configuration."));
        return -1;
    }
...
static int ssl_tls13_postprocess_client_hello(mbedtls_ssl_context* ssl, int hrr_required) {
    ...
    if (ssl->handshake->received_extensions & MBEDTLS_SSL_EXT_MASK(EARLY_DATA)) {
        ssl->handshake->early_data_accepted = (!hrr_required) && (ssl_tls13_check_early_data_requirements(ssl) == 0);
        ...
        if (ssl->handshake->early_data_accepted) {
            ...
            ssl->discard_early_data_record =
                hrr_required ? MBEDTLS_SSL_EARLY_DATA_DISCARD : MBEDTLS_SSL_EARLY_DATA_TRY_TO_DEPROTECT_AND_DISCARD;
        }
    }
#else
    ((void)hrr_required);
...
static int ssl_tls13_process_client_hello(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_tls13_fetch_handshake_msg(ssl, MBEDTLS_SSL_HS_CLIENT_HELLO, &buf, &buflen));
    ...
    MBEDTLS_SSL_PROC_CHK(
        ssl_tls13_postprocess_client_hello(ssl, parse_client_hello_ret == SSL_CLIENT_HELLO_HRR_REQUIRED));
...
static int ssl_tls13_prepare_server_hello(mbedtls_ssl_context* ssl) {
    ...
    if ((ret = psa_generate_random(server_randbytes, MBEDTLS_SERVER_HELLO_RANDOM_LEN)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "psa_generate_random", ret);
        return ret;
    }

    MBEDTLS_SSL_DEBUG_BUF(3, "server hello, random bytes", server_randbytes, MBEDTLS_SERVER_HELLO_RANDOM_LEN);
...
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED)
    if (mbedtls_ssl_tls13_named_group_is_ecdhe(named_group) || mbedtls_ssl_tls13_named_group_is_ffdh(named_group)) {
        ret = mbedtls_ssl_tls13_generate_and_write_xxdh_key_exchange(ssl, named_group, buf, end, out_len);
        if (ret != 0) {
...
static int ssl_tls13_write_key_share_ext(mbedtls_ssl_context* ssl,
    ...
    *out_len = 0;
    ...
    MBEDTLS_SSL_DEBUG_MSG(
        2, ("server hello, write selected_group: %s (%04x)", mbedtls_ssl_named_group_to_str(group), group));
    ...
    ret = ssl_tls13_generate_and_write_key_share(ssl, group, server_share + 4, end, &key_exchange_length);
    if (ret != 0) {
        return ret;
    }
...
static int ssl_tls13_write_hrr_key_share_ext(mbedtls_ssl_context* ssl,
    ...
    *out_len = 0;
    ...
    MBEDTLS_SSL_DEBUG_MSG(
        3, ("HRR selected_group: %s (%x)", mbedtls_ssl_named_group_to_str(selected_group), selected_group));
...
static int ssl_tls13_write_server_hello_body(mbedtls_ssl_context* ssl,
    ...
    *out_len = 0;
    ...
    if (is_hrr) {
        ...
        memcpy(p, &ssl->handshake->randbytes[MBEDTLS_CLIENT_HELLO_RANDOM_LEN], MBEDTLS_SERVER_HELLO_RANDOM_LEN);
    }
    ...
    *p++ = MBEDTLS_SSL_COMPRESS_NULL;
    ...
    if ((ret = ssl_tls13_write_server_hello_supported_versions_ext(ssl, p, end, &output_len)) != 0) {
    ...
    *out_len = p - buf;
    ...
    MBEDTLS_SSL_PRINT_EXTS(3, is_hrr ? MBEDTLS_SSL_TLS1_3_HS_HELLO_RETRY_REQUEST : MBEDTLS_SSL_HS_SERVER_HELLO,
...
static int ssl_tls13_write_server_hello(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_start_handshake_msg(ssl, MBEDTLS_SSL_HS_SERVER_HELLO, &buf, &buf_len));

    MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_server_hello_body(ssl, buf, buf + buf_len, &msg_len, 0));

    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_SERVER_HELLO, buf, msg_len));
...
static int ssl_tls13_write_encrypted_extensions(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_start_handshake_msg(ssl, MBEDTLS_SSL_HS_ENCRYPTED_EXTENSIONS, &buf, &buf_len));

    MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_encrypted_extensions_body(ssl, buf, buf + buf_len, &msg_len));

    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_ENCRYPTED_EXTENSIONS, buf, msg_len));
...
static int ssl_tls13_write_certificate_request(mbedtls_ssl_context* ssl) {
    ...
    if (ret == SSL_CERTIFICATE_REQUEST_SEND_REQUEST) {
        ...
        MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_start_handshake_msg(ssl, MBEDTLS_SSL_HS_CERTIFICATE_REQUEST, &buf, &buf_len));

        MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_certificate_request_body(ssl, buf, buf + buf_len, &msg_len));

        MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_CERTIFICATE_REQUEST, buf, msg_len));
...
static int ssl_tls13_end_of_early_data_coordinate(mbedtls_ssl_context* ssl) {
    ...
    if (ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE && ssl->in_msg[0] == MBEDTLS_SSL_HS_END_OF_EARLY_DATA) {
...
static int ssl_tls13_process_end_of_early_data(mbedtls_ssl_context* ssl) {
    ...
    if (ret == SSL_GOT_END_OF_EARLY_DATA) {
        ...
        MBEDTLS_SSL_PROC_CHK(
            mbedtls_ssl_tls13_fetch_handshake_msg(ssl, MBEDTLS_SSL_HS_END_OF_EARLY_DATA, &buf, &buf_len));
        ...
        MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_END_OF_EARLY_DATA, buf, buf_len));
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS) && defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_PSK_ENABLED)
...
static int ssl_tls13_prepare_new_session_ticket(mbedtls_ssl_context* ssl,
...
#if defined(MBEDTLS_SSL_EARLY_DATA)
    if (ssl->conf->early_data_enabled == MBEDTLS_SSL_EARLY_DATA_ENABLED && ssl->conf->max_early_data_size > 0) {
    ...
    if ((ret = psa_generate_random((unsigned char*)&session->ticket_age_add, sizeof(session->ticket_age_add)) != 0)) {
     ...
     */
    ret = mbedtls_ssl_tls13_hkdf_expand_label(psa_hash_alg, session->app_secrets.resumption_master_secret, hash_length,
                                              MBEDTLS_SSL_TLS1_3_LBL_WITH_LEN(resumption), ticket_nonce,
                                              ticket_nonce_size, session->resumption_key, hash_length);

    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(2, "Creating the ticket-resumed PSK failed", ret);
        return ret;
    }
    MBEDTLS_SSL_DEBUG_BUF(3, "Ticket-resumed PSK", session->resumption_key, session->resumption_key_len);

    MBEDTLS_SSL_DEBUG_BUF(3, "resumption_master_secret", session->app_secrets.resumption_master_secret, hash_length);
...
static int ssl_tls13_write_new_session_ticket_body(mbedtls_ssl_context* ssl,
    ...
    *out_len = 0;
...
#endif
    ret = ssl->conf->f_ticket_write(ssl->conf->p_ticket, session, p + 9 + ticket_nonce_size + 2, end, &ticket_len,
                                    &ticket_lifetime);
    if (ret != 0) {
    ...
    if (ticket_lifetime > MBEDTLS_SSL_TLS1_3_MAX_ALLOWED_TICKET_LIFETIME) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("Ticket lifetime (%u) is greater than 7 days.", (unsigned int)ticket_lifetime));
...
static int ssl_tls13_write_new_session_ticket(mbedtls_ssl_context* ssl) {
    ...
    if (ret == SSL_NEW_SESSION_TICKET_WRITE) {
        ...
        MBEDTLS_SSL_PROC_CHK(ssl_tls13_prepare_new_session_ticket(ssl, ticket_nonce, sizeof(ticket_nonce)));

        MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_start_handshake_msg(ssl, MBEDTLS_SSL_HS_NEW_SESSION_TICKET, &buf, &buf_len));

        MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_new_session_ticket_body(ssl, buf, buf + buf_len, &msg_len, ticket_nonce,
                                                                     sizeof(ticket_nonce)));
...
int mbedtls_ssl_tls13_handshake_server_step(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(
        2, ("tls13 server state: %s(%d)", mbedtls_ssl_states_str((mbedtls_ssl_states)ssl->state), ssl->state));
