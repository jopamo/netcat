...
static int mbedtls_x509write_crt_set_key_identifier(mbedtls_x509write_cert* ctx, int is_ca, unsigned char tag) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    memset(buf, 0, sizeof(buf));
    MBEDTLS_ASN1_CHK_ADD(len, mbedtls_pk_write_pubkey(&c, buf, is_ca ? ctx->issuer_key : ctx->subject_key));

    status = psa_hash_compute(PSA_ALG_SHA_1, buf + sizeof(buf) - len, len, buf + sizeof(buf) - 20, 20, &hash_length);
...
int mbedtls_x509write_crt_set_key_usage(mbedtls_x509write_cert* ctx, unsigned int key_usage) {
    ...
    const unsigned int allowed_bits =
        ...
        MBEDTLS_X509_KU_CRL_SIGN | MBEDTLS_X509_KU_ENCIPHER_ONLY | MBEDTLS_X509_KU_DECIPHER_ONLY;
    ...
    ret = mbedtls_x509write_crt_set_extension(ctx, MBEDTLS_OID_KEY_USAGE, MBEDTLS_OID_SIZE(MBEDTLS_OID_KEY_USAGE), 1, c,
                                              (size_t)ret);
    if (ret != 0) {
        return ret;
    }
...
int mbedtls_x509write_crt_set_ns_cert_type(mbedtls_x509write_cert* ctx, unsigned char ns_cert_type) {
    ...
    ret = mbedtls_x509write_crt_set_extension(ctx, MBEDTLS_OID_NS_CERT_TYPE, MBEDTLS_OID_SIZE(MBEDTLS_OID_NS_CERT_TYPE),
                                              0, c, (size_t)ret);
    if (ret != 0) {
        return ret;
    }
...
int mbedtls_x509write_crt_der(mbedtls_x509write_cert* ctx, unsigned char* buf, size_t size) {
    ...
    if ((ret = mbedtls_x509_oid_get_oid_by_sig_alg((mbedtls_pk_sigalg_t)pk_alg, ctx->md_alg, &sig_oid, &sig_oid_len)) !=
        0) {
        return ret;
    }
     ...
     */
    MBEDTLS_ASN1_CHK_ADD(pub_len, mbedtls_pk_write_pubkey_der(ctx->subject_key, buf, (size_t)(c - buf)));
    c -= pub_len;
    len += pub_len;
    ...
    MBEDTLS_ASN1_CHK_ADD(
        len, mbedtls_asn1_write_algorithm_identifier_ext(&c, buf, sig_oid, strlen(sig_oid), 0, write_sig_null_par));
    ...
    if ((ret = mbedtls_pk_sign_ext(pk_alg, ctx->issuer_key, ctx->md_alg, hash, hash_length, sig, sizeof(sig),
                                   &sig_len)) != 0) {
        return ret;
    }
     ...
     * into the CRT buffer. */
    c2 = buf + size;
    MBEDTLS_ASN1_CHK_ADD(sig_and_oid_len, mbedtls_x509_write_sig(&c2, c, sig_oid, sig_oid_len, sig, sig_len, pk_alg));
