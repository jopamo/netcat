 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_alpn_ext(mbedtls_ssl_context* ssl, unsigned char* buf, const unsigned char* end, size_t* out_len) {
    unsigned char* p = buf;
...
#if defined(MBEDTLS_SSL_TLS1_2_SOME_ECC) || defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED)
/*
    ...
    for (; *group_list != 0; group_list++) {
        ...
        if (propose_group) {
            ...
            MBEDTLS_SSL_DEBUG_MSG(3,
                                  ("NamedGroup: %s ( %x )", mbedtls_ssl_named_group_to_str(*group_list), *group_list));
        }
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_write_client_hello_cipher_suites(mbedtls_ssl_context* ssl,
                                                unsigned char* buf,
                                                unsigned char* end,
                                                int* tls12_uses_ec,
                                                size_t* out_len) {
    ...
    *out_len = 0;
     ...
     defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ALLOWED_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED))
        *tls12_uses_ec |= mbedtls_ssl_ciphersuite_uses_ec(ciphersuite_info);
        ...
        MBEDTLS_SSL_DEBUG_MSG(
            3, ("client hello, add ciphersuite: %04x, %s", (unsigned int)cipher_suite, ciphersuite_info->name));
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, got %" MBEDTLS_PRINTF_SIZET " cipher suites", cipher_suites_len / 2));
...
static int ssl_write_client_hello_body(mbedtls_ssl_context* ssl,
    ...
    *binders_len = 0;
    ...
    unsigned char propose_tls12 =
        (handshake->min_tls_version <= MBEDTLS_SSL_VERSION_TLS1_2) && (MBEDTLS_SSL_VERSION_TLS1_2 <= ssl->tls_version);
#endif
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
    unsigned char propose_tls13 =
        (handshake->min_tls_version <= MBEDTLS_SSL_VERSION_TLS1_3) && (MBEDTLS_SSL_VERSION_TLS1_3 <= ssl->tls_version);
#endif
    ...
    *p++ = MBEDTLS_SSL_COMPRESS_NULL;
...
#if defined(MBEDTLS_SSL_TLS1_2_SOME_ECC) || defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED)
    {
        ...
        if (ssl_write_supported_groups_ext_flags != 0) {
            ret = ssl_write_supported_groups_ext(ssl, p, end, ssl_write_supported_groups_ext_flags, &output_len);
            if (ret != 0) {
                return ret;
            }
            p += output_len;
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
    write_sig_alg_ext = write_sig_alg_ext || (propose_tls13 && mbedtls_ssl_conf_tls13_is_ephemeral_enabled(ssl));
#endif
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
    ...
    if (propose_tls13 && mbedtls_ssl_conf_tls13_is_some_psk_enabled(ssl)) {
        ret = mbedtls_ssl_tls13_write_identities_of_pre_shared_key_ext(ssl, p, end, &output_len, binders_len);
        if (ret != 0) {
            return ret;
        }
        p += output_len;
    ...
    if (extensions_len == 0) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, total extension length: %" MBEDTLS_PRINTF_SIZET, extensions_len));
...
static int ssl_generate_random(mbedtls_ssl_context* ssl) {
    ...
    if (ssl->tls_version == MBEDTLS_SSL_VERSION_TLS1_2) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, current time: %" MBEDTLS_PRINTF_LONGLONG, (long long)gmt_unix_time));
#endif /* MBEDTLS_HAVE_TIME */
    }

    ret = psa_generate_random(randbytes + gmt_unix_time_len, MBEDTLS_CLIENT_HELLO_RANDOM_LEN - gmt_unix_time_len);
    return ret;
}
...
static int ssl_prepare_client_hello(mbedtls_ssl_context* ssl) {
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3) && defined(MBEDTLS_SSL_SESSION_TICKETS) && defined(MBEDTLS_HAVE_TIME)

    /* Check if a tls13 ticket has been configured. */
    if (ssl->handshake->resume != 0 && session_negotiate->tls_version == MBEDTLS_SSL_VERSION_TLS1_3 &&
    ...
    if ((ssl->conf->transport != MBEDTLS_SSL_TRANSPORT_DATAGRAM) || (ssl->handshake->cookie == NULL))
#endif
    {
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
    ...
    if (session_id_len != session_negotiate->id_len) {
        ...
        if (session_id_len > 0) {
            ret = psa_generate_random(session_negotiate->id, session_id_len);
            if (ret != 0) {
    ...
    if (ssl->tls_version == MBEDTLS_SSL_VERSION_TLS1_3 && ssl->handshake->resume) {
        int hostname_mismatch = ssl->hostname != NULL || session_negotiate->hostname != NULL;
        if (ssl->hostname != NULL && session_negotiate->hostname != NULL) {
            hostname_mismatch = strcmp(ssl->hostname, session_negotiate->hostname) != 0;
        }
        ...
        return mbedtls_ssl_session_set_hostname(session_negotiate, ssl->hostname);
    }
#endif /* MBEDTLS_SSL_PROTO_TLS1_3 &&    \
          MBEDTLS_SSL_SESSION_TICKETS && \
...
int mbedtls_ssl_write_client_hello(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_start_handshake_msg(ssl, MBEDTLS_SSL_HS_CLIENT_HELLO, &buf, &buf_len));

    MBEDTLS_SSL_PROC_CHK(ssl_write_client_hello_body(ssl, buf, buf + buf_len, &msg_len, &binders_len));
...
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 && MBEDTLS_SSL_PROTO_DTLS */
    {
        ...
        if (binders_len > 0) {
            MBEDTLS_SSL_PROC_CHK(
                mbedtls_ssl_tls13_write_binders_of_pre_shared_key_ext(ssl, buf + msg_len - binders_len, buf + msg_len));
