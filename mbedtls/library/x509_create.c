...
static int oid_subidentifier_encode_into(unsigned char** p, unsigned char* bound, unsigned int value) {
    ...
    while (c <= end) {
        ...
        if (!in_attr_type && ((*c == ',' && *(c - 1) != '\\') || c == end)) {
            if (s == c) {
                ...
                parse_ret = parse_attribute_value_hex_der_encoded(s + 1, (size_t)(c - s) - 1, data, sizeof(data),
                                                                  &data_len, &tag);
                if (parse_ret != 0) {
                    mbedtls_free(oid.p);
                    return parse_ret;
            ...
            mbedtls_asn1_named_data* cur =
                mbedtls_asn1_store_named_data(head, (char*)oid.p, oid.len, (unsigned char*)data, data_len);
            mbedtls_free(oid.p);
            oid.p = NULL;
            if (cur == NULL) {
                return MBEDTLS_ERR_X509_ALLOC_FAILED;
...
int mbedtls_x509_write_sig(unsigned char** p,
    ...
    *--(*p) = 0;
    ...
    MBEDTLS_ASN1_CHK_ADD(len, mbedtls_asn1_write_algorithm_identifier_ext(p, start, oid, oid_len, 0, write_null_par));
