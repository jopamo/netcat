...
int mbedtls_x509write_csr_set_key_usage(mbedtls_x509write_csr* ctx, unsigned char key_usage) {
    ...
    ret = mbedtls_x509write_csr_set_extension(ctx, MBEDTLS_OID_KEY_USAGE, MBEDTLS_OID_SIZE(MBEDTLS_OID_KEY_USAGE), 0, c,
                                              (size_t)ret);
    if (ret != 0) {
        return ret;
    }
...
int mbedtls_x509write_csr_set_ns_cert_type(mbedtls_x509write_csr* ctx, unsigned char ns_cert_type) {
    ...
    ret = mbedtls_x509write_csr_set_extension(ctx, MBEDTLS_OID_NS_CERT_TYPE, MBEDTLS_OID_SIZE(MBEDTLS_OID_NS_CERT_TYPE),
                                              0, c, (size_t)ret);
    if (ret != 0) {
        return ret;
    }
...
static int x509write_csr_der_internal(mbedtls_x509write_csr* ctx,
    ...
    if ((ret = mbedtls_pk_sign_ext(pk_alg, ctx->key, ctx->md_alg, hash, 0, sig, sig_size, &sig_len)) != 0) {
        return ret;
    }

    if ((ret = mbedtls_x509_oid_get_oid_by_sig_alg(pk_alg, ctx->md_alg, &sig_oid, &sig_oid_len)) != 0) {
        return ret;
    }
...
int mbedtls_x509write_csr_pem(mbedtls_x509write_csr* ctx, unsigned char* buf, size_t size) {
    ...
    if ((ret = mbedtls_pem_write_buffer(PEM_BEGIN_CSR, PEM_END_CSR, buf + size - ret, ret, buf, size, &olen)) != 0) {
        return ret;
    }
