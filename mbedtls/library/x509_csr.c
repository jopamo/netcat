        ...
        *p += extn_oid.len;
        ...
        switch (ext_type) {
            ...
            case MBEDTLS_X509_EXT_SUBJECT_ALT_NAME:
                ...
                if ((ret = mbedtls_x509_get_subject_alt_name(p, end_ext_data, &csr->subject_alt_names)) != 0) {
                    return ret;
                }
                break;
...
static int mbedtls_x509_csr_parse_der_internal(mbedtls_x509_csr* csr,
    ...
    if ((ret = mbedtls_x509_get_sig_alg(&csr->sig_oid, &sig_params, &csr->sig_md, &csr->sig_pk)) != 0) {
        mbedtls_x509_csr_free(csr);
...
int mbedtls_x509_csr_parse(mbedtls_x509_csr* csr, const unsigned char* buf, size_t buflen) {
    ...
    if (buf[buflen - 1] == '\0') {
        ...
        ret = mbedtls_pem_read_buffer(&pem, "-----BEGIN CERTIFICATE REQUEST-----", "-----END CERTIFICATE REQUEST-----",
                                      buf, NULL, 0, &use_len);
...
int mbedtls_x509_csr_info(char* buf, size_t size, const char* prefix, const mbedtls_x509_csr* csr) {
    ...
    if (csr->ext_types & MBEDTLS_X509_EXT_SUBJECT_ALT_NAME) {
        ...
        if ((ret = mbedtls_x509_info_subject_alt_name(&p, &n, &csr->subject_alt_names, prefix)) != 0) {
            return ret;
        }
    }
