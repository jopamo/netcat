...
int mbedtls_ssl_get_peer_cid(mbedtls_ssl_context* ssl,
    ...
    *enabled = MBEDTLS_SSL_CID_DISABLED;
    ...
    if (ssl->conf->transport != MBEDTLS_SSL_TRANSPORT_DATAGRAM || mbedtls_ssl_is_handshake_over(ssl) == 0) {
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
...
typedef int
ssl_tls_prf_t(const unsigned char*, size_t, const char*, const unsigned char*, size_t, unsigned char*, size_t);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls12_session_load(mbedtls_ssl_session* session, const unsigned char* buf, size_t len);
...
void mbedtls_ssl_print_extension(const mbedtls_ssl_context* ssl,
    ...
    mbedtls_debug_print_msg(ssl, level, file, line, "%s: %s(%u) extension.", ssl_tls13_get_hs_msg_name(hs_msg_type),
...
void mbedtls_ssl_print_ticket_flags(const mbedtls_ssl_context* ssl,
    ...
    for (i = 0; i < ARRAY_LENGTH(ticket_flag_name_table); i++) {
        if ((flags & (1 << i))) {
            mbedtls_debug_print_msg(ssl, level, file, line, "- %s is set.", ticket_flag_name_table[i]);
        }
    }
}
...
static int ssl_update_checksum_start(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
#if defined(PSA_WANT_ALG_SHA_256) || defined(PSA_WANT_ALG_SHA_384)
    psa_status_t status;
#else  /* SHA-256 or SHA-384 */
    ((void)ssl);
...
#if defined(PSA_WANT_ALG_SHA_256)
static int ssl_update_checksum_sha256(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
    return mbedtls_md_error_from_psa(psa_hash_update(&ssl->handshake->fin_sha256_psa, buf, len));
}
#endif

#if defined(PSA_WANT_ALG_SHA_384)
static int ssl_update_checksum_sha384(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3) && defined(MBEDTLS_SSL_SRV_C) && defined(MBEDTLS_SSL_SESSION_TICKETS)
...
int mbedtls_ssl_set_session(mbedtls_ssl_context* ssl, const mbedtls_ssl_session* session) {
    ...
    if (session->tls_version == MBEDTLS_SSL_VERSION_TLS1_3) {
        ...
        const mbedtls_ssl_ciphersuite_t* ciphersuite_info = mbedtls_ssl_ciphersuite_from_id(session->ciphersuite);
...
static psa_status_t mbedtls_ssl_set_hs_ecjpake_password_common(mbedtls_ssl_context* ssl, mbedtls_svc_key_id_t pwd) {
    psa_status_t status;
...
static mbedtls_ssl_mode_t mbedtls_ssl_get_actual_mode(mbedtls_ssl_mode_t base_mode, int encrypt_then_mac) {
#if defined(MBEDTLS_SSL_SOME_SUITES_USE_CBC_ETM)
...
mbedtls_ssl_mode_t mbedtls_ssl_get_mode_from_transform(const mbedtls_ssl_transform* transform) {
    mbedtls_ssl_mode_t base_mode = mbedtls_ssl_get_base_mode(transform->psa_alg);
...
size_t mbedtls_ssl_get_output_record_size_limit(const mbedtls_ssl_context* ssl) {
    ...
    if (ssl->session != NULL && ssl->session->record_size_limit >= MBEDTLS_SSL_RECORD_SIZE_LIMIT_MIN &&
...
int mbedtls_ssl_get_max_out_record_payload(const mbedtls_ssl_context* ssl) {
    ...
    if (ssl->transform_out != NULL && ssl->transform_out->tls_version == MBEDTLS_SSL_VERSION_TLS1_3) {
        /*
         ...
         */
        max_len =
            ((max_len / MBEDTLS_SSL_CID_TLS1_3_PADDING_GRANULARITY) * MBEDTLS_SSL_CID_TLS1_3_PADDING_GRANULARITY) - 1;
    }
 ...
 */
static size_t ssl_tls12_session_save(const mbedtls_ssl_session* session, unsigned char* buf, size_t buf_len) {
    unsigned char* p = buf;
    size_t used = 0;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_session_load(mbedtls_ssl_session* session, const unsigned char* buf, size_t len) {
    const unsigned char* p = buf;
...
static int ssl_tls13_session_load(const mbedtls_ssl_session* session, const unsigned char* buf, size_t buf_len) {
    ((void)session);
    ((void)buf);
    ((void)buf_len);
    return MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;
...
void mbedtls_ssl_handshake_free(mbedtls_ssl_context* ssl) {
    ...
    defined(MBEDTLS_KEY_EXCHANGE_WITH_ECDSA_ANY_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
...
#if defined(MBEDTLS_SSL_CLI_C) && (defined(MBEDTLS_SSL_PROTO_DTLS) || defined(MBEDTLS_SSL_PROTO_TLS1_3))
...
static int ssl_context_load(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
    ...
    ret = ssl_tls12_populate_transform(ssl->transform, ssl->session->ciphersuite, ssl->session->master,
#if defined(MBEDTLS_SSL_SOME_SUITES_USE_CBC_ETM)
...
static const uint16_t ssl_preset_default_groups[] = {
    ...
    MBEDTLS_SSL_IANA_TLS_GROUP_FFDHE2048, MBEDTLS_SSL_IANA_TLS_GROUP_FFDHE3072, MBEDTLS_SSL_IANA_TLS_GROUP_FFDHE4096,
    MBEDTLS_SSL_IANA_TLS_GROUP_FFDHE6144, MBEDTLS_SSL_IANA_TLS_GROUP_FFDHE8192,
#endif
    MBEDTLS_SSL_IANA_TLS_GROUP_NONE};

static const int ssl_preset_suiteb_ciphersuites[] = {MBEDTLS_TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256,
...
#if defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ANY_ALLOWED_ENABLED) && defined(PSA_WANT_ALG_SHA_256) && \
    defined(PSA_WANT_ECC_SECP_R1_256)
    MBEDTLS_TLS1_3_SIG_ECDSA_SECP256R1_SHA256,
// == MBEDTLS_SSL_TLS12_SIG_AND_HASH_ALG(MBEDTLS_SSL_SIG_ECDSA, MBEDTLS_SSL_HASH_SHA256)
#endif

#if defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ANY_ALLOWED_ENABLED) && defined(PSA_WANT_ALG_SHA_384) && \
    defined(PSA_WANT_ECC_SECP_R1_384)
    MBEDTLS_TLS1_3_SIG_ECDSA_SECP384R1_SHA384,
// == MBEDTLS_SSL_TLS12_SIG_AND_HASH_ALG(MBEDTLS_SSL_SIG_ECDSA, MBEDTLS_SSL_HASH_SHA384)
#endif

#if defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ANY_ALLOWED_ENABLED) && defined(PSA_WANT_ALG_SHA_512) && \
...
static int ssl_check_no_sig_alg_duplication(const uint16_t* sig_algs) {
    ...
    for (i = 0; sig_algs[i] != MBEDTLS_TLS_SIG_NONE; i++) {
        for (j = 0; j < i; j++) {
            ...
            mbedtls_printf(" entry(%04x,%" MBEDTLS_PRINTF_SIZET ") is duplicated at %" MBEDTLS_PRINTF_SIZET "\n",
                           sig_algs[i], j, i);
            ret = -1;
        }
    }
...
int mbedtls_ssl_parse_sig_alg_ext(mbedtls_ssl_context* ssl, const unsigned char* buf, const unsigned char* end) {
    ...
    while (p < supported_sig_algs_end) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(4,
                              ("received signature algorithm: 0x%x %s", sig_alg, mbedtls_ssl_sig_alg_to_str(sig_alg)));
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
        if (ssl->tls_version == MBEDTLS_SSL_VERSION_TLS1_2 &&
            (!(mbedtls_ssl_sig_alg_is_supported(ssl, sig_alg) && mbedtls_ssl_sig_alg_is_offered(ssl, sig_alg)))) {
            continue;
        }
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
...
static psa_status_t setup_psa_key_derivation(psa_key_derivation_operation_t* derivation,
    ...
    if (PSA_ALG_IS_TLS12_PRF(alg) || PSA_ALG_IS_TLS12_PSK_TO_MS(alg)) {
        ...
        if (other_secret != NULL) {
            status = psa_key_derivation_input_bytes(derivation, PSA_KEY_DERIVATION_INPUT_OTHER_SECRET, other_secret,
                                                    other_secret_length);
            if (status != PSA_SUCCESS) {
                return status;
            }
        ...
        if (mbedtls_svc_key_id_is_null(key)) {
            status =
                psa_key_derivation_input_bytes(derivation, PSA_KEY_DERIVATION_INPUT_SECRET, raw_psk, raw_psk_length);
        }
        else {
        ...
        status = psa_key_derivation_input_bytes(derivation, PSA_KEY_DERIVATION_INPUT_LABEL, label, label_length);
        if (status != PSA_SUCCESS) {
            return status;
        }
    }
...
static int tls_prf_generic(mbedtls_md_type_t md_type,
    ...
    status = setup_psa_key_derivation(&derivation, master_key, alg, NULL, 0, random, rlen, (unsigned char const*)label,
                                      label_len, NULL, 0, dlen);
    if (status != PSA_SUCCESS) {
...
static int tls_prf_sha256(const unsigned char* secret,
                          ...
                          unsigned char* dstbuf,
                          size_t dlen) {
    return tls_prf_generic(MBEDTLS_MD_SHA256, secret, slen, label, strlen(label), random, rlen, dstbuf, dlen);
}
#endif /* PSA_WANT_ALG_SHA_256*/
...
static int tls_prf_sha384(const unsigned char* secret,
                          ...
                          unsigned char* dstbuf,
                          size_t dlen) {
    return tls_prf_generic(MBEDTLS_MD_SHA384, secret, slen, label, strlen(label), random, rlen, dstbuf, dlen);
}
#endif /* PSA_WANT_ALG_SHA_384*/
...
static int ssl_compute_master(mbedtls_ssl_handshake_params* handshake,
    ...
    if (mbedtls_ssl_ciphersuite_uses_psk(handshake->ciphersuite_info) == 1) {
        ...
        status = setup_psa_key_derivation(&derivation, psk, alg, ssl->conf->psk, ssl->conf->psk_len, seed, seed_len,
                                          (unsigned char const*)lbl, (size_t)strlen(lbl), other_secret,
...
#endif
    {
        ...
        if (handshake->ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE) {
            ...
            status = psa_key_derivation_set_capacity(&derivation, PSA_TLS12_ECJPAKE_TO_PMS_DATA_SIZE);
            if (status != PSA_SUCCESS) {
                psa_key_derivation_abort(&derivation);
                return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
            }
            ...
            status = psa_key_derivation_output_bytes(&derivation, handshake->premaster, handshake->pmslen);
            if (status != PSA_SUCCESS) {
                psa_key_derivation_abort(&derivation);
                return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
            }
...
#endif
        ret =
            handshake->tls_prf(handshake->premaster, handshake->pmslen, lbl, seed, seed_len, master, master_secret_len);
        if (ret != 0) {
...
int mbedtls_ssl_derive_keys(mbedtls_ssl_context* ssl) {
    ...
    ret = ssl_tls12_populate_transform(
        ssl->transform_negotiate, ssl->session_negotiate->ciphersuite, ssl->session_negotiate->master,
#if defined(MBEDTLS_SSL_SOME_SUITES_USE_CBC_ETM)
        ssl->session_negotiate->encrypt_then_mac,
#endif /* MBEDTLS_SSL_SOME_SUITES_USE_CBC_ETM */
        ssl->handshake->tls_prf, ssl->handshake->randbytes, ssl->tls_version, ssl->conf->endpoint, ssl);
    if (ret != 0) {
...
int mbedtls_ssl_write_certificate(mbedtls_ssl_context* ssl) {
    ...
    while (crt != NULL) {
        ...
        if (n > MBEDTLS_SSL_OUT_CONTENT_LEN - 3 - i) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("certificate too large, %" MBEDTLS_PRINTF_SIZET " > %" MBEDTLS_PRINTF_SIZET,
...
crt_verify:
    ...
    ret = mbedtls_ssl_verify_certificate(ssl, authmode, chain, ssl->handshake->ciphersuite_info, rs_ctx);
    if (ret != 0) {
        goto exit;
    }
...
static int ssl_calc_finished_tls_generic(mbedtls_ssl_context* ssl,
                                         void* ctx,
...
static int ssl_calc_finished_tls_sha256(mbedtls_ssl_context* ssl, unsigned char* buf, int from) {
    ...
    return ssl_calc_finished_tls_generic(ssl, &ssl->handshake->fin_sha256_psa, padbuf, sizeof(padbuf), buf, from);
}
#endif /* PSA_WANT_ALG_SHA_256*/
...
static int ssl_calc_finished_tls_sha384(mbedtls_ssl_context* ssl, unsigned char* buf, int from) {
    ...
    return ssl_calc_finished_tls_generic(ssl, &ssl->handshake->fin_sha384_psa, padbuf, sizeof(padbuf), buf, from);
}
#endif /* PSA_WANT_ALG_SHA_384*/
...
void mbedtls_ssl_handshake_wrapup(mbedtls_ssl_context* ssl) {
    ...
    if (ssl->conf->f_set_cache != NULL && ssl->session->id_len != 0 && resume == 0) {
        if (ssl->conf->f_set_cache(ssl->conf->p_cache, ssl->session->id, ssl->session->id_len, ssl->session) != 0) {
...
static int ssl_tls12_populate_transform(mbedtls_ssl_transform* transform,
    ...
    if ((status = mbedtls_ssl_cipher_to_psa((mbedtls_cipher_type_t)ciphersuite_info->cipher, transform->taglen, &alg,
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("keylen: %u, minlen: %u, ivlen: %u, maclen: %u", (unsigned)keylen,
    ...
    if (ssl->f_export_keys != NULL) {
        ssl->f_export_keys(ssl->p_export_keys, MBEDTLS_SSL_KEY_EXPORT_TLS12_MASTER_SECRET, master, 48, randbytes + 32,
                           randbytes, tls_prf_get_type(tls_prf));
    }
    ...
    if (alg != MBEDTLS_SSL_NULL_CIPHER) {
        ...
        if ((status = psa_import_key(&attributes, key1, PSA_BITS_TO_BYTES(key_bits), &transform->psa_key_enc)) !=
            PSA_SUCCESS) {
        ...
        if ((status = psa_import_key(&attributes, key2, PSA_BITS_TO_BYTES(key_bits), &transform->psa_key_dec)) !=
            PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "psa_import_key", ret);
            goto end;
    ...
    if (mac_key_len != 0) {
        ...
        if ((status = psa_import_key(&attributes, mac_enc, mac_key_len, &transform->psa_mac_enc)) != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
        ...
        if ((status = psa_import_key(&attributes, mac_dec, mac_key_len, &transform->psa_mac_dec)) != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "psa_import_mac_key", ret);
            goto end;
        }
...
int mbedtls_psa_ecjpake_read_round(psa_pake_operation_t* pake_ctx,
    ...
    for (; remaining_steps > 0; remaining_steps--) {
        for (psa_pake_step_t step = PSA_PAKE_STEP_KEY_SHARE; step <= PSA_PAKE_STEP_ZK_PROOF; ++step) {
...
int mbedtls_psa_ecjpake_write_round(psa_pake_operation_t* pake_ctx,
    ...
    for (; remaining_steps > 0; remaining_steps--) {
        for (psa_pake_step_t step = PSA_PAKE_STEP_KEY_SHARE; step <= PSA_PAKE_STEP_ZK_PROOF; ++step) {
             ...
             */
            status = psa_pake_output(pake_ctx, step, buf + output_offset + 1, len - output_offset - 1, &output_len);
            if (status != PSA_SUCCESS) {
                return PSA_TO_MBEDTLS_ERR(status);
            }
...
int mbedtls_ssl_get_key_exchange_md_tls1_2(mbedtls_ssl_context* ssl,
    ...
    if ((status = psa_hash_finish(&hash_operation, hash, PSA_HASH_MAX_SIZE, hashlen)) != PSA_SUCCESS) {
...
unsigned int mbedtls_ssl_tls12_get_preferred_hash_for_sig_alg(mbedtls_ssl_context* ssl, unsigned int sig_alg) {
    ...
    for (i = 0; received_sig_algs[i] != MBEDTLS_TLS_SIG_NONE; i++) {
        unsigned int hash_alg_received = MBEDTLS_SSL_TLS12_HASH_ALG_FROM_SIG_AND_HASH_ALG(received_sig_algs[i]);
        unsigned int sig_alg_received = MBEDTLS_SSL_TLS12_SIG_ALG_FROM_SIG_AND_HASH_ALG(received_sig_algs[i]);
        ...
        if (sig_alg == sig_alg_received) {
            if (ssl->handshake->key_cert && ssl->handshake->key_cert->key) {
                ...
                if (sig_alg_received == MBEDTLS_SSL_SIG_ECDSA &&
                    !mbedtls_pk_can_do_psa(ssl->handshake->key_cert->key, MBEDTLS_PK_ALG_ECDSA(psa_hash_alg),
                                           PSA_KEY_USAGE_SIGN_HASH)) {
                    continue;
                }

                if (sig_alg_received == MBEDTLS_SSL_SIG_RSA &&
                    !mbedtls_pk_can_do_psa(ssl->handshake->key_cert->key, PSA_ALG_RSA_PKCS1V15_SIGN(psa_hash_alg),
                                           PSA_KEY_USAGE_SIGN_HASH)) {
                    continue;
                }
            }
...
int mbedtls_ssl_validate_ciphersuite(const mbedtls_ssl_context* ssl,
    ...
    if ((suite_info->min_tls_version > max_tls_version) || (suite_info->max_tls_version < min_tls_version)) {
        return -1;
    }

#if defined(MBEDTLS_SSL_PROTO_TLS1_2) && defined(MBEDTLS_SSL_CLI_C)
#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
    if (suite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE && ssl->handshake->psa_pake_ctx_is_ok != 1) {
        return -1;
    }
#endif
...
#if defined(MBEDTLS_KEY_EXCHANGE_SOME_PSK_ENABLED)
    if (mbedtls_ssl_ciphersuite_uses_psk(suite_info) && mbedtls_ssl_conf_has_static_psk(ssl->conf) == 0) {
        return -1;
    }
...
int mbedtls_ssl_verify_certificate(mbedtls_ssl_context* ssl,
    ...
    if (ssl->conf->f_ca_cb != NULL) {
        ...
        ret = mbedtls_x509_crt_verify_with_ca_cb(chain, ssl->conf->f_ca_cb, ssl->conf->p_ca_cb, ssl->conf->cert_profile,
                                                 hostname, &ssl->session_negotiate->verify_result, f_vrfy, p_vrfy);
    }
    else
#endif /* MBEDTLS_X509_TRUSTED_CERTIFICATE_CALLBACK */
    {
        ...
        ret = mbedtls_x509_crt_verify_restartable(chain, ca_chain, ca_crl, ssl->conf->cert_profile, hostname,
     ...
     */
#if defined(MBEDTLS_SSL_PROTO_TLS1_2) && defined(PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY)
    if (ssl->tls_version == MBEDTLS_SSL_VERSION_TLS1_2 && PSA_KEY_TYPE_IS_ECC(mbedtls_pk_get_type(&chain->pk))) {
    ...
    if (ssl->session_negotiate->verify_result != 0) {
        MBEDTLS_SSL_DEBUG_MSG(
            3, ("! Certificate verification flags %08x", (unsigned int)ssl->session_negotiate->verify_result));
    }
    else {
...
static int mbedtls_ssl_tls12_export_keying_material(const mbedtls_ssl_context* ssl,
    ...
    memcpy(prf_input, ssl->transform->randbytes + MBEDTLS_SERVER_HELLO_RANDOM_LEN, MBEDTLS_CLIENT_HELLO_RANDOM_LEN);
    memcpy(prf_input + MBEDTLS_CLIENT_HELLO_RANDOM_LEN, ssl->transform->randbytes, MBEDTLS_SERVER_HELLO_RANDOM_LEN);
    if (use_context) {
    ...
    ret = tls_prf_generic(hash_alg, ssl->session->master, sizeof(ssl->session->master), label, label_len, prf_input,
                          prf_input_len, out, key_len);
...
static int mbedtls_ssl_tls13_export_keying_material(mbedtls_ssl_context* ssl,
    ...
    return mbedtls_ssl_tls13_exporter(psa_hash_alg, secret, hash_len, (const unsigned char*)label, label_len, context,
                                      context_len, out, key_len);
}
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
        case MBEDTLS_SSL_VERSION_TLS1_2:
            return mbedtls_ssl_tls12_export_keying_material(ssl, hash_alg, out, key_len, label, label_len, context,
