...
static int ssl_ticket_gen_key(mbedtls_ssl_ticket_context* ctx, unsigned char index) {
    ...
    ret = PSA_TO_MBEDTLS_ERR(psa_import_key(&attributes, buf, PSA_BITS_TO_BYTES(key->key_bits), &key->key));
...
int mbedtls_ssl_ticket_rotate(mbedtls_ssl_ticket_context* ctx,
    ...
    if ((status = psa_import_key(&attributes, k, PSA_BITS_TO_BYTES(key->key_bits), &key->key)) != PSA_SUCCESS) {
        ret = PSA_TO_MBEDTLS_ERR(status);
        return ret;
    }
...
int mbedtls_ssl_ticket_write(void* p_ticket,
    ...
    *ticket_lifetime = key->lifetime;
    ...
    if ((status = psa_aead_encrypt(key->key, key->alg, iv, TICKET_IV_BYTES, key_name, TICKET_ADD_DATA_LEN, state,
                                   clear_len, state, end - state, &ciph_len)) != PSA_SUCCESS) {
        ret = PSA_TO_MBEDTLS_ERR(status);
        goto cleanup;
    }
...
int mbedtls_ssl_ticket_parse(void* p_ticket, mbedtls_ssl_session* session, unsigned char* buf, size_t len) {
    ...
    if ((status = psa_aead_decrypt(key->key, key->alg, iv, TICKET_IV_BYTES, key_name, TICKET_ADD_DATA_LEN, ticket,
                                   enc_len + TICKET_AUTH_TAG_BYTES, ticket, enc_len, &clear_len)) != PSA_SUCCESS) {
        ret = PSA_TO_MBEDTLS_ERR(status);
        goto cleanup;
    }
