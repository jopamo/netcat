...
static int ssl_tls13_write_supported_versions_ext(mbedtls_ssl_context* ssl,
                                                  ...
                                                  size_t* out_len) {
    unsigned char* p = buf;
    unsigned char versions_len = (ssl->handshake->min_tls_version <= MBEDTLS_SSL_VERSION_TLS1_2) ? 4 : 2;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_parse_alpn_ext(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
...
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED)
    if (mbedtls_ssl_tls13_named_group_is_ecdhe(group_id) || mbedtls_ssl_tls13_named_group_is_ffdh(group_id)) {
...
static int ssl_tls13_write_key_share_ext(mbedtls_ssl_context* ssl,
    ...
    *out_len = 0;
    ...
    if (mbedtls_ssl_tls13_named_group_is_ecdhe(group_id) || mbedtls_ssl_tls13_named_group_is_ffdh(group_id)) {
        ...
        p += 4;
        ret = mbedtls_ssl_tls13_generate_and_write_xxdh_key_exchange(ssl, group_id, p, end, &key_exchange_len);
        p += key_exchange_len;
        if (ret != 0) {
...
static int ssl_tls13_parse_hrr_key_share_ext(mbedtls_ssl_context* ssl,
    ...
    for (; *group_list != 0; group_list++) {
        ...
        if (mbedtls_ssl_tls13_named_group_is_ecdhe(*group_list)) {
            if ((mbedtls_ssl_get_psa_curve_info_from_tls_id(*group_list, NULL, NULL) == PSA_ERROR_NOT_SUPPORTED) ||
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_parse_key_share_ext(mbedtls_ssl_context* ssl, const unsigned char* buf, const unsigned char* end) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    const unsigned char* p = buf;
    ...
    if (offered_group != group) {
        MBEDTLS_SSL_DEBUG_MSG(
            1, ("Invalid server key share, our group %u, their group %u", (unsigned)offered_group, (unsigned)group));
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_parse_cookie_ext(mbedtls_ssl_context* ssl, const unsigned char* buf, const unsigned char* end) {
    uint16_t cookie_len;
    const unsigned char* p = buf;
...
static int ssl_tls13_has_configured_ticket(mbedtls_ssl_context* ssl) {
    ...
    return ssl->handshake->resume && session != NULL && session->ticket != NULL &&
           mbedtls_ssl_conf_tls13_is_kex_mode_enabled(
               ssl, mbedtls_ssl_tls13_session_get_ticket_flags(session, MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_ALL));
}
...
static int ssl_tls13_write_binder(mbedtls_ssl_context* ssl,
    ...
    *out_len = 0;
    ...
    ret = mbedtls_ssl_get_handshake_transcript(ssl, mbedtls_md_type_from_psa_alg(hash_alg), transcript,
                                               sizeof(transcript), &transcript_len);
    if (ret != 0) {
        return ret;
    }

    ret = mbedtls_ssl_tls13_create_psk_binder(ssl, hash_alg, psk, psk_len, psk_type, transcript, buf + 1);
    if (ret != 0) {
...
int mbedtls_ssl_tls13_write_identities_of_pre_shared_key_ext(mbedtls_ssl_context* ssl,
    ...
    *binders_len = 0;
    ...
    if (ssl_tls13_ticket_get_identity(ssl, &hash_alg, &identity, &identity_len) == 0) {
        ...
        ret = ssl_tls13_write_identity(ssl, p, end, identity, identity_len, obfuscated_ticket_age, &output_len);
#else
...
int mbedtls_ssl_tls13_write_binders_of_pre_shared_key_ext(mbedtls_ssl_context* ssl,
    ...
    if (ssl_tls13_ticket_get_psk(ssl, &hash_alg, &psk, &psk_len) == 0) {
        ret =
            ssl_tls13_write_binder(ssl, p, end, MBEDTLS_SSL_TLS1_3_PSK_RESUMPTION, hash_alg, psk, psk_len, &output_len);
        if (ret != 0) {
            return ret;
        }
        p += output_len;
    ...
    if (ssl_tls13_psk_get_psk(ssl, &hash_alg, &psk, &psk_len) == 0) {
        ret = ssl_tls13_write_binder(ssl, p, end, MBEDTLS_SSL_TLS1_3_PSK_EXTERNAL, hash_alg, psk, psk_len, &output_len);
        if (ret != 0) {
            return ret;
        }
        p += output_len;
...
static int ssl_tls13_parse_server_pre_shared_key_ext(mbedtls_ssl_context* ssl,
    ...
    if (mbedtls_md_psa_alg_from_type((mbedtls_md_type_t)ssl->handshake->ciphersuite_info->mac) != hash_alg) {
...
int mbedtls_ssl_tls13_write_client_hello_exts(mbedtls_ssl_context* ssl,
    ...
    *out_len = 0;
    ...
    if (!ssl->handshake->hello_retry_request_flag) {
        if (mbedtls_ssl_conf_tls13_is_some_psk_enabled(ssl) && ssl_tls13_early_data_has_valid_ticket(ssl) &&
...
static int ssl_tls13_is_supported_versions_ext_present(mbedtls_ssl_context* ssl,
    ...
    return mbedtls_ssl_tls13_is_supported_versions_ext_present_in_exts(ssl, p, end, &supported_versions_data,
...
static int ssl_tls13_is_downgrade_negotiation(mbedtls_ssl_context* ssl,
    ...
    static const unsigned char magic_downgrade_string[] = {0x44, 0x4F, 0x57, 0x4E, 0x47, 0x52, 0x44};
    ...
    if (memcmp(last_eight_bytes_of_random, magic_downgrade_string, sizeof(magic_downgrade_string)) == 0) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_server_hello_is_hrr(mbedtls_ssl_context* ssl, const unsigned char* buf, const unsigned char* end) {
...
static int ssl_tls13_preprocess_server_hello(mbedtls_ssl_context* ssl,
    ...
    if (ret == 0) {
        ...
        MBEDTLS_SSL_PROC_CHK(
            mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_SERVER_HELLO, buf, (size_t)(end - buf)));
...
static int ssl_tls13_parse_server_hello(mbedtls_ssl_context* ssl,
    ...
    int hs_msg_type = is_hrr ? MBEDTLS_SSL_TLS1_3_HS_HELLO_RETRY_REQUEST : MBEDTLS_SSL_HS_SERVER_HELLO;
     ...
     */
    if (!is_hrr) {
        memcpy(&handshake->randbytes[MBEDTLS_CLIENT_HELLO_RANDOM_LEN], p, MBEDTLS_SERVER_HELLO_RANDOM_LEN);
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, chosen ciphersuite: ( %04x ) - %s", cipher_suite, ciphersuite_info->name));
    ...
    handshake->received_extensions = MBEDTLS_SSL_EXT_MASK_NONE;
    allowed_extensions_mask = is_hrr ? MBEDTLS_SSL_TLS1_3_ALLOWED_EXTS_OF_HRR : MBEDTLS_SSL_TLS1_3_ALLOWED_EXTS_OF_SH;
    ...
    while (p < extensions_end) {
        ...
        ret = mbedtls_ssl_tls13_check_received_extension(ssl, hs_msg_type, extension_type, allowed_extensions_mask);
        if (ret != 0) {
            return ret;
        }
        ...
        switch (extension_type) {
            ...
            case MBEDTLS_TLS_EXT_PRE_SHARED_KEY:
                ...
                if ((ret = ssl_tls13_parse_server_pre_shared_key_ext(ssl, p, extension_data_end)) != 0) {
...
cleanup:
    ...
    if (fatal_alert == MBEDTLS_SSL_ALERT_MSG_UNSUPPORTED_EXT) {
        MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_UNSUPPORTED_EXT, MBEDTLS_ERR_SSL_UNSUPPORTED_EXTENSION);
...
static int ssl_tls13_postprocess_server_hello(mbedtls_ssl_context* ssl) {
     ...
     */
    switch (handshake->received_extensions & (MBEDTLS_SSL_EXT_MASK(PRE_SHARED_KEY) | MBEDTLS_SSL_EXT_MASK(KEY_SHARE))) {
    ...
    if (!mbedtls_ssl_conf_tls13_is_kex_mode_enabled(ssl, handshake->key_exchange_mode)) {
        ret = MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
        MBEDTLS_SSL_DEBUG_MSG(
            2, ("Key exchange mode(%s) is not supported.", ssl_tls13_get_kex_mode_str(handshake->key_exchange_mode)));
        goto cleanup;
    }

    MBEDTLS_SSL_DEBUG_MSG(3,
                          ("Selected key exchange mode: %s", ssl_tls13_get_kex_mode_str(handshake->key_exchange_mode)));
...
static int ssl_tls13_process_server_hello(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_tls13_fetch_handshake_msg(ssl, MBEDTLS_SSL_HS_SERVER_HELLO, &buf, &buf_len));
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_SERVER_HELLO, buf, buf_len));
...
cleanup:
    MBEDTLS_SSL_DEBUG_MSG(2, ("<= %s ( %s )", __func__, is_hrr ? "HelloRetryRequest" : "ServerHello"));
    return ret;
}
...
static int ssl_tls13_parse_encrypted_extensions(mbedtls_ssl_context* ssl,
    ...
    while (p < extensions_end) {
        ...
        ret = mbedtls_ssl_tls13_check_received_extension(ssl, MBEDTLS_SSL_HS_ENCRYPTED_EXTENSIONS, extension_type,
        ...
        switch (extension_type) {
            ...
            default:
                MBEDTLS_SSL_PRINT_EXT(3, MBEDTLS_SSL_HS_ENCRYPTED_EXTENSIONS, extension_type, "( ignored )");
                break;
        }
    ...
    if ((handshake->received_extensions & MBEDTLS_SSL_EXT_MASK(RECORD_SIZE_LIMIT)) &&
        ...
        MBEDTLS_SSL_DEBUG_MSG(3, ("Record size limit extension cannot be used with max fragment length extension"));
        MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER, MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER);
        return MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
    }
...
static int ssl_tls13_process_encrypted_extensions(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(
        mbedtls_ssl_tls13_fetch_handshake_msg(ssl, MBEDTLS_SSL_HS_ENCRYPTED_EXTENSIONS, &buf, &buf_len));
    ...
    if (handshake->received_extensions & MBEDTLS_SSL_EXT_MASK(EARLY_DATA)) {
         ...
         */
        if ((!mbedtls_ssl_tls13_key_exchange_mode_with_psk(ssl)) || handshake->selected_identity != 0 ||
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_ENCRYPTED_EXTENSIONS, buf, buf_len));
...
static int ssl_tls13_write_end_of_early_data(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_start_handshake_msg(ssl, MBEDTLS_SSL_HS_END_OF_EARLY_DATA, &buf, &buf_len));

    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_hdr_to_checksum(ssl, MBEDTLS_SSL_HS_END_OF_EARLY_DATA, 0));
...
static int ssl_tls13_certificate_request_coordinate(mbedtls_ssl_context* ssl) {
    ...
    if ((ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE) && (ssl->in_msg[0] == MBEDTLS_SSL_HS_CERTIFICATE_REQUEST)) {
...
static int ssl_tls13_parse_certificate_request(mbedtls_ssl_context* ssl,
    ...
    while (p < extensions_end) {
        ...
        ret = mbedtls_ssl_tls13_check_received_extension(ssl, MBEDTLS_SSL_HS_CERTIFICATE_REQUEST, extension_type,
        ...
        switch (extension_type) {
            ...
            default:
                MBEDTLS_SSL_PRINT_EXT(3, MBEDTLS_SSL_HS_CERTIFICATE_REQUEST, extension_type, "( ignored )");
                break;
        }
...
static int ssl_tls13_process_certificate_request(mbedtls_ssl_context* ssl) {
    ...
    if (ret == SSL_CERTIFICATE_REQUEST_EXPECT_REQUEST) {
        ...
        MBEDTLS_SSL_PROC_CHK(
            mbedtls_ssl_tls13_fetch_handshake_msg(ssl, MBEDTLS_SSL_HS_CERTIFICATE_REQUEST, &buf, &buf_len));

        MBEDTLS_SSL_PROC_CHK(ssl_tls13_parse_certificate_request(ssl, buf, buf + buf_len));

        MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_CERTIFICATE_REQUEST, buf, buf_len));
    }
...
static int ssl_tls13_parse_new_session_ticket_early_data_ext(mbedtls_ssl_context* ssl,
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("received max_early_data_size: %u", (unsigned int)session->max_early_data_size));
...
static int ssl_tls13_parse_new_session_ticket_exts(mbedtls_ssl_context* ssl,
    ...
    while (p < end) {
        ...
        ret = mbedtls_ssl_tls13_check_received_extension(ssl, MBEDTLS_SSL_HS_NEW_SESSION_TICKET, extension_type,
        ...
        switch (extension_type) {
            ...
            default:
                MBEDTLS_SSL_PRINT_EXT(3, MBEDTLS_SSL_HS_NEW_SESSION_TICKET, extension_type, "( ignored )");
                break;
        }
...
static int ssl_tls13_postprocess_new_session_ticket(mbedtls_ssl_context* ssl,
    ...
    ret = mbedtls_ssl_tls13_hkdf_expand_label(psa_hash_alg, session->app_secrets.resumption_master_secret, hash_length,
                                              ...
                                              ticket_nonce_len, session->resumption_key, hash_length);
...
static int ssl_tls13_process_new_session_ticket(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_tls13_fetch_handshake_msg(ssl, MBEDTLS_SSL_HS_NEW_SESSION_TICKET, &buf, &buf_len));
    ...
    MBEDTLS_SSL_PROC_CHK(ssl_tls13_parse_new_session_ticket(ssl, buf, buf + buf_len, &ticket_nonce, &ticket_nonce_len));

    MBEDTLS_SSL_PROC_CHK_NEG(ssl_tls13_postprocess_new_session_ticket(ssl, ticket_nonce, ticket_nonce_len));
