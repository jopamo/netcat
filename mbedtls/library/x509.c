    ...
    *salt_len = 20;
    ...
    if ((ret = mbedtls_asn1_get_tag(&p, end, &len, MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_ASN1_CONSTRUCTED | 1)) ==
        ...
        if (MBEDTLS_OID_CMP(MBEDTLS_OID_MGF1, &alg_id) != 0) {
            return MBEDTLS_ERROR_ADD(MBEDTLS_ERR_X509_FEATURE_UNAVAILABLE, MBEDTLS_ERR_X509_UNKNOWN_OID);
        }
     ...
     * salt_len
     */
    if ((ret = mbedtls_asn1_get_tag(&p, end, &len, MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_ASN1_CONSTRUCTED | 2)) ==
        0) {
        end2 = p + len;
     ...
     */
    if ((ret = mbedtls_asn1_get_tag(&p, end, &len, MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_ASN1_CONSTRUCTED | 3)) ==
        0) {
        int trailer_field;
...
int mbedtls_x509_get_sig_alg(const mbedtls_x509_buf* sig_oid,
    ...
    if (*pk_alg == MBEDTLS_PK_SIGALG_RSA_PSS) {
        ...
        ret = mbedtls_x509_get_rsassa_pss_params(sig_params, md_alg, &mgf1_hash_id, &expected_salt_len);
        if (ret != 0) {
            return ret;
        }
...
int mbedtls_x509_time_cmp(const mbedtls_x509_time* t1, const mbedtls_x509_time* t2) {
    ...
    x = (((t1->year << 9) | (t1->mon << 5) | (t1->day)) - ((t2->year << 9) | (t2->mon << 5) | (t2->day)));
    if (x != 0) {
        return x;
    }

    x = (((t1->hour << 12) | (t1->min << 6) | (t1->sec)) - ((t2->hour << 12) | (t2->min << 6) | (t2->sec)));
    return x;
}
...
int mbedtls_x509_parse_subject_alt_name(const mbedtls_x509_buf* san_buf, mbedtls_x509_subject_alternative_name* san) {
    ...
    switch (san_buf->tag & (MBEDTLS_ASN1_TAG_CLASS_MASK | MBEDTLS_ASN1_TAG_VALUE_MASK)) {
        ...
        case (MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_X509_SAN_DIRECTORY_NAME): {
            ...
            ret =
                mbedtls_asn1_get_tag(&p, p + san_buf->len, &name_len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
...
#if !defined(MBEDTLS_X509_REMOVE_INFO)
int mbedtls_x509_info_subject_alt_name(char** buf,
                                       size_t* size,
        ...
        switch (san.type) {
            ...
            case MBEDTLS_X509_SAN_IP_ADDRESS: {
                ...
                if (san.san.unstructured_name.len == 4) {
                    ...
                    ret = mbedtls_snprintf(p, n, "%X%X:%X%X:%X%X:%X%X:%X%X:%X%X:%X%X:%X%X", ip[0], ip[1], ip[2], ip[3],
                                           ip[4], ip[5], ip[6], ip[7], ip[8], ip[9], ip[10], ip[11], ip[12], ip[13],
                                           ip[14], ip[15]);
                    MBEDTLS_X509_SAFE_SNPRINTF;
                }
                else {
