...
static void mbedtls_debug_print_buf_one_line(char* out_buf,
                                             size_t out_size,
    ...
    for (i = 0; i < 16; i++) {
        if (i < in_size) {
            idx += mbedtls_snprintf(out_buf + idx, out_size - idx, " %02x", (unsigned int)in_buf[i]);
...
static void mbedtls_debug_print_buf_ext(const mbedtls_ssl_context* ssl,
                                        int level,
                                        const char* file,
                                        int line,
                                        const char* text,
    ...
    while (len > 0) {
        ...
        mbedtls_debug_print_buf_one_line(str + idx, sizeof(str) - idx, &buf[curr_offset], chunk_len, add_text);
        debug_send_line(ssl, level, file, line, str);
        curr_offset += 16;
        len -= chunk_len;
    }
...
void mbedtls_debug_print_buf(const mbedtls_ssl_context* ssl,
                             ...
                             const unsigned char* buf,
                             size_t len) {
    mbedtls_debug_print_buf_ext(ssl, level, file, line, text, buf, len, MBEDTLS_DEBUG_PRINT_BUF_ADD_TEXT);
}
...
static void debug_print_pk(const mbedtls_ssl_context* ssl,
    ...
    if (ret == 0) {
        mbedtls_debug_print_buf_ext(ssl, level, file, line, text, buf, buf_len, MBEDTLS_DEBUG_PRINT_BUF_NO_TEXT);
    }
    else {
        mbedtls_debug_print_msg(ssl, level, file, line, "failed to export public key from PK context");
    }
}
