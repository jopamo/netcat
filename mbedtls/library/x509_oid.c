...
#define FN_OID_TYPED_FROM_ASN1(TYPE_T, NAME, LIST)                                          \
    static const TYPE_T* oid_##NAME##_from_asn1(const mbedtls_asn1_buf* oid) {              \
        const TYPE_T* p = (LIST);                                                           \
        const mbedtls_x509_oid_descriptor_t* cur = (const mbedtls_x509_oid_descriptor_t*)p; \
        if (p == NULL || oid == NULL)                                                       \
            return NULL;                                                                    \
        while (cur->asn1 != NULL) {                                                         \
...
FN_OID_GET_ATTR1(mbedtls_x509_oid_get_attr_short_name, oid_x520_attr_t, x520_attr, const char*, short_name)
...
FN_OID_GET_DESCRIPTOR_ATTR1(mbedtls_x509_oid_get_sig_alg_desc, oid_sig_alg_t, sig_alg, const char*, description)
