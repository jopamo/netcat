...
static int ssl_write_ecjpake_kkpp_ext(mbedtls_ssl_context* ssl,
    ...
    *olen = 0;
    ...
    if (ssl->handshake->ecjpake_cache == NULL || ssl->handshake->ecjpake_cache_len == 0) {
        ...
        ret = mbedtls_psa_ecjpake_write_round(&ssl->handshake->psa_pake_ctx, p + 2, end - p - 2, &kkpp_len,
...
static int ssl_write_use_srtp_ext(mbedtls_ssl_context* ssl,
    ...
    *p++ = MBEDTLS_BYTE_0(2 * ssl->conf->dtls_srtp_profile_list_len);
    ...
    for (protection_profiles_index = 0; protection_profiles_index < ssl->conf->dtls_srtp_profile_list_len;
        ...
        profile_value =
            mbedtls_ssl_check_srtp_profile_value(ssl->conf->dtls_srtp_profile_list[protection_profiles_index]);
...
static int ssl_parse_renegotiation_info(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
    ...
    if (ssl->renego_status != MBEDTLS_SSL_INITIAL_HANDSHAKE) {
        ...
        if (len != 1 + ssl->verify_data_len * 2 || buf[0] != ssl->verify_data_len * 2 ||
            ...
            mbedtls_ct_memcmp(buf + 1 + ssl->verify_data_len, ssl->peer_verify_data, ssl->verify_data_len) != 0) {
...
static int ssl_parse_use_srtp_ext(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
    ...
    if (server_protection != MBEDTLS_TLS_SRTP_UNSET) {
        MBEDTLS_SSL_DEBUG_MSG(3, ("found srtp profile: %s", mbedtls_ssl_get_srtp_profile_as_string(server_protection)));
    }
    ...
    for (i = 0; i < ssl->conf->dtls_srtp_profile_list_len; i++) {
        if (server_protection == ssl->conf->dtls_srtp_profile_list[i]) {
            ...
            MBEDTLS_SSL_DEBUG_MSG(
                3, ("selected srtp profile: %s", mbedtls_ssl_get_srtp_profile_as_string(server_protection)));
            break;
        }
    }
...
static int ssl_parse_server_hello(mbedtls_ssl_context* ssl) {
    ...
    if (ssl->tls_version < ssl->conf->min_tls_version || ssl->tls_version > ssl->conf->max_tls_version) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("server version out of bounds -  min: [0x%x], server: [0x%x], max: [0x%x]",
    ...
    MBEDTLS_SSL_DEBUG_MSG(
        3, ("server hello, current time: %lu", ((unsigned long)buf[2] << 24) | ((unsigned long)buf[3] << 16) |
    ...
    if (mbedtls_ssl_validate_ciphersuite(ssl, suite_info, ssl->tls_version, ssl->tls_version) != 0) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello message"));
        mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE);
        return MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
    }
...
static int ssl_parse_server_ecdh_params(mbedtls_ssl_context* ssl, unsigned char** p, unsigned char* end) {
    ...
    *p += 2;
    ...
    if (mbedtls_ssl_check_curve_tls_id(ssl, tls_id) != 0) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("bad server key exchange message (ECDHE curve): %u", (unsigned)tls_id));
        return MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
    }

    /* Convert EC's TLS ID to PSA key type. */
    if (mbedtls_ssl_get_psa_curve_info_from_tls_id(tls_id, &key_type, &ec_bits) == PSA_ERROR_NOT_SUPPORTED) {
        return MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
    }
...
start_processing:
    ...
    if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
        ...
        if (mbedtls_ssl_get_pk_sigalg_and_md_alg_from_sig_alg(sig_alg, &pk_alg, &md_alg) != 0 &&
            !mbedtls_ssl_sig_alg_is_offered(ssl, sig_alg) && !mbedtls_ssl_sig_alg_is_supported(ssl, sig_alg)) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("bad server key exchange message"));
            mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER);
            return MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
        }
        ...
        if (!mbedtls_pk_can_do_psa(peer_pk, mbedtls_psa_alg_from_pk_sigalg(pk_alg, psa_hash_alg),
        ...
        if (pk_alg == MBEDTLS_PK_SIGALG_RSA_PSS) {
            ret = mbedtls_pk_verify_ext((mbedtls_pk_sigalg_t)pk_alg, peer_pk, md_alg, hash, hashlen, p, sig_len);
        }
        else
...
static int ssl_parse_certificate_request(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("got %s certificate request", ssl->handshake->client_auth ? "a" : "no"));
    ...
    for (size_t i = 0; i < sig_alg_len; i += 2) {
        MBEDTLS_SSL_DEBUG_MSG(3, ("Supported Signature Algorithm found: %02x %02x", sig_alg[i], sig_alg[i + 1]));
    }
#endif
    ...
    for (size_t i = 0, dni_len = 0; i < dn_len; i += 2 + dni_len) {
        ...
        if (dni_len > dn_len - i - 2 ||
            mbedtls_asn1_get_tag(&p, p + dni_len, &asn1_len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE) != 0 ||
...
static int ssl_write_client_key_exchange(mbedtls_ssl_context* ssl) {
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_RSA ||
        ...
        status = psa_export_public_key(handshake->xxdh_psa_privkey, own_pubkey, own_pubkey_max_len, &own_pubkey_len);
        if (status != PSA_SUCCESS) {
        ...
        status = psa_raw_key_agreement(PSA_ALG_ECDH, handshake->xxdh_psa_privkey, handshake->xxdh_psa_peerkey,
                                       handshake->xxdh_psa_peerkey_len, ssl->handshake->premaster,
        ...
        if (header_len + content_len_size + ssl->conf->psk_identity_len > MBEDTLS_SSL_OUT_CONTENT_LEN) {
        ...
        *p = (unsigned char)own_pubkey_len;
        ...
        status =
            psa_raw_key_agreement(PSA_ALG_ECDH, handshake->xxdh_psa_privkey, handshake->xxdh_psa_peerkey,
                                  handshake->xxdh_psa_peerkey_len, pms + zlen_size, pms_end - (pms + zlen_size), &zlen);
        ...
        ret = mbedtls_psa_ecjpake_write_round(&ssl->handshake->psa_pake_ctx, out_p, end_p - out_p, &content_len,
