...
static int ssl_parse_renegotiation_info(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
    ...
    defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ALLOWED_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
/*
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_supported_groups_ext(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
...
static int ssl_parse_ecjpake_kkpp(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
    ...
    if ((ret = mbedtls_psa_ecjpake_read_round(&ssl->handshake->psa_pake_ctx, buf, len, MBEDTLS_ECJPAKE_ROUND_ONE)) !=
        0) {
...
static int ssl_parse_use_srtp_ext(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t len) {
    ...
    for (j = 0; j < profile_length; j += 2) {
        ...
        if (client_protection != MBEDTLS_TLS_SRTP_UNSET) {
            MBEDTLS_SSL_DEBUG_MSG(
                3, ("found srtp profile: %s", mbedtls_ssl_get_srtp_profile_as_string(client_protection)));
        }
        else {
            continue;
        }
        ...
        for (i = 0; i < ssl->conf->dtls_srtp_profile_list_len; i++) {
            if (client_protection == ssl->conf->dtls_srtp_profile_list[i]) {
                ...
                MBEDTLS_SSL_DEBUG_MSG(
                    3, ("selected srtp profile: %s", mbedtls_ssl_get_srtp_profile_as_string(client_protection)));
                break;
            }
        }
    ...
    if (mki_length > MBEDTLS_TLS_SRTP_MAX_MKI_LENGTH || mki_length + profile_length + size_of_lengths != len) {
        mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR);
        return MBEDTLS_ERR_SSL_DECODE_ERROR;
    }
...
static int ssl_pick_cert(mbedtls_ssl_context* ssl, const mbedtls_ssl_ciphersuite_t* ciphersuite_info) {
    ...
    for (cur = list; cur != NULL; cur = cur->next) {
        ...
        key_type_matches =
            ((ssl->conf->f_async_sign_start != NULL || mbedtls_pk_can_do_psa(cur->key, pk_alg, pk_usage)) &&
             mbedtls_pk_can_do_psa(&cur->cert->pk, pk_alg, PSA_KEY_USAGE_VERIFY_HASH));
#else
        key_type_matches = (mbedtls_pk_can_do_psa(cur->key, pk_alg, pk_usage));
         ...
         */
        if (mbedtls_ssl_check_cert_usage(cur->cert, ciphersuite_info, MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_VERSION_TLS1_2,
                                         &flags) != 0) {
...
#if defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ALLOWED_ENABLED)
        if (pk_alg == MBEDTLS_PK_ECDSA && ssl_check_key_curve(&cur->cert->pk, ssl->handshake->curves_tls_id) != 0) {
...
static int ssl_ciphersuite_match(mbedtls_ssl_context* ssl,
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("trying ciphersuite: %#04x (%s)", (unsigned int)suite_id, suite_info->name));

    if (suite_info->min_tls_version > ssl->tls_version || suite_info->max_tls_version < ssl->tls_version) {
    ...
    if (sig_type != MBEDTLS_PK_SIGALG_NONE &&
        mbedtls_ssl_tls12_get_preferred_hash_for_sig_alg(ssl, mbedtls_ssl_sig_from_pk_alg(sig_type)) ==
...
read_record_header:
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, message len.: %d", MBEDTLS_GET_UINT16_BE(ssl->in_len, 0)));
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        }
        {
            ...
            MBEDTLS_SSL_DEBUG_MSG(4, ("fragment_offset=%u fragment_length=%u length=%u", (unsigned)fragment_offset,
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (ssl->conf->f_cookie_check != NULL
            ...
            if (ssl->conf->f_cookie_check(ssl->conf->p_cookie, buf + cookie_offset + 1, cookie_len, ssl->cli_id,
                                          ssl->cli_id_len) != 0) {
    ...
    if (!sig_hash_alg_ext_present) {
        ...
        MBEDTLS_STATIC_ASSERT(sizeof(default_sig_algs) / sizeof(default_sig_algs[0]) <= MBEDTLS_RECEIVED_SIG_ALGS_SIZE,
...
have_ciphersuite:
    ...
    if (sig_alg != MBEDTLS_PK_SIGALG_NONE) {
        unsigned int sig_hash =
            mbedtls_ssl_tls12_get_preferred_hash_for_sig_alg(ssl, mbedtls_ssl_sig_from_pk_alg(sig_alg));
...
static void ssl_write_ecjpake_kkpp_ext(mbedtls_ssl_context* ssl, unsigned char* buf, size_t* olen) {
    ...
    *olen = 0;
    ...
    ret = mbedtls_psa_ecjpake_write_round(&ssl->handshake->psa_pake_ctx, p + 2, (size_t)(end - p - 2), &kkpp_len,
...
static void ssl_write_use_srtp_ext(mbedtls_ssl_context* ssl, unsigned char* buf, size_t* olen) {
    ...
    *olen = 0;
    ...
    buf[4] = 0x00;
    buf[5] = 0x02;
    profile_value = mbedtls_ssl_check_srtp_profile_value(ssl->dtls_srtp_info.chosen_dtls_srtp_profile);
...
static int ssl_write_hello_verify_request(mbedtls_ssl_context* ssl) {
    ...
    if ((ret = ssl->conf->f_cookie_write(ssl->conf->p_cookie, &p, ssl->out_buf + MBEDTLS_SSL_OUT_BUFFER_LEN,
...
static int ssl_write_server_hello(mbedtls_ssl_context* ssl) {
...
#if defined(MBEDTLS_SSL_DTLS_HELLO_VERIFY)
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM && ssl->handshake->cookie_verify_result != 0) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, current time: %" MBEDTLS_PRINTF_LONGLONG, (long long)t));
#else
    ...
    if (mbedtls_ssl_conf_is_tls13_enabled(ssl->conf)) {
        static const unsigned char magic_tls12_downgrade_string[] = {'D', 'O', 'W', 'N', 'G', 'R', 'D', 1};
    ...
    if (ssl->handshake->resume == 0) {
...
#endif /* MBEDTLS_SSL_SESSION_TICKETS */
        {
            ...
            if ((ret = psa_generate_random(ssl->session_negotiate->id, n)) != 0) {
                return ret;
            }
        }
    }
    ...
    *p++ = MBEDTLS_BYTE_0(MBEDTLS_SSL_COMPRESS_NULL);
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, compress alg.: 0x%02X", (unsigned int)MBEDTLS_SSL_COMPRESS_NULL));
...
#if defined(MBEDTLS_KEY_EXCHANGE_SOME_ECDH_OR_ECDHE_1_2_ENABLED) || \
    defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ALLOWED_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
    const mbedtls_ssl_ciphersuite_t* suite = mbedtls_ssl_ciphersuite_from_id(ssl->session_negotiate->ciphersuite);
...
static int ssl_prepare_server_key_exchange(mbedtls_ssl_context* ssl, size_t* signature_len) {
    ...
    curve_matching_done:
        ...
        MBEDTLS_SSL_DEBUG_MSG(2, ("ECDHE curve: %s", mbedtls_ssl_get_curve_name_from_tls_id(*curr_tls_id)));
        ...
        if (mbedtls_ssl_get_psa_curve_info_from_tls_id(*curr_tls_id, &key_type, &ec_bits) == PSA_ERROR_NOT_SUPPORTED) {
        ...
        *p++ = MBEDTLS_ECP_TLS_NAMED_CURVE;
        ...
        size_t own_pubkey_max_len = (size_t)(MBEDTLS_SSL_OUT_CONTENT_LEN - (own_pubkey - ssl->out_msg));

        status = psa_export_public_key(handshake->xxdh_psa_privkey, own_pubkey, own_pubkey_max_len, &len);
        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
    ...
    if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
        ...
        mbedtls_pk_sigalg_t sig_alg = mbedtls_ssl_get_ciphersuite_sig_pk_alg(ciphersuite_info);

        unsigned char sig_hash =
            (unsigned char)mbedtls_ssl_tls12_get_preferred_hash_for_sig_alg(ssl, mbedtls_ssl_sig_from_pk_alg(sig_alg));
         ...
         * ssl_write_server_key_exchange also takes care of incrementing
         * ssl->out_msglen. */
        if ((ret = mbedtls_pk_sign_ext(sig_alg, mbedtls_ssl_own_key(ssl), md_alg, hash, hashlen,
...
static int ssl_parse_client_psk_identity(mbedtls_ssl_context* ssl, unsigned char** p, const unsigned char* end) {
    ...
    *p += 2;
    ...
    if (ssl->conf->f_psk != NULL) {
        ...
        if (n != ssl->conf->psk_identity_len || mbedtls_ct_memcmp(ssl->conf->psk_identity, *p, n) != 0) {
            ret = MBEDTLS_ERR_SSL_UNKNOWN_IDENTITY;
        }
    }
...
static int ssl_parse_client_key_exchange(mbedtls_ssl_context* ssl) {
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_RSA ||
        ...
        if (data_len < 1 || data_len > buf_len) {
            MBEDTLS_SSL_DEBUG_MSG(
                1, ("Invalid data length: %" MBEDTLS_PRINTF_SIZET " > %" MBEDTLS_PRINTF_SIZET, data_len, buf_len));
            return MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
        }

        /* Store peer's ECDH public key. */
        if (data_len > sizeof(handshake->xxdh_psa_peerkey)) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("Invalid public key length: %" MBEDTLS_PRINTF_SIZET " > %" MBEDTLS_PRINTF_SIZET,
        ...
        status = psa_raw_key_agreement(PSA_ALG_ECDH, handshake->xxdh_psa_privkey, handshake->xxdh_psa_peerkey,
                                       handshake->xxdh_psa_peerkey_len, handshake->premaster,
        ...
        /* Compute ECDH shared secret. */
        status =
            psa_raw_key_agreement(PSA_ALG_ECDH, handshake->xxdh_psa_privkey, handshake->xxdh_psa_peerkey,
                                  handshake->xxdh_psa_peerkey_len, psm + zlen_size, psm_end - (psm + zlen_size), &zlen);
...
#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
        if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE) {
        if ((ret = mbedtls_psa_ecjpake_read_round(&ssl->handshake->psa_pake_ctx, p, (size_t)(end - p),
...
static int ssl_parse_certificate_verify(mbedtls_ssl_context* ssl) {
     ...
     * Signature
     */
    if ((pk_alg = mbedtls_ssl_pk_sig_alg_from_sig(ssl->in_msg[i])) == MBEDTLS_PK_SIGALG_NONE) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("peer not adhering to requested sig_alg"
                                  " for verify message"));
        return MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
    }
    ...
    if ((ret = mbedtls_pk_verify_ext(pk_alg, peer_pk, md_alg, hash_start, hashlen, ssl->in_msg + i, sig_len)) != 0) {
...
#endif
    if ((ret = ssl->conf->f_ticket_write(ssl->conf->p_ticket, ssl->session_negotiate, ssl->out_msg + 10,
                                         ssl->out_msg + MBEDTLS_SSL_OUT_CONTENT_LEN, &tlen, &lifetime)) != 0) {
