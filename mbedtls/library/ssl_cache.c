...
static int ssl_cache_find_entry(mbedtls_ssl_cache_context* cache,
    ...
    for (cur = cache->chain; cur != NULL; cur = cur->next) {
        ...
        if (session_id_len != cur->session_id_len || memcmp(session_id, cur->session_id, cur->session_id_len) != 0) {
            continue;
        }
...
int mbedtls_ssl_cache_set(void* data,
    ...
    ret = mbedtls_ssl_session_save(session, session_serialized, session_serialized_len, &session_serialized_len);
    if (ret != 0) {
        goto exit;
    }
