...
const uint8_t mbedtls_ssl_tls13_hello_retry_request_magic[MBEDTLS_SERVER_HELLO_RANDOM_LEN] = {
...
static void ssl_tls13_create_verify_structure(const unsigned char* transcript_hash,
    ...
    if (from == MBEDTLS_SSL_IS_CLIENT) {
        memcpy(verify_buffer + idx, mbedtls_ssl_tls13_labels.client_cv, MBEDTLS_SSL_TLS1_3_LBL_LEN(client_cv));
        idx += MBEDTLS_SSL_TLS1_3_LBL_LEN(client_cv);
    }
    else { /* from == MBEDTLS_SSL_IS_SERVER */
        memcpy(verify_buffer + idx, mbedtls_ssl_tls13_labels.server_cv, MBEDTLS_SSL_TLS1_3_LBL_LEN(server_cv));
...
static int ssl_tls13_parse_certificate_verify(mbedtls_ssl_context* ssl,
    ...
    if (mbedtls_ssl_get_pk_sigalg_and_md_alg_from_sig_alg(algorithm, &sig_alg, &md_alg) != 0) {
        goto error;
    }
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("Certificate Verify: Signature algorithm ( %04x )", (unsigned int)algorithm));

    /*
     * Check the certificate's key type matches the signature alg
     */
    if (!mbedtls_pk_can_do_psa(&ssl->session_negotiate->peer_cert->pk,
                               mbedtls_psa_alg_from_pk_sigalg(sig_alg, hash_alg), PSA_KEY_USAGE_VERIFY_HASH)) {
    ...
    status = psa_hash_compute(hash_alg, verify_buffer, verify_buffer_len, verify_hash, sizeof(verify_hash),
                              &verify_hash_len);
    ...
    if ((ret = mbedtls_pk_verify_ext((mbedtls_pk_sigalg_t)sig_alg, &ssl->session_negotiate->peer_cert->pk, md_alg,
                                     verify_hash, verify_hash_len, p, signature_len)) == 0) {
        return 0;
    }
...
int mbedtls_ssl_tls13_process_certificate_verify(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_tls13_fetch_handshake_msg(ssl, MBEDTLS_SSL_HS_CERTIFICATE_VERIFY, &buf, &buf_len));

    /* Need to calculate the hash of the transcript first
     * before reading the message since otherwise it gets
     * included in the transcript
     */
    ret = mbedtls_ssl_get_handshake_transcript(ssl, (mbedtls_md_type_t)ssl->handshake->ciphersuite_info->mac,
                                               transcript, sizeof(transcript), &transcript_len);
    if (ret != 0) {
        MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_INTERNAL_ERROR, MBEDTLS_ERR_SSL_INTERNAL_ERROR);
        return ret;
    }
    ...
    ssl_tls13_create_verify_structure(
        transcript, transcript_len, verify_buffer, &verify_buffer_len,
        (ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT) ? MBEDTLS_SSL_IS_SERVER : MBEDTLS_SSL_IS_CLIENT);

    /* Process the message contents */
    MBEDTLS_SSL_PROC_CHK(ssl_tls13_parse_certificate_verify(ssl, buf, buf + buf_len, verify_buffer, verify_buffer_len));

    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_CERTIFICATE_VERIFY, buf, buf_len));
...
int mbedtls_ssl_tls13_parse_certificate(mbedtls_ssl_context* ssl, const unsigned char* buf, const unsigned char* end) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if ((ssl->session_negotiate->peer_cert = mbedtls_calloc(1, sizeof(mbedtls_x509_crt))) == NULL) {
        ...
        MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_INTERNAL_ERROR, MBEDTLS_ERR_SSL_ALLOC_FAILED);
    ...
    while (p < certificate_list_end) {
        ...
        switch (ret) {
            ...
            case MBEDTLS_ERR_X509_ALLOC_FAILED:
                MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_INTERNAL_ERROR, MBEDTLS_ERR_X509_ALLOC_FAILED);
                MBEDTLS_SSL_DEBUG_RET(1, " mbedtls_x509_crt_parse_der", ret);
                return ret;

            case MBEDTLS_ERR_X509_UNKNOWN_VERSION:
                MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_UNSUPPORTED_CERT, MBEDTLS_ERR_X509_UNKNOWN_VERSION);
                MBEDTLS_SSL_DEBUG_RET(1, " mbedtls_x509_crt_parse_der", ret);
                return ret;
        ...
        while (p < extensions_end) {
            ...
            ret = mbedtls_ssl_tls13_check_received_extension(ssl, MBEDTLS_SSL_HS_CERTIFICATE, extension_type,
    ...
    if (ssl->session_negotiate->peer_cert == NULL) {
        ...
        if (ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER) {
            ...
            if (authmode == MBEDTLS_SSL_VERIFY_OPTIONAL) {
                ...
                MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_NO_CERT, MBEDTLS_ERR_SSL_NO_CLIENT_CERTIFICATE);
        ...
        if (ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT) {
            MBEDTLS_SSL_PEND_FATAL_ALERT(MBEDTLS_SSL_ALERT_MSG_NO_CERT, MBEDTLS_ERR_SSL_FATAL_ALERT_MESSAGE);
            return MBEDTLS_ERR_SSL_FATAL_ALERT_MESSAGE;
        }
#endif /* MBEDTLS_SSL_CLI_C */
    }

    return mbedtls_ssl_verify_certificate(ssl, authmode, ssl->session_negotiate->peer_cert, NULL, NULL);
}
...
int mbedtls_ssl_tls13_process_certificate(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_tls13_fetch_handshake_msg(ssl, MBEDTLS_SSL_HS_CERTIFICATE, &buf, &buf_len));
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_CERTIFICATE, buf, buf_len));
...
int mbedtls_ssl_tls13_write_certificate(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_start_handshake_msg(ssl, MBEDTLS_SSL_HS_CERTIFICATE, &buf, &buf_len));

    MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_certificate_body(ssl, buf, buf + buf_len, &msg_len));

    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_CERTIFICATE, buf, msg_len));
...
static int ssl_tls13_write_certificate_verify_body(mbedtls_ssl_context* ssl,
    ...
    *out_len = 0;
    ...
    ssl_tls13_create_verify_structure(handshake_hash, handshake_hash_len, verify_buffer, &verify_buffer_len,
                                      ssl->conf->endpoint);
    ...
    for (; *sig_alg != MBEDTLS_TLS1_3_SIG_NONE; sig_alg++) {
        ...
        if (mbedtls_ssl_get_pk_sigalg_and_md_alg_from_sig_alg(*sig_alg, &pk_type, &md_alg) != 0) {
            return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
        }
        ...
        if ((ret = mbedtls_pk_sign_ext((mbedtls_pk_sigalg_t)pk_type, own_key, md_alg, verify_hash, verify_hash_len,
                                       p + 4, (size_t)(end - (p + 4)), &signature_len)) != 0) {
            MBEDTLS_SSL_DEBUG_MSG(2,
                                  ("CertificateVerify signature failed with %s", mbedtls_ssl_sig_alg_to_str(*sig_alg)));
        ...
        MBEDTLS_SSL_DEBUG_MSG(2, ("CertificateVerify signature with %s", mbedtls_ssl_sig_alg_to_str(*sig_alg)));
...
int mbedtls_ssl_tls13_write_certificate_verify(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_start_handshake_msg(ssl, MBEDTLS_SSL_HS_CERTIFICATE_VERIFY, &buf, &buf_len));

    MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_certificate_verify_body(ssl, buf, buf + buf_len, &msg_len));

    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_CERTIFICATE_VERIFY, buf, msg_len));
...
static int ssl_tls13_preprocess_finished_message(mbedtls_ssl_context* ssl) {
    ...
    ret = mbedtls_ssl_tls13_calculate_verify_data(
        ssl, ssl->handshake->state_local.finished_in.digest, sizeof(ssl->handshake->state_local.finished_in.digest),
...
static int ssl_tls13_parse_finished_message(mbedtls_ssl_context* ssl,
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "verify_data (self-computed):", expected_verify_data, expected_verify_data_len);
...
int mbedtls_ssl_tls13_process_finished_message(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_tls13_fetch_handshake_msg(ssl, MBEDTLS_SSL_HS_FINISHED, &buf, &buf_len));
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_FINISHED, buf, buf_len));
...
static int ssl_tls13_prepare_finished_message(mbedtls_ssl_context* ssl) {
    ...
    ret = mbedtls_ssl_tls13_calculate_verify_data(
        ssl, ssl->handshake->state_local.finished_out.digest, sizeof(ssl->handshake->state_local.finished_out.digest),
...
int mbedtls_ssl_tls13_write_finished_message(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_start_handshake_msg(ssl, MBEDTLS_SSL_HS_FINISHED, &buf, &buf_len));

    MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_finished_message_body(ssl, buf, buf + buf_len, &msg_len));

    MBEDTLS_SSL_PROC_CHK(mbedtls_ssl_add_hs_msg_to_checksum(ssl, MBEDTLS_SSL_HS_FINISHED, buf, msg_len));
...
int mbedtls_ssl_tls13_write_change_cipher_spec(mbedtls_ssl_context* ssl) {
    ...
    MBEDTLS_SSL_PROC_CHK(ssl_tls13_write_change_cipher_spec_body(
        ssl, ssl->out_msg, ssl->out_msg + MBEDTLS_SSL_OUT_CONTENT_LEN, &ssl->out_msglen));
...
int mbedtls_ssl_tls13_write_early_data_ext(mbedtls_ssl_context* ssl,
    ...
    *out_len = 0;
    ...
    if (in_new_session_ticket) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(4, ("Sent max_early_data_size=%u", (unsigned int)ssl->conf->max_early_data_size));
    }
#endif
...
int mbedtls_ssl_tls13_check_early_data_len(mbedtls_ssl_context* ssl, size_t early_data_len) {
     ...
     * down.
     */
    if (early_data_len > (ssl->session_negotiate->max_early_data_size - ssl->total_early_data_size)) {
...
int mbedtls_ssl_reset_transcript_for_hrr(mbedtls_ssl_context* ssl) {
    ...
    ret = mbedtls_ssl_get_handshake_transcript(ssl, (mbedtls_md_type_t)ciphersuite_info->mac, hash_transcript + 4,
                                               PSA_HASH_MAX_SIZE, &hash_len);
    if (ret != 0) {
...
int mbedtls_ssl_tls13_read_public_xxdhe_share(mbedtls_ssl_context* ssl, const unsigned char* buf, size_t buf_len) {
    ...
    if (peerkey_len > sizeof(handshake->xxdh_psa_peerkey)) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("Invalid public key length: %u > %" MBEDTLS_PRINTF_SIZET, (unsigned)peerkey_len,
...
static psa_status_t mbedtls_ssl_get_psa_ffdh_info_from_tls_id(uint16_t tls_id, size_t* bits, psa_key_type_t* key_type) {
    switch (tls_id) {
...
int mbedtls_ssl_tls13_parse_record_size_limit_ext(mbedtls_ssl_context* ssl,
    ...
    if (extension_data_len != MBEDTLS_SSL_RECORD_SIZE_LIMIT_EXTENSION_DATA_LENGTH) {
        MBEDTLS_SSL_DEBUG_MSG(
            2, ("record_size_limit extension has invalid length: %" MBEDTLS_PRINTF_SIZET " Bytes", extension_data_len));
