...
int LLVMFuzzerTestOneInput(const uint8_t* Data, size_t Size) {
    ...
    if (initialized == 0) {
        ...
        if (mbedtls_x509_crt_parse(&cacert, (const unsigned char*)mbedtls_test_cas_pem, mbedtls_test_cas_pem_len) !=
            0) {
            return 1;
        }
#endif
    ...
    mbedtls_ssl_conf_session_tickets(
        &conf, (options & 0x100) ? MBEDTLS_SSL_SESSION_TICKETS_DISABLED : MBEDTLS_SSL_SESSION_TICKETS_ENABLED);
#endif
