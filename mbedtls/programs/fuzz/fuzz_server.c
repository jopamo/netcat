...
int LLVMFuzzerTestOneInput(const uint8_t* Data, size_t Size) {
    ...
    mbedtls_ssl_conf_cert_req_ca_list(
        &conf, (options & 0x1) ? MBEDTLS_SSL_CERT_REQ_CA_LIST_ENABLED : MBEDTLS_SSL_CERT_REQ_CA_LIST_DISABLED);
    ...
    mbedtls_ssl_conf_extended_master_secret(
        &conf, (options & 0x10) ? MBEDTLS_SSL_EXTENDED_MS_DISABLED : MBEDTLS_SSL_EXTENDED_MS_ENABLED);
#endif
    ...
    mbedtls_ssl_conf_renegotiation(
        &conf, (options & 0x80) ? MBEDTLS_SSL_RENEGOTIATION_ENABLED : MBEDTLS_SSL_RENEGOTIATION_DISABLED);
#endif
