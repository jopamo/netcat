...
#define CHECK_PADDING_SIGNED(TYPE, NAME)                                                \
    do {                                                                                \
        if (sizeof(TYPE) == 2 || sizeof(TYPE) == 4 || sizeof(TYPE) == 8) {              \
            if ((sizeof(TYPE) == 2 && (int16_t)NAME##_MAX != 0x7FFF) ||                 \
                (sizeof(TYPE) == 4 && (int32_t)NAME##_MAX != 0x7FFFFFFF) ||             \
                (sizeof(TYPE) == 8 && (int64_t)NAME##_MAX != 0x7FFFFFFFFFFFFFFF)) {     \
            ...
            mbedtls_printf(                                                             \
                "Padding checks only implemented for types of size 2, 4 or 8"           \
...
#define CHECK_PADDING_UNSIGNED(TYPE, NAME)                                       \
    do {                                                                         \
        if ((sizeof(TYPE) == 2 && (uint16_t)NAME##_MAX != 0xFFFF) ||             \
            (sizeof(TYPE) == 4 && (uint32_t)NAME##_MAX != 0xFFFFFFFF) ||         \
            (sizeof(TYPE) == 8 && (uint64_t)NAME##_MAX != 0xFFFFFFFFFFFFFFFF)) { \
            mbedtls_printf("Type '" #TYPE "' has padding bits\n");               \
            mbedtls_exit(MBEDTLS_EXIT_FAILURE);                                  \
        }                                                                        \
    } while (0)
