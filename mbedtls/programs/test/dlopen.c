...
#pragma GCC diagnostic ignored "-Wpedantic"
    const int* (*ssl_list_ciphersuites)(void) = dlsym(tls_so, "mbedtls_ssl_list_ciphersuites");
#pragma GCC diagnostic pop
    ...
    const mbedtls_x509_crt_profile* profile = dlsym(x509_so, "mbedtls_x509_crt_profile_default");
...
#pragma GCC diagnostic push
     ...
     * fine on platforms that have dlsym(). */
#pragma GCC diagnostic ignored "-Wpedantic"
    psa_status_t (*dyn_psa_crypto_init)(void) = dlsym(crypto_so, "psa_crypto_init");
    psa_status_t (*dyn_psa_hash_compute)(psa_algorithm_t, const uint8_t*, size_t, uint8_t*, size_t, size_t*) =
...
#pragma GCC diagnostic pop
    ...
    status = dyn_psa_hash_compute(PSA_ALG_SHA_256, input, sizeof(input) - 1, hash, sizeof(hash), &hash_len);
    if (status != PSA_SUCCESS) {
