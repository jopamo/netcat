...
static int ctx_buffer_flush(ctx_buffer* buf) {
    ...
    mbedtls_printf("  %05u flush    %s: %u bytes, %u datagrams, last %u ms\n", elapsed_time(), buf->description,
                   (unsigned)buf->len, buf->num_datagrams, elapsed_time() - buf->packet_lifetime);
...
static int ctx_buffer_append(ctx_buffer* buf, const unsigned char* data, size_t len) {
    int ret;
...
static int dispatch_data(mbedtls_net_context* ctx, const unsigned char* data, size_t len) {
    int ret;
#if defined(MBEDTLS_TIMING_C)
    ctx_buffer* buf = NULL;
    if (opt.pack > 0) {
...
static int send_packet(const packet* p, const char* why) {
    ...
    if (opt.inject_clihlo != 0 && inject_clihlo_state == ICH_INIT && strcmp(p->type, "ClientHello") == 0) {
        memcpy(&initial_clihlo, p, sizeof(packet));
        inject_clihlo_state = ICH_CACHED;
    }

    /* insert corrupted CID record? */
    if (opt.bad_cid != 0 && strcmp(p->type, "CID") == 0 && (rand() % opt.bad_cid) == 0) {
        unsigned char buf[MAX_MSG_SIZE];
    ...
    if (opt.duplicate != 0 && strcmp(p->type, "ApplicationData") != 0 && rand() % opt.duplicate == 0) {
    ...
    if (opt.inject_clihlo != 0 && inject_clihlo_state == ICH_CACHED && strcmp(p->type, "ApplicationData") == 0) {
        print_packet(&initial_clihlo, "injected");

        if ((ret = dispatch_data(dst, initial_clihlo.buf, initial_clihlo.len)) <= 0) {
            mbedtls_printf("  ! dispatch returned %d\n", ret);
            return ret;
        }
...
static int handle_message(const char* way, mbedtls_net_context* dst, mbedtls_net_context* src) {
    int ret;
    packet cur;
    size_t id;
    ...
    if ((opt.mtu != 0 && cur.len > (unsigned)opt.mtu) ||
        (opt.drop != 0 && strcmp(cur.type, "CID") != 0 && strcmp(cur.type, "ApplicationData") != 0 &&
         !(opt.protect_hvr && strcmp(cur.type, "HelloVerifyRequest") == 0) && cur.len != (size_t)opt.protect_len &&
         held[id] < HOLD_MAX && rand() % opt.drop == 0)) {
        ++held[id];
    }
    else if ((opt.delay_ccs == 1 && strcmp(cur.type, "ChangeCipherSpec") == 0) ||
             (opt.delay != 0 && strcmp(cur.type, "CID") != 0 && strcmp(cur.type, "ApplicationData") != 0 &&
              !(opt.protect_hvr && strcmp(cur.type, "HelloVerifyRequest") == 0) && cur.len != (size_t)opt.protect_len &&
              held[id] < HOLD_MAX && rand() % opt.delay == 0)) {
        ++held[id];
        delay_packet(&cur);
    }
    else {
...
int main(int argc, char* argv[]) {
     ...
     */
    mbedtls_printf("  . Connect to server on UDP/%s/%s ...", opt.server_addr, opt.server_port);
    fflush(stdout);

    if ((ret = mbedtls_net_connect(&server_fd, opt.server_addr, opt.server_port, MBEDTLS_NET_PROTO_UDP)) != 0) {
     ...
     */
    mbedtls_printf("  . Bind on UDP/%s/%s ...", opt.listen_addr, opt.listen_port);
    fflush(stdout);

    if ((ret = mbedtls_net_bind(&listen_fd, opt.listen_addr, opt.listen_port, MBEDTLS_NET_PROTO_UDP)) != 0) {
