...
#if !defined(MBEDTLS_X509_CSR_WRITE_C) || !defined(MBEDTLS_X509_CRT_PARSE_C) || !defined(MBEDTLS_PK_PARSE_C) || \
    !defined(PSA_WANT_ALG_SHA_256) || !defined(MBEDTLS_PEM_WRITE_C) || !defined(MBEDTLS_FS_IO) ||               \
    !defined(MBEDTLS_MD_C)
int main(void) {
    mbedtls_printf(
        "MBEDTLS_X509_CSR_WRITE_C and/or MBEDTLS_FS_IO and/or "
...
int main(int argc, char* argv[]) {
    ...
    if (opt.san_list != NULL) {
        ...
        if (ret != 0) {
            mbedtls_printf(" failed\n  !  mbedtls_x509write_csr_set_subject_alternative_name returned %d", ret);
            goto exit;
        }
    }
