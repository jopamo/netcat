...
#if !defined(MBEDTLS_X509_CRT_WRITE_C) || !defined(MBEDTLS_X509_CRT_PARSE_C) || !defined(MBEDTLS_FS_IO) || \
    !defined(MBEDTLS_ERROR_C) || !defined(PSA_WANT_ALG_SHA_256) || !defined(MBEDTLS_PEM_WRITE_C) ||        \
...
typedef enum { SERIAL_FRMT_UNSPEC, SERIAL_FRMT_DEC, SERIAL_FRMT_HEX } serial_format_t;
...
static int parse_serial_decimal_format(unsigned char* obuf, size_t obufmax, const char* ibuf, size_t* len) {
...
int main(int argc, char* argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        *q++ = '\0';
        ...
        if (strcmp(p, "request_file") == 0) {
            ...
            if (opt.authority_identifier != 0 && opt.authority_identifier != 1) {
            ...
            while (q != NULL) {
                ...
                if ((subtype_value = strchr(q, ':')) != NULL) {
                    *subtype_value++ = '\0';
                    ...
                    mbedtls_printf("Invalid argument for option SAN: Entry must be of the form TYPE:value\n");
                    goto usage;
                }
                if (strcmp(q, "RFC822") == 0) {
                    ...
                    if ((ret = mbedtls_x509_string_to_names(&tmp_san_dirname, subtype_value)) != 0) {
    ...
    if (serial_frmt == SERIAL_FRMT_HEX) {
        ret = mbedtls_test_unhexify(serial, sizeof(serial), opt.serial_hex, &serial_len);
    }
    ...
    if (!opt.selfsign && !strlen(opt.request_file)) {
        ...
        ret = mbedtls_pk_parse_keyfile(&loaded_subject_key, opt.subject_key, opt.subject_pwd);
        if (ret != 0) {
            mbedtls_strerror(ret, buf, sizeof(buf));
            mbedtls_printf(
    ...
    if (opt.san_list != NULL) {
        ...
        if (ret != 0) {
            mbedtls_printf(" failed\n  !  mbedtls_x509write_crt_set_subject_alternative_name returned %d", ret);
            goto exit;
        }
    }
