...
static int ssl_check_record(mbedtls_ssl_context const* ssl, unsigned char const* buf, size_t len) {
...
static int recv_timeout_cb(void* ctx, unsigned char* buf, size_t len, uint32_t timeout) {
    io_ctx_t* io_ctx = (io_ctx_t*)ctx;
    int ret;
    size_t recv_len;
...
#define MBEDTLS_SSL_SIG_ALG(hash) \
    ((hash << 8) | MBEDTLS_SSL_SIG_ECDSA), ((hash << 8) | MBEDTLS_SSL_SIG_RSA), (0x800 | hash),
#else
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
/* See above */
#define MBEDTLS_SSL_SIG_ALG(hash) ((hash << 8) | MBEDTLS_SSL_SIG_RSA), (0x800 | hash),
#else
 ...
 *  for more info.
 */
static int x509_crt_verify_info(char* buf, size_t size, const char* prefix, uint32_t flags) {
