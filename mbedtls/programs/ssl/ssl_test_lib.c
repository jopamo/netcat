...
int rng_seed(int reproducible, const char* pers) {
    ...
    (void)pers;
    if (reproducible) {
        mbedtls_fprintf(stderr, "The PSA RNG does not support reproducible mode.\n");
        return -1;
    }
    return 0;
}
...
int key_opaque_alg_parse(const char* arg, const char** alg1, const char** alg2) {
    ...
    *alg2 = separator + 1;
    ...
    if (strcmp(*alg1, "rsa-sign-pkcs1") != 0 && strcmp(*alg1, "rsa-sign-pss") != 0 &&
        strcmp(*alg1, "rsa-sign-pss-sha256") != 0 && strcmp(*alg1, "rsa-sign-pss-sha384") != 0 &&
        strcmp(*alg1, "rsa-sign-pss-sha512") != 0 && strcmp(*alg1, "ecdsa-sign") != 0 && strcmp(*alg1, "ecdh") != 0) {
        return 1;
    }

    if (strcmp(*alg2, "rsa-sign-pkcs1") != 0 && strcmp(*alg2, "rsa-sign-pss") != 0 &&
        strcmp(*alg1, "rsa-sign-pss-sha256") != 0 && strcmp(*alg1, "rsa-sign-pss-sha384") != 0 &&
        strcmp(*alg1, "rsa-sign-pss-sha512") != 0 && strcmp(*alg2, "ecdsa-sign") != 0 && strcmp(*alg2, "ecdh") != 0 &&
        strcmp(*alg2, "none") != 0) {
        return 1;
    }
...
int key_opaque_set_alg_usage(const char* alg1,
                             const char* alg2,
...
int pk_wrap_as_opaque(mbedtls_pk_context* pk,
...
int ca_callback(void* data, mbedtls_x509_crt const* child, mbedtls_x509_crt** candidates) {
    int ret = 0;
...
#endif
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED) && defined(PSA_WANT_ALG_FFDH)
...
int parse_groups(const char* groups, uint16_t* group_list, size_t group_list_len) {
    ...
    if (strcmp(p, "none") == 0) {
        ...
        if (i == group_list_len - 1 && *p != '\0') {
            mbedtls_printf("groups list too long, maximum %u", (unsigned int)(group_list_len - 1));
            return -1;
        }
