...
#if !defined(MBEDTLS_NET_C) || !defined(MBEDTLS_SSL_SRV_C) || !defined(MBEDTLS_PEM_PARSE_C) || \
...
int main(void) {
    ...
    while (1) {
        ...
        if ((ret = mbedtls_ssl_setup(&ssl, &conf)) != 0) {
            mbedtls_printf("pid %d: SSL setup failed!  mbedtls_ssl_setup returned %d\n\n", pid, ret);
            goto exit;
        }
        ...
        while ((ret = mbedtls_ssl_handshake(&ssl)) != 0) {
            if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
                mbedtls_printf("pid %d: SSL handshake failed!  mbedtls_ssl_handshake returned %d\n\n", pid, ret);
                goto exit;
            }
        }
        ...
        while (cnt++ < 10) {
            while ((ret = mbedtls_ssl_write(&ssl, buf, len)) <= 0) {
                ...
                if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
                    mbedtls_printf("pid %d: Write failed!  mbedtls_ssl_write returned %d\n\n", pid, ret);
                    goto exit;
                }
            }
            len = ret;
