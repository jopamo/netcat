...
#if !defined(MBEDTLS_BIGNUM_C) || !defined(MBEDTLS_ENTROPY_C) || !defined(MBEDTLS_SSL_TLS_C) || \
    !defined(MBEDTLS_SSL_CLI_C) || !defined(MBEDTLS_NET_C) || !defined(MBEDTLS_RSA_C) ||        \
...
#else
#define USAGE_AUTH "    authentication options disabled. (Require MBEDTLS_BASE64_C)\n"
...
int main(int argc, char* argv[]) {
     ...
     */
    mbedtls_printf("  . Connecting to tcp/%s/%s...", opt.server_name, opt.server_port);
    fflush(stdout);
    ...
    if (opt.authentication) {
        ...
        ret = mbedtls_base64_encode(base, sizeof(base), &n, (const unsigned char*)opt.user_name, strlen(opt.user_name));
        ...
        ret = mbedtls_base64_encode(base, sizeof(base), &n, (const unsigned char*)opt.user_pwd, strlen(opt.user_pwd));
