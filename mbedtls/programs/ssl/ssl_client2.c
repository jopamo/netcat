...
#define USAGE_ECRESTART "    ec_max_ops=%%s       default: library default (restart disabled)\n"
#else
...
#define USAGE3                                                                                             \
    ...
    "                        default: 1\n"                                                                 \
    "    reconnect_hard=%%d   default: 0 (disabled)\n" USAGE_TICKETS USAGE_EAP_TLS USAGE_MAX_FRAG_LEN      \
        USAGE_CONTEXT_CRT_CB USAGE_ALPN USAGE_EMS USAGE_ETM USAGE_REPRODUCIBLE USAGE_GROUPS USAGE_SIG_ALGS \
            USAGE_EARLY_DATA USAGE_KEY_OPAQUE_ALGS "\n"
...
static int report_cid_usage(mbedtls_ssl_context* ssl, const char* additional_description) {
    ...
    if (cid_negotiated == MBEDTLS_SSL_CID_DISABLED) {
        ...
        ret = mbedtls_ssl_get_peer_cid(ssl, &cid_negotiated, NULL, &peer_cid_len);
        if (ret != 0) {
            mbedtls_printf(" failed\n  ! mbedtls_ssl_get_peer_cid returned -0x%x\n\n", (unsigned int)-ret);
            return ret;
        }
...
static int ssl_save_session_serialize(mbedtls_ssl_context* ssl,
    ...
    *session_data = mbedtls_calloc(1, *session_data_len);
    ...
    if ((ret = mbedtls_ssl_session_save(&exported_session, *session_data, *session_data_len, session_data_len)) != 0) {
...
static int build_http_request(unsigned char* buf, size_t buf_size, size_t* request_len) {
    ...
    if (opt.request_size != DFL_REQUEST_SIZE && len + tail_len < request_size) {
    ...
    if (opt.request_size != DFL_REQUEST_SIZE && len > request_size) {
        len = request_size;
...
int main(int argc, char* argv[]) {
        ...
        *q++ = '\0';
        ...
        if (strcmp(p, "server_name") == 0) {
            ...
            *p++ = '\0';
            ...
            if (key_opaque_alg_parse(q, &opt.key_opaque_alg1, &opt.key_opaque_alg2) != 0) {
                goto usage;
            }
        }
        else {
    ...
    {
        ret =
            mbedtls_pk_parse_key(&pkey, (const unsigned char*)mbedtls_test_cli_key, mbedtls_test_cli_key_len, NULL, 0);
    }
    if (ret != 0) {
    ...
    if (opt.key_opaque != 0) {
        ...
        if (key_opaque_set_alg_usage(opt.key_opaque_alg1, opt.key_opaque_alg2, &psa_alg, &psa_alg2, &usage,
    ...
    mbedtls_printf(" ok (key type: %s)\n", strlen(opt.key_file) || strlen(opt.key_opaque_alg1)
                                               ? mbedtls_x509_pk_type_as_string(&pkey)
                                               : "none");
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */
    ...
    if ((ret = mbedtls_ssl_config_defaults(&conf, MBEDTLS_SSL_IS_CLIENT, opt.transport, MBEDTLS_SSL_PRESET_DEFAULT)) !=
        0) {
    ...
    if (opt.psk_opaque != 0) {
        ...
        if ((ret = mbedtls_ssl_conf_psk_opaque(&conf, slot, (const unsigned char*)opt.psk_identity,
        ...
        ret =
            mbedtls_ssl_conf_psk(&conf, psk, psk_len, (const unsigned char*)opt.psk_identity, strlen(opt.psk_identity));
        if (ret != 0) {
    ...
    if (opt.use_srtp != 0 && strlen(opt.mki) != 0) {
        ...
        if ((ret = mbedtls_ssl_dtls_srtp_set_mki_value(&ssl, mki, (uint16_t)strlen(opt.mki) / 2)) != 0) {
    ...
    mbedtls_printf("  . Connecting to %s/%s/%s...", opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM ? "tcp" : "udp",
    ...
    if (opt.reconnect != 0 && ssl.tls_version != MBEDTLS_SSL_VERSION_TLS1_3) {
        ...
        if (opt.reco_mode == 1) {
            if ((ret = ssl_save_session_serialize(&ssl, &session_data, &session_data_len)) != 0) {
...
send_request:
    ...
    buf[written] = '\0';
    mbedtls_printf(" %" MBEDTLS_PRINTF_SIZET " bytes written in %" MBEDTLS_PRINTF_SIZET " fragments\n\n%s\n", written,
                   frags, (char*)buf);
...
reconnect:
    if (opt.reconnect != 0) {
        ...
        if (opt.reco_mode == 1) {
            if ((ret = mbedtls_ssl_session_load(&saved_session, session_data, session_data_len)) != 0) {
        ...
        if (opt.reco_server_name != NULL && (ret = mbedtls_ssl_set_hostname(&ssl, opt.reco_server_name)) != 0) {
            mbedtls_printf(" failed\n  ! mbedtls_ssl_set_hostname returned %d\n\n", ret);
            goto exit;
        }
#endif
        ...
        if (ssl.conf->early_data_enabled == MBEDTLS_SSL_EARLY_DATA_ENABLED) {
            ...
            written = 0;
            do {
                while ((ret = mbedtls_ssl_write_early_data(&ssl, buf + written, len - written)) < 0) {
        ...
        end_of_early_data:
            ...
            buf[written] = '\0';
            mbedtls_printf(" %" MBEDTLS_PRINTF_SIZET " bytes of early data written in %" MBEDTLS_PRINTF_SIZET
                           " fragments\n\n%s\n",
    ...
    if (opt.psk_opaque != 0) {
        ...
        if ((status != PSA_SUCCESS) && (opt.query_config_mode == DFL_QUERY_CONFIG_MODE)) {
            mbedtls_printf("Failed to destroy key slot %u - error was %d",
                           (unsigned)MBEDTLS_SVC_KEY_ID_GET_KEY_ID(slot), (int)status);
            if (ret == 0) {
                ret = MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
            }
        }
