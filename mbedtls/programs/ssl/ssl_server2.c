...
#define USAGE_CID                                                                                               \
    ...
    "                       default: 0 (disabled)\n"                                                            \
    "    cid_renego=%%d      Disable (0) or enable (1) the use of the DTLS Connection ID extension during "     \
    "renegotiation.\n"                                                                                          \
...
#define USAGE_CACHE_TIME "    cache_timeout=%%d    default: cache default (1d)\n"
#else
...
#define USAGE_BADMAC_LIMIT "    badmac_limit=%%d     default: (library default: disabled)\n"
...
#define USAGE_REPRODUCIBLE "    reproducible=0/1     default: 0 (disabled)\n"
...
#define USAGE_EARLY_DATA ""
    ...
    (defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED) && defined(PSA_WANT_ALG_FFDH))
#define USAGE_GROUPS                                                          \
...
#define USAGE3                                                                                             \
    ...
    "\n" USAGE_TICKETS USAGE_EAP_TLS USAGE_REPRODUCIBLE USAGE_NSS_KEYLOG USAGE_NSS_KEYLOG_FILE USAGE_CACHE \
        USAGE_CACHE_TIME USAGE_MAX_FRAG_LEN USAGE_ALPN USAGE_EMS USAGE_ETM USAGE_GROUPS USAGE_SIG_ALGS     \
            USAGE_KEY_OPAQUE_ALGS USAGE_EARLY_DATA "\n"
...
static int cert_callback(mbedtls_ssl_context* ssl) {
    ...
    if (cur != NULL) {
        ...
        if (strlen(cur->name) != name_len || memcmp(cur->name, name, name_len) != 0) {
...
static psk_entry* psk_parse(char* psk_string) {
    ...
    *end = ',';
    ...
    while (p <= end) {
        ...
        if (mbedtls_test_unhexify(new->key, MBEDTLS_PSK_MAX_LEN, key_hex, &new->key_len) != 0) {
            goto error;
        }
 ...
 * PSK callback
 */
static int psk_callback(void* p_info, mbedtls_ssl_context* ssl, const unsigned char* name, size_t name_len) {
...
static int ssl_async_start(mbedtls_ssl_context* ssl,
                           ...
                           size_t input_len) {
    ssl_async_key_context_t* config_data = mbedtls_ssl_conf_get_async_config_data(ssl->conf);
    unsigned slot;
    ...
    for (slot = 0; slot < config_data->slots_used; slot++) {
        if (mbedtls_pk_check_pair(&cert->pk, config_data->slots[slot].pk) == 0) {
            break;
        }
    }
...
static int ssl_async_resume(mbedtls_ssl_context* ssl, unsigned char* output, size_t* output_len, size_t output_size) {
    ...
    switch (ctx->operation_type) {
        case ASYNC_OP_SIGN:
            ret =
                mbedtls_pk_sign(key_slot->pk, ctx->md_alg, ctx->input, ctx->input_len, output, output_size, output_len);
            break;
        default:
...
#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
static int report_cid_usage(mbedtls_ssl_context* ssl, const char* additional_description) {
    int ret;
    ...
    ret = mbedtls_ssl_get_peer_cid(ssl, &cid_negotiated, peer_cid, &peer_cid_len);
    if (ret != 0) {
    ...
    if (cid_negotiated == MBEDTLS_SSL_CID_DISABLED) {
        ...
        mbedtls_printf("(%s) Peer CID (length %u Bytes): ", additional_description, (unsigned)peer_cid_len);
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS) && defined(MBEDTLS_SSL_TICKET_C) && defined(MBEDTLS_HAVE_TIME)
static inline void put_unaligned_uint32(void* p, uint32_t x) {
    memcpy(p, &x, sizeof(x));
}

/* Functions for session ticket tests */
static int dummy_ticket_write(void* p_ticket,
    ...
    *ticket_lifetime = 7 * 24 * 3600;
    ...
    /* Dump session state */
    if ((ret = mbedtls_ssl_session_save(session, p, end - p, &clear_len)) != 0) {
        return ret;
    }
...
static int dummy_ticket_parse(void* p_ticket, mbedtls_ssl_session* session, unsigned char* buf, size_t len) {
    int ret;
    ((void)p_ticket);
    ...
    switch (opt.dummy_ticket % 11) {
        ...
        case 4:
            ...
            session->ticket_creation_time = mbedtls_ms_time() - (7 * 24 * 3600 * 1000 + 1000);
            break;
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
        case 5:
...
int main(int argc, char* argv[]) {
    ...
    const mbedtls_ssl_srtp_profile default_profiles[] = {
        ...
        MBEDTLS_TLS_SRTP_NULL_HMAC_SHA1_80, MBEDTLS_TLS_SRTP_NULL_HMAC_SHA1_32, MBEDTLS_TLS_SRTP_UNSET};
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (strcmp(p, "help_ciphersuites") == 0) {
            ...
            for (list = mbedtls_ssl_list_ciphersuites(); *list != 0; list++) {
        ...
        *q++ = '\0';
        ...
        if (strcmp(p, "server_port") == 0) {
            ...
            int n = atoi(q);
            if (n < -SSL_ASYNC_INJECT_ERROR_MAX || n > SSL_ASYNC_INJECT_ERROR_MAX) {
                ret = 2;
                goto usage;
            }
             ...
             */
                ...
                opt.tls13_kex_modes =
                    MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK | MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL;
            }
            else {
                goto usage;
            }
            ...
            if (strcmp(q, "tls12") == 0 || strcmp(q, "dtls12") == 0) {
            ...
            *p++ = '\0';
            ...
            if (key_opaque_alg_parse(q, &opt.key1_opaque_alg1, &opt.key1_opaque_alg2) != 0) {
                goto usage;
            }
        }
        else if (strcmp(p, "key_opaque_algs2") == 0) {
            if (key_opaque_alg_parse(q, &opt.key2_opaque_alg1, &opt.key2_opaque_alg2) != 0) {
                goto usage;
            }
        }
        else {
    ...
    if (opt.response_size != DFL_RESPONSE_SIZE && buf_content_size < (size_t)opt.response_size) {
    ...
    if (opt.psk_opaque != 0) {
        ...
        if (opt.force_ciphersuite[0] <= 0) {
            mbedtls_printf(
                "opaque PSKs are only supported in conjunction with forcing TLS 1.2 and a PSK-only ciphersuite through "
                "the 'force_ciphersuite' option.\n");
            ret = 2;
            goto usage;
        }
    }
    ...
    if (mbedtls_test_unhexify(cid_renego, sizeof(cid_renego), opt.cid_val_renego, &cid_renego_len) != 0) {
        mbedtls_printf("CID not valid hex\n");
        goto exit;
    }
#endif /* MBEDTLS_SSL_DTLS_CONNECTION_ID */
    ...
    if (opt.sig_algs != NULL) {
        ...
        if (i == (SIG_ALG_LIST_SIZE - 1) && *p != '\0') {
            mbedtls_printf("signature algorithm list too long, maximum %d", SIG_ALG_LIST_SIZE - 1);
            goto exit;
        }
...
#endif
    {
        ...
        if (ret == 0) {
            for (i = 0; mbedtls_test_cas_der[i] != NULL; i++) {
                ret = mbedtls_x509_crt_parse_der(&cacert, (const unsigned char*)mbedtls_test_cas_der[i],
    ...
    if (strlen(opt.key_file) && strcmp(opt.key_file, "none") != 0) {
        key_cert_init++;
        if ((ret = mbedtls_pk_parse_keyfile(&pkey, opt.key_file, opt.key_pwd)) != 0) {
    ...
    if (strlen(opt.key_file2) && strcmp(opt.key_file2, "none") != 0) {
        key_cert_init2++;
        if ((ret = mbedtls_pk_parse_keyfile(&pkey2, opt.key_file2, opt.key_pwd2)) != 0) {
...
#endif
    if (key_cert_init == 0 && strcmp(opt.crt_file, "none") != 0 && strcmp(opt.key_file, "none") != 0 &&
        key_cert_init2 == 0 && strcmp(opt.crt_file2, "none") != 0 && strcmp(opt.key_file2, "none") != 0) {
#if defined(MBEDTLS_RSA_C)
        if ((ret = mbedtls_x509_crt_parse(&srvcert, (const unsigned char*)mbedtls_test_srv_crt_rsa,
                                          mbedtls_test_srv_crt_rsa_len)) != 0) {
            mbedtls_printf(" failed\n  !  mbedtls_x509_crt_parse returned -0x%x\n\n", (unsigned int)-ret);
            goto exit;
        }
        if ((ret = mbedtls_pk_parse_key(&pkey, (const unsigned char*)mbedtls_test_srv_key_rsa,
        ...
        if ((ret = mbedtls_x509_crt_parse(&srvcert2, (const unsigned char*)mbedtls_test_srv_crt_ec,
                                          mbedtls_test_srv_crt_ec_len)) != 0) {
            mbedtls_printf(" failed\n  !  x509_crt_parse2 returned -0x%x\n\n", (unsigned int)-ret);
            goto exit;
        }
        if ((ret = mbedtls_pk_parse_key(&pkey2, (const unsigned char*)mbedtls_test_srv_key_ec,
    ...
    if (opt.key_opaque != 0) {
        ...
        if (key_opaque_set_alg_usage(opt.key1_opaque_alg1, opt.key1_opaque_alg2, &psa_alg, &psa_alg2, &psa_usage,
        ...
        if (key_opaque_set_alg_usage(opt.key2_opaque_alg1, opt.key2_opaque_alg2, &psa_alg, &psa_alg2, &psa_usage,
    ...
    if ((ret = mbedtls_ssl_config_defaults(&conf, MBEDTLS_SSL_IS_SERVER, opt.transport, MBEDTLS_SSL_PRESET_DEFAULT)) !=
        0) {
    ...
    if (opt.cid_enabled == 1 || opt.cid_enabled_renego == 1) {
        if (opt.cid_enabled == 1 && opt.cid_enabled_renego == 1 && cid_len != cid_renego_len) {
            mbedtls_printf("CID length must not change during renegotiation\n");
            goto usage;
        }

        if (opt.cid_enabled == 1) {
            ret = mbedtls_ssl_conf_cid(&conf, cid_len, MBEDTLS_SSL_UNEXPECTED_CID_IGNORE);
        }
        else {
            ret = mbedtls_ssl_conf_cid(&conf, cid_renego_len, MBEDTLS_SSL_UNEXPECTED_CID_IGNORE);
        }
    ...
    if (opt.use_srtp == 1) {
        ...
        mbedtls_ssl_conf_srtp_mki_value_supported(
            &conf, opt.support_mki ? MBEDTLS_SSL_DTLS_SRTP_MKI_SUPPORTED : MBEDTLS_SSL_DTLS_SRTP_MKI_UNSUPPORTED);
    }
    ...
    if (opt.tickets != MBEDTLS_SSL_SESSION_TICKETS_DISABLED) {
#if defined(MBEDTLS_HAVE_TIME)
        if (opt.dummy_ticket) {
            mbedtls_ssl_conf_session_tickets_cb(&conf, dummy_ticket_write, dummy_ticket_parse, NULL);
        }
        else
#endif /* MBEDTLS_HAVE_TIME */
        {
            if ((ret = mbedtls_ssl_ticket_setup(&ticket_ctx, opt.ticket_alg, opt.ticket_key_type, opt.ticket_key_bits,
        ...
        if (opt.ticket_rotate) {
            ...
            if ((ret = psa_generate_random(name, sizeof(name))) != 0 ||
                ...
                (ret = mbedtls_ssl_ticket_rotate(&ticket_ctx, name, sizeof(name), kbuf, sizeof(kbuf),
                                                 opt.ticket_timeout)) != 0) {
    ...
    if (key_cert_init) {
        ...
        if (opt.async_private_delay1 >= 0) {
            ret = ssl_async_set_key(&ssl_async_keys, &srvcert, pk, 0, opt.async_private_delay1);
            if (ret < 0) {
    ...
    if (key_cert_init2) {
        ...
        if (opt.async_private_delay2 >= 0) {
            ret = ssl_async_set_key(&ssl_async_keys, &srvcert2, pk, 0, opt.async_private_delay2);
            if (ret < 0) {
                mbedtls_printf("  Test error: ssl_async_set_key failed (%d)\n", ret);
                goto exit;
            }
    ...
    if (opt.async_operations[0] != '-') {
        ...
        ssl_async_keys.inject_error =
            (opt.async_private_error < 0 ? -opt.async_private_error : opt.async_private_error);
        mbedtls_ssl_conf_async_private_cb(&conf, sign, ssl_async_resume, ssl_async_cancel, &ssl_async_keys);
    }
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */
    ...
    if (opt.sni != NULL) {
        ...
        if (opt.async_private_delay2 >= 0) {
            ...
            for (cur = sni_info; cur != NULL; cur = cur->next) {
                ret = ssl_async_set_key(&ssl_async_keys, cur->cert, cur->key, 1, opt.async_private_delay2);
                if (ret < 0) {
                    mbedtls_printf("  Test error: ssl_async_set_key failed (%d)\n", ret);
                    goto exit;
                }
...
#if defined(PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY) || \
    (defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED) && defined(PSA_WANT_ALG_FFDH))
    if (opt.groups != NULL && strcmp(opt.groups, "default") != 0) {
    ...
    if (strlen(opt.psk) != 0 && strlen(opt.psk_identity) != 0) {
        if (opt.psk_opaque != 0) {
            ...
            if ((ret = mbedtls_ssl_conf_psk_opaque(&conf, psk_slot, (const unsigned char*)opt.psk_identity,
            ...
            ret = mbedtls_ssl_conf_psk(&conf, psk, psk_len, (const unsigned char*)opt.psk_identity,
    ...
    if (opt.psk_list != NULL) {
        if (opt.psk_list_opaque != 0) {
            ...
            for (cur_psk = psk_info; cur_psk != NULL; cur_psk = cur_psk->next) {
                status = psa_setup_psk_key_slot(&cur_psk->slot, alg, cur_psk->key, cur_psk->key_len);
                if (status != PSA_SUCCESS) {
                    ret = MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
                    goto exit;
                }
    ...
    if (opt.eap_tls != 0) {
        mbedtls_ssl_set_export_keys_cb(&ssl, eap_tls_key_derivation, &eap_tls_keying);
    }
        ...
        mbedtls_ssl_set_export_keys_cb(&ssl, dtls_srtp_key_derivation, &dtls_srtp_keying);
    }
#endif /* MBEDTLS_SSL_DTLS_SRTP */

    io_ctx.ssl = &ssl;
    io_ctx.net = &client_fd;
    mbedtls_ssl_set_bio(&ssl, &io_ctx, send_cb, recv_cb, opt.nbio == 0 ? recv_timeout_cb : NULL);

#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        if ((ret = mbedtls_ssl_set_cid(&ssl, opt.cid_enabled, cid, cid_len)) != 0) {
     ...
     */
    mbedtls_printf("  . Bind on %s://%s:%s/ ...", opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM ? "tcp" : "udp",
                   opt.server_addr ? opt.server_addr : "*", opt.server_port);
    fflush(stdout);

    if ((ret = mbedtls_net_bind(
             &listen_fd, opt.server_addr, opt.server_port,
             opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM ? MBEDTLS_NET_PROTO_TCP : MBEDTLS_NET_PROTO_UDP)) != 0) {
...
reset:
    ...
    if (opt.ecjpake_pw != DFL_ECJPAKE_PW) {
        if (opt.ecjpake_pw_opaque != DFL_ECJPAKE_PW_OPAQUE) {
            ...
            status = psa_import_key(&attributes, (const unsigned char*)opt.ecjpake_pw, strlen(opt.ecjpake_pw),
                                    &ecjpake_pw_slot);
            if (status != PSA_SUCCESS) {
                mbedtls_printf(" failed\n  ! psa_import_key returned %d\n\n", status);
                goto exit;
            }
            if ((ret = mbedtls_ssl_set_hs_ecjpake_password_opaque(&ssl, ecjpake_pw_slot)) != 0) {
            ...
            if ((ret = mbedtls_ssl_set_hs_ecjpake_password(&ssl, (const unsigned char*)opt.ecjpake_pw,
...
handshake:
...
#if defined(MBEDTLS_SSL_ASYNC_PRIVATE)
        if (ret == MBEDTLS_ERR_SSL_ASYNC_IN_PROGRESS && ssl_async_keys.inject_error == SSL_ASYNC_INJECT_ERROR_CANCEL) {
    ...
    if (ret == MBEDTLS_ERR_SSL_HELLO_VERIFY_REQUIRED) {
...
#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
        if (ret == MBEDTLS_ERR_X509_CERT_VERIFY_FAILED || ret == MBEDTLS_ERR_SSL_BAD_CERTIFICATE) {
            char vrfy_buf[512];
        ...
        mbedtls_printf(
            ...
            "    [ Ciphersuite is %s ]\n"
            "    [ Key size is %u ]\n",
            mbedtls_ssl_get_version(&ssl), mbedtls_ssl_ciphersuite_get_name(ciphersuite_info),
            (unsigned int)mbedtls_ssl_ciphersuite_get_cipher_key_bitlen(ciphersuite_info));
    }
    ...
    if (opt.eap_tls != 0) {
        ...
        if ((ret = mbedtls_ssl_tls_prf(eap_tls_keying.tls_prf_type, eap_tls_keying.master_secret,
        ...
        if ((ret = mbedtls_ssl_tls_prf(eap_tls_keying.tls_prf_type, NULL, 0, eap_tls_label, eap_tls_keying.randbytes,
                                       sizeof(eap_tls_keying.randbytes), eap_tls_iv, sizeof(eap_tls_iv))) != 0) {
            mbedtls_printf(" failed\n  ! mbedtls_ssl_tls_prf returned -0x%x\n\n", (unsigned int)-ret);
            goto reset;
        }
    ...
    if (opt.exp_label != NULL && opt.exp_len > 0) {
        ...
        ret = mbedtls_ssl_export_keying_material(&ssl, exported_key, (size_t)opt.exp_len, opt.exp_label,
                                                 strlen(opt.exp_label), NULL, 0, 0);
        if (ret != 0) {
        ...
        if (dtls_srtp_negotiation_result.chosen_dtls_srtp_profile == MBEDTLS_TLS_SRTP_UNSET) {
            mbedtls_printf(
                "    Unable to negotiate "
                "the use of DTLS-SRTP\n");
        }
        else {
            if ((ret = mbedtls_ssl_tls_prf(dtls_srtp_keying.tls_prf_type, dtls_srtp_keying.master_secret,
                                           sizeof(dtls_srtp_keying.master_secret), dtls_srtp_label,
                                           dtls_srtp_keying.randbytes, sizeof(dtls_srtp_keying.randbytes),
                                           dtls_srtp_key_material, sizeof(dtls_srtp_key_material))) != 0) {
                mbedtls_printf(" failed\n  ! mbedtls_ssl_tls_prf returned -0x%x\n\n", (unsigned int)-ret);
                goto exit;
            }
    ...
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        if ((ret = mbedtls_ssl_set_cid(&ssl, opt.cid_enabled_renego, cid_renego, cid_renego_len)) != 0) {
            mbedtls_printf(" failed\n  ! mbedtls_ssl_set_cid returned %d\n\n", ret);
            goto exit;
        }
    }
...
data_exchange:
    ...
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM) {
        ...
        do {
            ...
            /* For event-driven IO, wait for socket to become available */
            if (mbedtls_ssl_check_pending(&ssl) == 0 && opt.event == 1 /* level triggered IO */) {
#if defined(MBEDTLS_TIMING_C)
    ...
    if (opt.serialize != 0) {
        ...
        if (0 < strlen(opt.context_file)) {
            ...
            if ((ret = mbedtls_base64_encode(b64_buf, b64_len, &b64_len, context_buf, buf_len)) != 0) {
                mbedtls_printf(
...
exit:
    ...
    if (opt.psk_opaque != 0) {
        ...
        if ((status != PSA_SUCCESS) && (opt.query_config_mode == DFL_QUERY_CONFIG_MODE)) {
            mbedtls_printf("Failed to destroy key slot %u - error was %d",
                           (unsigned)MBEDTLS_SVC_KEY_ID_GET_KEY_ID(psk_slot), (int)status);
        }
    }
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_PSK_ENABLED */
    ...
    if ((opt.ecjpake_pw_opaque != DFL_ECJPAKE_PW_OPAQUE)) {
        ...
        if (psa_get_key_attributes(ecjpake_pw_slot, &check_attributes) != PSA_SUCCESS) {
            if (ret == 0) {
                ret = 1;
            }
