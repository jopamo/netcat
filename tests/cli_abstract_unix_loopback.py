...
def run_abstract_unix_loopback(nc_path: str) -> None:
    if sys.platform != "linux":
        print("Abstract unix sockets only supported on Linux")
        return

    abstract_path = "@nc_test_socket_" + str(os.getpid())
    
    # Server: -l -U [path] -v -N
    server_cmd = [nc_path, "-v", "-l", "-U", abstract_path, "-N"]
    ...
    # Client: -U [path] -N
    client_cmd = [nc_path, "-U", "-N", abstract_path]
...
def main() -> int:
    ...
    try:
        run_abstract_unix_loopback(args.nc_path)
