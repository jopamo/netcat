...
void arm_timer(num, secs) unsigned int num;
unsigned int secs;
{
    if (secs == 0) { /* reset */
        signal(SIGALRM, SIG_IGN);
...
void loadports(block, lo, hi) char* block;
USHORT lo;
USHORT hi;
{
    USHORT x;
...
void doexec_new(fd) int fd;
{
    ...
    Debug(("gonna exec \"%s\" using /bin/sh...", pr00gie)) execl("/bin/sh", "sh", "-c", pr00gie, NULL);
...
/* don't even bother compiling if we can't do IP options here! */
        ...
        if (!optbuf) { /* and don't already *have* a srcrt set */
            ...
            optbuf = Hmalloc(48);
            opp = optbuf;
            *opp++ = (unsigned char)IPOPT_LSRR;                        /* option */
...
dol_noop:
    ...
    if (rp)
        if (z != rp)
            x = 1;
    if (x) /* guilty! */
        bail("invalid connection to [%s] from %s [%s] %d", cp, whozis->name, whozis->addrs[0], z);
...
void oprint(which, buf, n) int which;
char* buf;
int n;
{
...
void atelnet(buf, size) unsigned char* buf; /* has to be unsigned here! */
unsigned int size;
{
    ...
    while (argv[optind]) {
        ...
        Debug(("Single %d, curport %d", Single, curport))
            ...
            Debug(("netfd %d from port %d to port %d", netfd, ourport,
                   curport)) if (netfd > 0) if (o_zero && o_udpmode) { /* if UDP scanning... */
#ifdef INET6
                if (want6)
...
#ifdef INET6
                if (want6)
                    holler("%s [%s] %d (%s) open", whereto6->name, whereto6->addrs[0], curport, portpoop->name);
                else
#endif
...
#ifdef INET6
                    if (want6)
                        holler("%s [%s] %d (%s)", whereto6->name, whereto6->addrs[0], curport, portpoop->name);
                    else
#endif
                        holler("%s [%s] %d (%s)", whereto->name, whereto->addrs[0], curport, portpoop->name);
                }
            } /* if netfd */
